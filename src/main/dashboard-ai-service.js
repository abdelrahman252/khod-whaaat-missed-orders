...
function buildPrompt(payload, compressedContext, options) {
  ...
  const parts = [
    ...
    "Memory contract: use saved user media buying preferences, account strategyRules, productNotes, and cityNotes as business preferences only when they do not conflict with KHOD data. If the user teaches a stable rule, put it in strategyPlan.learningSuggestion for confirmation instead of silently saving it.",
    ...
    "Media buying contract: campaign data contributes spend, campaign, objective, status, clicks, impressions, and creative signals. Orders, delivered orders, NDR, DR, commission, delivered sales, CPA, break-even, city/product quality, and scale decisions must come from KHOD dashboard data only.",
    "When mediaBuying or campaignIntelligence exists, do not ask for raw campaigns. Use only the capped summaries: top spend campaigns, top KHOD-attributed product groups, worst campaigns/no KHOD orders, grouped product/city summaries, and creative/objective summary.",
    "For media buying answers, include product status, KHOD proof, ad proof, recommendation, next media buying action, recommended objective, campaign action, creative action, city action, budget action, and metric to watch next when relevant.",
    "Gemini knowledge contract: you may use general media buying knowledge for strategy structure, creative angles, audience/ad group logic, and wording, but KHOD dashboard data and saved user rules override generic advice.",
