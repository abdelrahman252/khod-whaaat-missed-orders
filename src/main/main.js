...
const { validateCurrentYearDashboardRange } = require("./dashboard-date-range");
...
function runSoonAfterStartup(task, delayMs = 500) {
  setTimeout(() => {
    try {
      task();
    } catch (error) {
      log.warn("[Startup] Deferred task failed:", error && error.message ? error.message : error);
    }
  }, delayMs);
}
...
const dashboardQueryService = createDashboardQueryService({
  readFlags: readDashboardRolloutFlags,
  logger: log,
  loadDashboardAccounts: () => dashboardStore.get("accounts", {}),
  getAccountMeta: (accountId) => {
    const account = getStoredAccountById(accountId);
    return {
      label: accountDisplayName(account, accountId),
      email: account && (account.khodEmail || account.easyEmail || account.email || "") || "",
    };
  },
});
...
function dashboardRowsInRange(rows, dateFrom, dateTo) {
  const from = normalizeDashboardDateKey(dateFrom);
  const to = normalizeDashboardDateKey(dateTo);
  return (Array.isArray(rows) ? rows : []).filter((row) => {
    const key = dashboardRowDateKey(row);
    return !from || !to || (key && key >= from && key <= to);
  });
}

function validateDashboardReplacement(existingRows, incomingRows, dateFrom, dateTo) {
  const existing = dashboardRowsInRange(existingRows, dateFrom, dateTo).length;
  const incoming = dashboardRowsInRange(incomingRows, dateFrom, dateTo).length;
  return {
    existingRows: existing,
    incomingRows: incoming,
    suspicious: existing >= 50 && incoming < existing,
  };
}
...
function persistDashboardSnapshot(accountId, data, options = {}) {
  ...
  const validation = validateDashboardReplacement(accounts[accountId].snapshot, rows, rangeFrom, rangeTo);
  ...
  if (requiresConfirmation && options.requireConfirmation && !options.allowSuspiciousReplacement) {
    return { saved: false, rows, validation, requiresConfirmation };
  }
  ...
  return { saved: true, rows, mergedRows, validation, requiresConfirmation, enriched };
}
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    backgroundColor: bgColor, icon: getIconPath(), show: !startHiddenInTray,
  });
  ...
  mainWindow.once("ready-to-show", () => {
    if (startHiddenInTray) mainWindow.hide();
  });
...
app.whenReady().then(() => {
  ...
  runSoonAfterStartup(() => createTray(), 800);
  runSoonAfterStartup(() => purgeOldAnalyticsRuns(store.get("analyticsPurgeDays", 30)), 1200);
...
function persistAccountsAfterAdminDelete(nextAccounts) {
  ...
  if (first) {
    ...
    ["easyEmail", "easyPassword", "easyStore", "khodEmail", "khodPassword", "khodCountry"].forEach(key => {
      store.delete(key);
    });
  }
  invalidateAnalyticsRunsCache();
    ...
    try {
      ...
      if (Array.isArray(rows)) {
        if (accounts.length > 0) {
          const serverHashes = rows.map(r => r.account_hash).filter(Boolean);
          const keptAccounts = [];
          const deletedAccounts = [];
          for (const account of accounts) {
            const hash = accountHash(account);
            if (serverHashes.includes(hash)) keptAccounts.push(account);
...
ipcMain.handle("run-dashboard-fetch", async (_, { accountId, dateFrom, dateTo } = {}) => {
  ...
  dateFrom = rangeValidation.dateFrom;
  dateTo = rangeValidation.dateTo;
    ...
    child.on("message", async (msg) => {
      if (msg.type === "dashboard-result") {
        ...
        try {
          ...
          persistDashboardSnapshot(dashboardAccountId, {
            snapshot: rows,
            ...
            source: "live-fetch",
            timestampKey: "autoFetchTimestamp",
          });
        } catch (e) {
        ...
        safeResolve({ success: true, rows: rows.length, snapshotMonth: msg.snapshotMonth, exportDateFrom: msg.exportDateFrom || "", exportDateTo: msg.exportDateTo || "" });
...
function staticDashboardAccount(accountId) {
  const accounts = store.get("accounts", []) || [];
  if (accountId === "__single__" && accounts.length === 0) {
    return {
      id: "__single__",
      khodEmail: store.get("khodEmail", ""),
  ...
  const account = accounts.find((candidate) => candidate && String(candidate.id) === accountId);
  if (!account) throw new Error("Dashboard account was not found.");
  return account;
}

function prepareStaticDashboardUpdate(payload = {}) {
  const accountId = String(payload.accountId || "");
  staticDashboardAccount(accountId);
  const processed = processDashboardSheet({
    khodBuffer: payload.khodBuffer,
    dateFrom: payload.dateFrom,
    dateTo: payload.dateTo,
  });
  const accounts = dashboardStore.get("accounts", {});
  const existingRows = accounts[accountId] && accounts[accountId].snapshot;
  const validation = validateDashboardReplacement(existingRows, processed.rows, processed.dateFrom, processed.dateTo);
  return {
    accountId,
    rows: processed.rows,
    ...
    validation,
    requiresConfirmation: validation.suspicious || processed.rows.length === 0,
  };
}
...
function staticDashboardResult(prepared, extra = {}) {
  return {
    ...
    status: extra.status || (prepared.requiresConfirmation ? "confirmation_required" : "ready"),
    parsedRows: prepared.rows.length,
    parsedOrders: prepared.rows.length,
    warnings: prepared.warnings || [],
    dateFrom: prepared.dateFrom,
    dateTo: prepared.dateTo,
    snapshotMonth: prepared.snapshotMonth,
    parseDiagnostics: prepared.parseDiagnostics,
...
ipcMain.handle("inspect-static-dashboard-update", async (_, payload) => {
  try {
    if (licenseStore.get("dashboardEnabled", false) !== true) throw new Error("Dashboard access is not enabled.");
    return staticDashboardResult(prepareStaticDashboardUpdate(payload), { saved: false });
  } catch (err) {
    console.error("[Dashboard] inspect-static-dashboard-update error:", err.message);
    monitoring.captureException(err, { operation: "dashboard.inspectStaticUpdate", extra: { accountId: payload && payload.accountId } });
    return { ok: false, status: "failure", accountId: payload && payload.accountId, warnings: [], error: err.message };
  }
});
...
ipcMain.handle("apply-static-dashboard-update", async (_, payload) => {
  try {
    ...
    const persisted = persistDashboardSnapshot(prepared.accountId, {
      ...
      allowSuspiciousReplacement: payload && payload.allowSuspiciousReplacement === true,
    });
    return staticDashboardResult(prepared, {
      saved: persisted.saved,
      status: persisted.saved ? "success" : "confirmation_required",
      storedRows: persisted.saved ? persisted.mergedRows.length : null,
      enriched: persisted.saved ? persisted.enriched : 0,
    });
  } catch (err) {
    console.error("[Dashboard] apply-static-dashboard-update error:", err.message);
    monitoring.captureException(err, { operation: "dashboard.applyStaticUpdate", extra: { accountId: payload && payload.accountId } });
    return { ok: false, status: "failure", accountId: payload && payload.accountId, warnings: [], error: err.message };
  }
});
...
ipcMain.handle("save-dashboard-snapshot", async (_, accountId, data) => {
  try {
    const persisted = persistDashboardSnapshot(accountId, data, {
      source: "manual-snapshot",
      timestampKey: "manualFetchTimestamp",
    });
...
ipcMain.handle("get-dashboard-query-flags", async () => {
  return dashboardQueryService.flags();
});

ipcMain.handle("query-dashboard-data", async (_, payload) => {
  try {
    return await dashboardQueryService.query(payload || {});
  } catch (err) {
    monitoring.captureException(err, { operation: "dashboard.queryData" });
    return { ok: false, error: err.message || String(err), code: "DASHBOARD_QUERY_ERROR" };
  }
});
...
function getCachedMarketingStatus(accountId, platform) {
  ...
  if (accountId === "__all__") {
    ...
    allSummary.lastSyncAt = latestSyncAt;
    allSummary.reportTimezone = platform === "tiktok" ? "Local" : "platform_default";
...
async function callMarketingBackend(action, accountId, platform, range) {
  ...
  const marketingFlags = readDashboardRolloutFlags();
  const syncMode = (action === "sync" || action === "sync_all") && marketingFlags.marketingIncrementalSync ? "incremental" : "legacy";
  ...
  log.info("[Marketing][Main] response", {
    ...
    claimableAccountCount: Array.isArray(result && result.claimableAccounts) ? result.claimableAccounts.length : 0,
    mappedAccountCount: Array.isArray(result && result.mappedAccounts) ? result.mappedAccounts.length : (result && result.mappings ? Object.keys(result.mappings).length : 0),
    ...
    mode: result && result.mode || result && result.cache && result.cache.mode || "",
...
ipcMain.handle("claim-marketing-source-account", async (_, accountId, platform = "tiktok", sourceAccountId = "") => {
  ...
  try {
    ...
    monitoring.captureException(error, { operation: "marketing.claim", extra: { platform } });
    return { ok: false, error: error.message };
  }
});
...
ipcMain.handle("release-marketing-source-account", async (_, accountId, platform = "tiktok", sourceAccountId = "") => {
  ...
  try {
    ...
    monitoring.captureException(error, { operation: "marketing.release", extra: { platform } });
    return { ok: false, error: error.message };
  }
});
...
ipcMain.handle("open-external-url", async (_, externalUrl) => {
  try {
    ...
    const allowedHosts = new Set(["onboard.windsor.ai", "khod-whaaat-portal.vercel.app"]);
