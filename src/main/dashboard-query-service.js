...
function productNameOverride(sku, fallback, overrides, overrideKeys) {
  const wanted = lower(sku);
  if (!wanted) return text(fallback);
  const keys = overrideKeys || Object.keys(overrides || {});
  if (!keys.length) return text(fallback);
  const key = keys.find((candidate) => lower(candidate) === wanted);
...
function statusBucket(row) {
  ...
  const status = lower(explicit || row.orderStatus || row.status);
  if (!status || status === "real" || status === "missed" || status === "pending khod sync") return "other";
  if (status === "canceled_by_you") return "canceled_by_you";
  if (status === "pending" || status === "received") return "pending";
  if (status === "confirmed") return "confirmed";
  if (status === "processing" || status === "under processing" || status === "under_processing") return "processing";
  if (status === "waiting" || status === "awaiting shipment" || status === "awaiting_shipment") return "waiting";
  if (status === "shipping" || status === "in shipping" || status === "in_shipping") return "shipping";
  if (status === "delivered") return "delivered";
  if (status === "failed" || status === "returned" || status === "return") return "failed";
  if (status === "canceled" || status === "cancelled" || status === "cancelled by you" || status === "canceled by you") return "canceled";
  return "other";
}
...
function isLifecycleBucket(bucket) {
  return ["pending", "confirmed", "processing", "waiting", "shipping", "delivered", "failed", "canceled"].includes(bucket);
}

function isNetOrderBucket(bucket) {
  return isLifecycleBucket(bucket);
}
...
function isIncomingBucket(bucket) {
  return ["shipping", "confirmed", "processing", "waiting", "pending"].includes(bucket);
}
...
function rowTotal(row) {
  return number(row.dashboardTotalPrice ?? row.totalPrice ?? row.priceNoShipping ?? row.subtotal ?? row.salesValue ?? row.amountDue ?? row.total ?? row.orderValue);
}
...
function rowProfit(row) {
  ...
  return number(row && (row.profitAfterTax ?? row.khodProfit ?? row.taagerProfit ?? row.profitAfterFees ?? row.commission ?? row.marketerCommission));
}
...
const PREPAID_METHODS = ['prepaid', 'online', 'card', 'visa', 'mada', 'apple pay', 'applepay', 'stc pay', 'stcpay', 'tabby', 'tabi', 'tamara', 'paymob', 'network'];
...
function stableRowIdentity(row) {
  return text(row && (row._stableKey || row.orderScopeKey || row.key || row.campaignId || row.khodOrderNumber || row.id || row.taagerOrderNumber || row.orderNumber)) +
    "|" + text(row && (row.accountId || row.dashboardAccountId));
}

function accountCountry(account, row) {
  return text(row && (row.khodCountry || row.taagerCountry || row.country) || account && (account.khodCountry || account.country || account.taagerCountry) || "sa").toLowerCase();
}
...
function normalizeOrderMoney(row, account, input) {
  ...
  return {
    ...
    khodProfit: reportingProfit,
    taagerProfit: reportingProfit,
    commission: reportingProfit,
    khodCountry: accountCountry(account, row),
    taagerCountry: accountCountry(account, row),
  };
}
...
function createDashboardQueryService(options) {
  ...
  function scopedRows(input, options = {}) {
    ...
    // Page, sort and filter controls do not affect snapshot scoping or money
    // normalization. Cache that expensive shared preparation independently so
    // Orders, Products, Campaigns and Cities reuse the same normalized rows.
    const scopeCacheInput = {
      accountIds: input && input.accountIds,
      dateFrom: input && input.dateFrom,
      dateTo: input && input.dateTo,
      deliveredDateMode: input && input.deliveredDateMode,
      ndrDateFrom: input && (input.ndrDateFrom || input.ndrFrom),
      ndrDateTo: input && (input.ndrDateTo || input.ndrTo),
      reportingCurrency: input && (input.reportingCurrency || input.currency),
      exchangeRates: input && input.exchangeRates,
      exchangeRatesUpdatedAt: input && input.exchangeRatesUpdatedAt,
      exchangeRateSource: input && input.exchangeRateSource,
      egpRate: input && input.egpRate,
      includeNdrUnion,
      includeCreatedUnion,
    };
    return cached("scoped-rows", scopeCacheInput, () => {
      ...
      Object.keys(marketing).forEach((platform) => {
        ...
        (Array.isArray(rows) ? rows : []).forEach((row, index) => {
          ...
          campaigns.push({
            ...
            accountId,
            dashboardAccountId: accountId,
            country: row.khodCountry || row.country || accounts[accountId] && (accounts[accountId].khodCountry || accounts[accountId].country || accounts[accountId].taagerCountry) || "",
  ...
  function assignCampaignProducts(campaigns, products, productNameOverrides) {
    ...
    return campaigns.map((campaign) => {
      ...
      return match ? {
        ...
        product: productNameOverride(match.sku, match.name, overrides, overrideKeys),
  ...
  function campaignProductRows(rows, productNameOverrides) {
    ...
    rows.forEach((row, index) => {
      ...
      const name = productNameOverride(sku, rowProduct(row) || sku || "Unknown product", overrides, overrideKeys);
      const productKeyValue = String(sku || name).toLowerCase();
      if (!productKeyValue) return;
      const country = lower(row.khodCountry || row.taagerCountry || row.country || "unknown");
      ...
      const rawOrderKey = orderKey(row, index);
      const uniqueOrderKey = key + ":" + rawOrderKey;
      const lifecycleOrder = isLifecycleBucket(bucket);
      const visibleOrder = isNetOrderBucket(bucket);
      if (!seen.has("campaignTotal:" + uniqueOrderKey)) {
        ...
        if (lifecycleOrder) {
          ...
          if (bucket === "canceled") product.canceledCount += 1;
        }
      }
      if (visibleOrder && !seen.has("campaignPlaced:" + uniqueOrderKey)) {
        seen.add("campaignPlaced:" + uniqueOrderKey);
        product.orderKeys.add(rawOrderKey);
        ...
        if (isFailedBucket(bucket)) product.failedCount += 1;
          ...
          else if (bucket === "pending") product.pendingCount += 1;
        }
      }
      if (visibleOrder && !seen.has("campaignNdr:" + uniqueOrderKey)) {
        seen.add("campaignNdr:" + uniqueOrderKey);
        product.ndrBaseCount += 1;
      }
      if (visibleOrder && !seen.has("campaignSales:" + uniqueOrderKey)) {
        seen.add("campaignSales:" + uniqueOrderKey);
        product.totalSales += rowTotal(row);
      }
      if (bucket === "delivered" && visibleOrder && !seen.has("campaignNdrDelivered:" + uniqueOrderKey)) {
        seen.add("campaignNdrDelivered:" + uniqueOrderKey);
        product.ndrDeliveredCount += 1;
      }
      if (bucket === "delivered" && visibleOrder) {
    ...
    return Array.from(products.values()).map((product) => {
      ...
      return {
        ...
        netOrderKeys: Array.from(product.orderKeys || []),
      };
    ...
    return cached("orders", input, () => {
      ...
      orders = orders.filter((row) => {
        ...
        if (filters.stageId) {
          ...
          const isExplicitExactStatus = stageId.indexOf("status:") === 0;
          const requestedBucket = isExplicitExactStatus ? stageId.slice(7) : stageId;
          if (requestedBucket === "confirmed" && !isExplicitExactStatus) {
            if (!isConfirmedBucket(bucket)) return false;
          } else if (bucket !== requestedBucket) return false;
        }
        ...
        return lower([
          row.khodOrderNumber, row.taagerOrderNumber, row.orderNumber, row.customerName, row.name, row.phone, row.phone1,
  ...
  function productRows(input) {
    return cached("products", input, () => {
      ...
      const productNameOverrideKeys = Object.keys(productNameOverrides);
      ...
      rows.forEach((row, index) => {
        ...
        if (inCreatedPeriod && isNetOrderBucket(bucket)) globalNetOrderKeys.add(productOrderKey);
        ...
        if (inCreatedPeriod && isNewOrder) {
          ...
          if (isNetOrderBucket(bucket)) {
            product.netOrderCount += 1;
            const createdAtKey = dateKey(row.createdAt || row.date || row.dashboardDate);
            if (createdAtKey && (!product.firstOrderCreatedAt || createdAtKey < product.firstOrderCreatedAt)) {
              product.firstOrderCreatedAt = createdAtKey;
            }
          }
          if (bucket === "canceled") product.canceledCount += 1;
          if (isFailedBucket(bucket)) product.failedCount += 1;
          ...
          if (bucket === "pending") product.pendingCount += 1;
        }
        ...
        if (inNdrCohort && isNetOrderBucket(bucket)) {
        ...
        if (inNdrCohort && isNetOrderBucket(bucket) && !product.ndrOrderKeys.has(productOrderKey)) {
        ...
        if (inCreatedPeriod && isNewOrder && isNetOrderBucket(bucket)) {
      ...
      if (text(input && input.deliveredDateMode) === "expected" && ndrRows.length) {
        ndrRows.forEach((row, index) => {
          const bucket = statusBucket(row);
          if (!isNetOrderBucket(bucket)) return;
          const productOrderKey = orderKey(row, index);
          globalNdrOrderKeys.add(productOrderKey);
          if (isConfirmedBucket(bucket)) globalNdrConfirmedOrderKeys.add(productOrderKey);
          if (bucket === "delivered") globalNdrDeliveredOrderKeys.add(productOrderKey);
      ...
      rawOrderRows.forEach((row) => {
        ...
        if (!productMap.has(key)) {
          productMap.set(key, {
            key,
            sku,
            name: productNameOverride(sku, rowProduct(row), productNameOverrides, productNameOverrideKeys),
            country,
            accounts: {},
          });
        }
  ...
  function campaignOverview(input) {
    return cached("campaign-overview", input, () => {
      ...
      return {
        ...
        sourceOfTruth: "Product decisions use unified SKU or exact full-name campaign attribution with KHOD WHAAT orders, delivery, and marketer commission. Unknown campaign SKUs remain unmatched. Campaign rows retain native ad-account money.",
        lastSyncAt,
        totals: intel.totals,
  ...
  function citiesQuery(input) {
      return cached("cities", input, () => {
        const { rows, ndrRows, scope, accounts, periodFlags } = scopedRows(input, { includeNdrUnion: true });
        const cityStats = {};
        const reportingCurrency = cleanCurrency(input && (input.reportingCurrency || input.currency) || "SAR", "SAR");
      ...
      const ndrTo   = dateKey((input && (input.ndrDateTo   || input.ndrTo))   || "");
        ...
        rows.forEach((row, index) => {
          const rowCountry = lower(row.khodCountry || row.taagerCountry || row.country || "sa");
        ...
        if (!cityStats[cityKeyName]) {
          cityStats[cityKeyName] = {
            ...
            canceledCount: 0, failedCount: 0, shippingCount: 0, confirmedCount: 0, processingCount: 0,
            statusTotalCount: 0, confirmationStatusCount: 0, cancelStatusCount: 0, pendingStatusCount: 0,
        ...
        const bucket = statusBucket(row);
        const uniqueOrderKey = orderKey(row, index);
        const rowIsVisibleLifecycle = isLifecycleBucket(bucket);
        const rowIsNetOrder = isNetOrderBucket(bucket);
        ...
        if (inPrimaryPeriod && rowIsVisibleLifecycle && addOnce('status:' + uniqueOrderKey)) {
          cs.statusTotalCount++;
          const group = productStatusGroup(bucket);
        ...
        } else if (bucket === "canceled" && inPrimaryPeriod && rowIsVisibleLifecycle) {
          if (addOnce('canceled:' + uniqueOrderKey)) {
            cs.canceledCount++;
        ...
        if (cityProductKey) {
          if (!cs.productMap[cityProductKey]) {
            cs.productMap[cityProductKey] = {
              ...
              orders: 0, delivered: 0, failed: 0, canceled: 0, commission: 0, revenue: 0,
              activeOrders: 0, ndrBaseOrders: 0, confirmed: 0,
              statusTotalCount: 0, confirmationStatusCount: 0, cancelStatusCount: 0, pendingStatusCount: 0,
              prepaidCount: 0, codCount: 0,
              prepaidNdrBaseOrders: 0, codNdrBaseOrders: 0,
              prepaidDelivered: 0, prepaidFailed: 0, prepaidCanceled: 0,
              codDelivered: 0, codFailed: 0, codCanceled: 0
            };
          }
          const cp = cs.productMap[cityProductKey];

          if (inPrimaryPeriod && rowIsVisibleLifecycle && addOnce('cityProductStatus:' + cityProductKey + ':' + uniqueOrderKey)) {
            cp.statusTotalCount++;
            const group = productStatusGroup(bucket);
          ...
          if (isDeliveredInPeriod && rowIsNetOrder) {
            ...
            if (addOnce('cityProductFailed:' + cityProductKey + ':' + uniqueOrderKey)) {
              cp.failed++;
              if (rowIsPrepaid) {
                cp.prepaidFailed++;
              } else {
            ...
            if (addOnce('cityProductCanceled:' + cityProductKey + ':' + uniqueOrderKey)) {
              cp.canceled++;
              if (rowIsPrepaid) {
                cp.prepaidCanceled++;
              } else {
      ...
      if (citiesExpectedMode && ndrRows.length) {
        ndrRows.forEach((row, index) => {
          ...
          const cityName = text(row.city || row.customerCity || row.province || "");
          const bucket = statusBucket(row);
          if (!cityName || !isNetOrderBucket(bucket)) return;
      ...
      const sortedCities = Object.keys(cityStats).map((keyName) => {
        ...
        return {
          ...
          prepaidFailedCount: stat.prepaidFailedCount,
          codFailedCount: stat.codFailedCount,
