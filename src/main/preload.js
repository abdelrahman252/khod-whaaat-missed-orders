...
const APP_VERSION_ARG_PREFIX = "--khod-app-version=";

function resolvePreloadAppVersion() {
  const versionArg = Array.isArray(process.argv)
    ? process.argv.find((arg) => String(arg).startsWith(APP_VERSION_ARG_PREFIX))
    : null;
  return versionArg ? String(versionArg).slice(APP_VERSION_ARG_PREFIX.length) : "";
}

const PRELOAD_APP_VERSION = resolvePreloadAppVersion();
...
contextBridge.exposeInMainWorld("monitoring", {
  ...
  getMeta: () => ({ appVersion: PRELOAD_APP_VERSION }),
});
...
contextBridge.exposeInMainWorld("api", {
  ...
  approveBotPreview: (payload) => monitoredInvoke("approve-bot-preview", payload),
  isBulkOrderAccessConfigured: () => monitoredInvoke("bulk-orders:is-configured"),
  verifyBulkOrderAccess: (credentials) => monitoredInvoke("bulk-orders:verify-access", credentials),
  getBulkOrderSample: () => monitoredInvoke("bulk-orders:get-sample"),
  inspectRecoverySheets: (params) => monitoredInvoke("bulk-orders:inspect-recovery", params),
  getRecoveryProducts: (params) => monitoredInvoke("bulk-orders:get-recovery-products", params),
  previewRecoveryOrders: (params) => monitoredInvoke("bulk-orders:preview-recovery", params),
  runRecoveryOrders: (params) => monitoredInvoke("bulk-orders:run-recovery", params),
  inspectBulkOrderSheet: (params) => monitoredInvoke("bulk-orders:inspect-sheet", params),
  runBulkOrderSheet: (params) => monitoredInvoke("bulk-orders:run", params),
