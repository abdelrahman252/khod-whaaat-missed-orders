(function () {
  ...
  function query(kind, params, data) {
    if (!window.api || typeof window.api.queryDashboardData !== "function") {
      if (window.DashboardTrace) window.DashboardTrace.emit(kind, "query:unavailable", {});
    ...
    var traceTimer = window.DashboardTrace ? window.DashboardTrace.start(kind, "query", {
      requestChannel: payload.requestChannel || "active",
      page: payload.page || payload.campaignPage || 1,
      pageSize: payload.pageSize || payload.productPageSize || payload.campaignPageSize || null,
    ...
    return Promise.race([
      ...
      traceResult = result;
      if (requestSeq[requestKey] !== seq) {
        traceResult = { ok: false, stale: true };
        return traceResult;
      }
      ...
      traceResult = { ok: false, error: error && error.message ? error.message : String(error || "DASHBOARD_QUERY_FAILED"), kind: kind };
      return traceResult;
    }).finally(function () {
      if (timeoutTimer) window.clearTimeout(timeoutTimer);
      if (traceTimer && window.DashboardTrace) {
        var pagination = traceResult && traceResult.pagination || {};
        window.DashboardTrace.end(traceTimer, {
          ok: !!(traceResult && traceResult.ok),
          stale: !!(traceResult && traceResult.stale),
          timeout: !!(traceResult && traceResult.timeout),
          rows: Array.isArray(traceResult && traceResult.rows) ? traceResult.rows.length : null,
          total: pagination.total != null ? pagination.total : null,
          error: traceResult && traceResult.error || ""
        });
      }
    });
  }
  ...
  function stableIdentity(row) {
    ...
    var direct = text(row.orderScopeKey || row.key || row.legacyKey || row.campaignId || row.id || row.khodOrderNumber || row.taagerOrderNumber || row.orderNumber || row.sku || row.name || row.campaign || "");
  ...
  function statusBucket(row) {
    ...
    var status = explicit || text(row && (row.orderStatus || row.status));
    ...
    var key = status.toLowerCase().replace(/_/g, " ");
    if (key === "pending" || key === "confirmed" || key === "processing" || key === "waiting" || key === "shipping" || key === "delivered" || key === "failed" || key === "canceled") return key;
    if (key === "under processing") return "processing";
    if (key === "in shipping") return "shipping";
    if (key === "cancelled") return "canceled";
    return "other";
  }

  function orderValue(row) {
    return number(row && (row.totalPrice != null ? row.totalPrice : row.priceNoShipping != null ? row.priceNoShipping : row.subtotal != null ? row.subtotal : row.salesValue != null ? row.salesValue : row.dashboardTotalPrice != null ? row.dashboardTotalPrice : row.total != null ? row.total : row.orderValue));
  }

  function orderProfit(row) {
    return number(row && (row.marketerCommission != null ? row.marketerCommission : row.commission != null ? row.commission : row.commissionValue != null ? row.commissionValue : row.dashboardCommission != null ? row.dashboardCommission : row.profitAfterTax != null ? row.profitAfterTax : row.taagerProfit));
  }

  function orderIdentity(row, index) {
    var accountId = text(row && (row.accountId || row.dashboardAccountId || row.account || ""));
    var direct = text(row && (row.orderScopeKey || row.khodOrderNumber || row.taagerOrderNumber || row.orderNumber || row.orderId || row.id || row.reference || ""));
    if (direct && accountId && direct.indexOf(accountId + "|") === 0) return direct;
  ...
  function compareOrderShadow(data, result) {
    ...
    legacyRows.forEach(function (row) {
      ...
      if (bucket !== "other") statusBreakdown[bucket] = (statusBreakdown[bucket] || 0) + 1;
  ...
  function compareCampaignShadow(data, result) {
    var intelligence = window.KhodCampaignIntelligence || window.TaagerCampaignIntelligence;
  ...
  function observe(sectionId, data) {
    // A Cities query scans the full scoped order set. The Cities section owns
    // that refresh so we do not accidentally run two city scans for one open.
