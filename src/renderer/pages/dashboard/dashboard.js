...
(function () {
  ...
  window.renderDashboard = function () {
    ...
    var marketingUiSections = ['products', 'campaigns', 'dailyPerformance', 'commission', 'calculator', 'productForecast', 'taagerAi'];
    ...
    function exactMarketingCacheMatches(status, syncPayload) {
      var summary = status && status.summary;
      if (!summary || !syncPayload) return false;
      var summaryFrom = String(summary.dateFrom || '').slice(0, 10);
      var summaryTo = String(summary.dateTo || '').slice(0, 10);
      var payloadFrom = String(syncPayload.dateFrom || '').slice(0, 10);
      var payloadTo = String(syncPayload.dateTo || '').slice(0, 10);
      var summaryCurrency = String(summary.currency || '').toUpperCase();
      var payloadCurrency = String(syncPayload.targetCurrency || '').toUpperCase();
      var summaryRate = Number(summary.egpRate || 0);
      var payloadRate = Number(syncPayload.egpRate || 0);
      var rateMatches = !summaryRate || !payloadRate || Math.abs(summaryRate - payloadRate) < 0.0001;
      var payloadRates = syncPayload.exchangeRates && typeof syncPayload.exchangeRates === 'object' ? syncPayload.exchangeRates : null;
      ...
      if (payloadRates) {
        ...
        var normalizedSummaryRates = {};
        var normalizedRequestRates = {};
        supported.forEach(function (currency) {
          normalizedSummaryRates[currency] = Number(summaryRates[currency]) || 0;
          normalizedRequestRates[currency] = Number(syncPayload.exchangeRates[currency]) || 0;
        });
        rateMatches = JSON.stringify(normalizedSummaryRates) === JSON.stringify(normalizedRequestRates) && !!JSON.stringify(syncPayload.exchangeRates);
      }
      return !!(
    ...
    function triggerMarketingSync(reason) {
      ...
      if (store && typeof store.sync === 'function' && syncPayload.dateFrom && syncPayload.dateTo) {
        ...
        var hasExactCachedRange = exactMarketingCacheMatches(cachedStatus, syncPayload) && !cachedStatus.error && !cachedStatus.reconnectRequired;
    ...
    function refreshAfterBackgroundMarketing(marketingPromise) {
      if (!marketingPromise || typeof marketingPromise.then !== 'function') return;
      Promise.resolve(marketingPromise).catch(function () { return null; }).then(function () {
        if (activeMarketingSyncPromise === marketingPromise) {
          activeMarketingSyncPromise = null;
          activeMarketingSyncKey = '';
        }
        updateMarketingPendingOverlay();
      });
    }
    ...
    function syncMarketingAndRunAggregator(showLoader, reason) {
      ...
      // saved order data and local aggregation should make the dashboard usable
      // immediately. Marketing refreshes in the background and its section
      // listeners quietly update only when spend or sync identity really changes.
      triggerMarketingSync(reason);
      refreshAfterBackgroundMarketing(activeMarketingSyncPromise);
      return runAggregator(showLoader);
    }

    window.syncDashboardMarketingOnOpen = function () {
      return syncMarketingAndRunAggregator(false, 'dashboard-open');
    };
