  ...
  window.renderDashboard = function () {
    ...
    function runAggregator(showLoader) {
      ...
      window.runDashboardAggregator(function (result) {
        ...
        dashData._loading = true;
        var rolloutReady = window.DashboardQueryRuntime && typeof window.DashboardQueryRuntime.applyDashboardRollouts === 'function'
          ? window.DashboardQueryRuntime.applyDashboardRollouts(dashData)
          : Promise.resolve(dashData);
        rolloutReady.then(function () {
          return ensureMarketingStatusLoaded(dashData);
        }).then(function () {
          dashData._loading = false;
          dashData._version = ++dashVersion;
          window.dashboardGeoData = dashData;
