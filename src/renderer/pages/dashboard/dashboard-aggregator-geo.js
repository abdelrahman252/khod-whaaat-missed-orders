...
(function () {
  ...
  /* ── Province metadata — single source of truth for name, color, coords ──── */
  var PROVINCE_META = {
    riyadh:   { name: 'منطقة الرياض',           color: '#a855f7', x: 230.6, y: 164.1, rx: 95, ry: 80 },
    eastern:  { name: 'المنطقة الشرقية',         color: '#14b8a6', x: 298,   y: 140,   rx: 55, ry: 45 },
    mecca:    { name: 'منطقة مكة المكرمة',      color: '#3b82f6', x: 95,    y: 238,   rx: 50, ry: 38 },
    jazan:    { name: 'منطقة جازان',             color: '#ec4899', x: 158,   y: 304,   rx: 28, ry: 22 },
    baha:     { name: 'منطقة الباحة',            color: '#f97316', x: 132,   y: 258,   rx: 24, ry: 18 },
    madinah:  { name: 'منطقة المدينة المنورة',  color: '#f59e0b', x: 92,    y: 175,   rx: 38, ry: 30 },
    aseer:    { name: 'منطقة عسير',              color: '#ef4444', x: 152,   y: 272,   rx: 35, ry: 24 },
    qassim:   { name: 'منطقة القصيم',            color: '#8b5cf6', x: 194,   y: 132,   rx: 38, ry: 30 },
    tabuk:    { name: 'منطقة تبوك',              color: '#0ea5e9', x: 80,    y: 110,   rx: 42, ry: 32 },
    hail:     { name: 'منطقة حائل',              color: '#84cc16', x: 160,   y: 125,   rx: 32, ry: 24 },
    najran:   { name: 'منطقة نجران',             color: '#06b6d4', x: 198,   y: 304,   rx: 28, ry: 20 },
    jawf:     { name: 'منطقة الجوف',             color: '#a3e635', x: 140,   y: 80,    rx: 30, ry: 22 },
    northern: { name: 'منطقة الحدود الشمالية',  color: '#fb923c', x: 100,   y: 70,    rx: 28, ry: 20 },
  ...
  window.buildGeoProductMap = function (cityStats, productStats, nationalAverages) {
    ...
    var T = typeof window.getDashboardThresholds === 'function'
      ...
      : { NDR_DANGER: 0.45, NDR_SAFE: 0.75, PREPAID_ADVANTAGE_THRESHOLD: 0.15, SCALING_MIN_ORDERS: 30 };
    ...
    Object.keys(cityStats).forEach(function (cityName) {
      ...
      Object.keys(productMap).forEach(function (productKey) {
        ...
        var ndr = safeNdr(cp.delivered || 0, cp.orders || 0);
        ...
        var codNdr    = safeNdr(codDelivered, cp.codCount || 0);
        var prepaidNdr = safeNdr(prepaidDelivered, cp.prepaidCount || 0);
  ...
  function _buildPrepaidIntelligence(cityStats, geoProductMap, nationalAverages, T) {
    ...
    Object.keys(cityStats).forEach(function (cityName) {
      ...
      if ((cs.count || 0) >= 10) {
        cityPrepaidList.push({ city: cityName, prepaidPct: prep, orders: cs.count });
      }

      // COD-heavy and dangerous
      var cityNdr = safeNdr(cs.deliveredOrders || 0, cs.count || 0);
      var codPct  = safePct(cs.codCount || 0, cs.count || 0);
      if (codPct > T.COD_HEAVY_THRESHOLD && cityNdr < 0.45 && (cs.count || 0) >= 15) {
        codHeavyCities.push({
          city: cityName, codPct: codPct, codNdr: cityNdr,
          riskLevel: cityNdr < 0.35 ? 'critical' : 'high'
        });
      }
    });

    var globalPrepaidNdr = safeNdr(prepaidDelivered, totalPrepaid);
    var globalCodNdr     = safeNdr(codDelivered, totalCod);
