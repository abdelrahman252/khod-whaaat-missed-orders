...
(function () {
  ...
  var analysisCache = typeof WeakMap === 'function' ? new WeakMap() : null;
  ...
  function exactBucket(row) {
    ...
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.dashboardBucket === 'function') {
      return window.KhodDashboardLogic.dashboardBucket(status) || 'other';
    }
    ...
    if (status.indexOf('cancel') !== -1) return 'canceled';
    if (status.indexOf('fail') !== -1 || status.indexOf('return') !== -1) return 'failed';
    return status || 'other';
  }

  function isCanceledByYou(bucket) {
    return bucket === 'canceled' || bucket === 'cancelled' || bucket === 'canceled_by_you' || bucket === 'cancelled_by_you';
  }

  function isDelivered(bucket) {
    if (bucket === 'delivered') return true;
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.isDelivered === 'function') {
      return window.KhodDashboardLogic.isDelivered(bucket);
    }
  ...
  function productName(row) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.productName === 'function') return window.KhodDashboardLogic.productName(row);
  ...
  function indexOrdersByDate(orders) {
    return (orders || []).reduce(function (index, order) {
      if (!index[order.date]) index[order.date] = [];
      index[order.date].push(order);
      return index;
    }, {});
  }

  function summarizeWindow(orders, dateFrom, dateTo, ordersByDate) {
    var cities = {};
    var products = {};
    var summary = {
      dateFrom: dateFrom,
    ...
    if (ordersByDate) {
      var cursor = parseIso(dateFrom);
      var end = parseIso(dateTo);
      while (cursor && end && cursor <= end) {
        (ordersByDate[toIso(cursor)] || []).forEach(visitOrder);
        cursor = addDays(cursor, 1);
      }
    } else {
      orders.forEach(function (order) {
        if (order.date >= dateFrom && order.date <= dateTo) visitOrder(order);
      });
    }
  ...
  function analyzeUncached(data, opts) {
    opts = opts || {};
    ...
    // Index once, then visit only the days inside each candidate cycle. The
    // previous implementation rescanned the complete order array for every
    // possible window, making an unrelated dashboard section click O(days *
    // orders). A seven-day cycle now visits each order at most seven times.
    var ordersByDate = indexOrdersByDate(orders);
    var average = summarizeWindow(orders, period.dateFrom, period.dateTo, ordersByDate);
    var cycles = [];
    var best = null;
    ...
    for (var cursor = start; cursor <= lastStart; cursor = addDays(cursor, 1)) {
      ...
      var summary = summarizeWindow(orders, from, to, ordersByDate);
  ...
  function analyze(data, opts) {
    opts = opts || {};
    var period = periodFromData(data, opts);
    var cacheKey = period && period.dateFrom && period.dateTo
      ? [
          period.dateFrom,
          period.dateTo,
          Number(opts.cycleDays || DEFAULT_CYCLE_DAYS) || DEFAULT_CYCLE_DAYS,
          Number(opts.minSample || DEFAULT_MIN_SAMPLE) || DEFAULT_MIN_SAMPLE
        ].join('|')
      : '';
    if (analysisCache && data && typeof data === 'object') {
      var cached = analysisCache.get(data);
      if (cached && cached.key === cacheKey) return cached.result;
      var result = analyzeUncached(data, opts);
      analysisCache.set(data, { key: cacheKey, result: result });
      return result;
    }
    return analyzeUncached(data, opts);
  }
