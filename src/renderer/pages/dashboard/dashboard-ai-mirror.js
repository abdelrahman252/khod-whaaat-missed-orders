(function () {
  ...
  function decisionFromSharedEvaluator(input) {
    var evaluator = window.KhodCampaignDecision || window.TaagerCampaignDecision;
  ...
  function scoreProduct(p) {
    ...
    var decision = decisionFromSharedEvaluator({
      ...
      avgDeliveredCommission: next.avgCommissionSar,
  ...
  function normalizeCampaign(item) {
    ...
    var grossProfit = Number(item.commission || item.earnedCommission || item.profit || item.earnedProfitAfterTax || 0);
    ...
    var decision = decisionFromSharedEvaluator({
      ...
      avgDeliveredCommission: item.avgDeliveredCommission != null ? item.avgDeliveredCommission : item.avgDeliveredProfit,
      netProfit: netProfit
    });
    return {
      ...
      nextAction: taagerOrders === 0 && spend > 0 ? "Reduce spend until it creates KHOD-attributed orders." : decision.nextAction
    };
  }
  ...
  function buildMirror(data, opts) {
    ...
    var accountSummary = {
      ...
      earnedCommission: num(overview.earnedCommission && overview.earnedCommission.value || overview.earnedProfitAfterTax && overview.earnedProfitAfterTax.value || 0, 2),
      lostCommission: num(overview.lostCommission && overview.lostCommission.value || overview.lostProfitAfterTax && overview.lostProfitAfterTax.value || 0, 2),
      earnedProfitAfterTax: num(overview.earnedCommission && overview.earnedCommission.value || overview.earnedProfitAfterTax && overview.earnedProfitAfterTax.value || 0, 2),
      lostProfitAfterTax: num(overview.lostCommission && overview.lostCommission.value || overview.lostProfitAfterTax && overview.lostProfitAfterTax.value || 0, 2),
      netProfit: num((overview.earnedCommission && overview.earnedCommission.value || overview.earnedProfitAfterTax && overview.earnedProfitAfterTax.value || 0) - (context.productFinancials && context.productFinancials.accountAdSpend || data.roi && data.roi.adSpend || 0), 2),
    ...
    var rankings = {
      ...
      topCities: sortBy(cities, function (c) { return c.earnedCommission || c.earnedProfitAfterTax || 0; }, "desc").slice(0, 10),
  ...
  function factsFor(row, kind, currency, ar) {
    ...
    if (kind === "cities") {
      return (ar ? [
        ...
        (row.earnedCommission || row.earnedProfitAfterTax) ? "\u0627\u0644\u0639\u0645\u0648\u0644\u0629 \u0627\u0644\u0645\u062d\u0642\u0642\u0629 " + num(row.earnedCommission || row.earnedProfitAfterTax, 0) + (currency ? " " + currency : "") : ""
      ] : [
        row.orders ? row.orders + " orders" : "",
        row.delivered != null ? row.delivered + " delivered" : "",
        row.ndrPct != null ? "NDR " + row.ndrPct + "%" : "",
        (row.earnedCommission || row.earnedProfitAfterTax) ? "Earned marketer commission " + num(row.earnedCommission || row.earnedProfitAfterTax, 0) + (currency ? " " + currency : "") : ""
      ]).filter(Boolean);
    }
    return (ar ? [
      row.orders ? row.orders + " \u0637\u0644\u0628" : "",
  ...
  function compactPlan(command, mirror, row, kind, currency) {
    ...
    var watch = [
      ...
      row && (row.earnedCommission || row.earnedProfitAfterTax) ? (ar ? "\u0627\u0644\u0639\u0645\u0648\u0644\u0629 \u0627\u0644\u0645\u062d\u0642\u0642\u0629 " : "Earned marketer commission ") + num(row.earnedCommission || row.earnedProfitAfterTax, 0) + " " + currency : ""
  ...
  function accountDiagnosis(mirror, currency, ar) {
    ...
    if (a.lostCommission > a.earnedCommission) causes.push(ar ? "\u0627\u0644\u0639\u0645\u0648\u0644\u0629 \u0627\u0644\u0645\u0641\u0642\u0648\u062f\u0629 " + num(a.lostCommission, 0) + " " + currency + " \u0623\u0639\u0644\u0649 \u0645\u0646 \u0627\u0644\u0639\u0645\u0648\u0644\u0629 \u0627\u0644\u0645\u062d\u0642\u0642\u0629 " + num(a.earnedCommission, 0) + " " + currency : "lost marketer commission " + num(a.lostCommission, 0) + " " + currency + " exceeds earned marketer commission " + num(a.earnedCommission, 0) + " " + currency);
  ...
  function localAnswer(command, mirror, parsedIntent, localStrategic) {
    ...
    if (intent === "operator_next") {
      ...
      why = row ? (ar ? [row.spend + " " + row.currency + " \u0625\u0646\u0641\u0627\u0642", row.orders + " \u0637\u0644\u0628", "\u062a\u0643\u0644\u0641\u0629 CPA " + row.cpa + " " + row.currency] : [row.spend + " " + row.currency + " spend", row.orders + " KHOD orders", "CPA " + row.cpa]).filter(Boolean) : [];
      action = row ? (ar ? labels.fixAction : (row.nextAction || (row.action === "reduce" ? "Reduce spend until it creates KHOD-attributed orders." : "Keep campaign changes small and watch CPA versus break-even."))) : labels.nextFallback;
    } else {
      var a = mirror.accountSummary || {};
