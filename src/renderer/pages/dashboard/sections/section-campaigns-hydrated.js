(function () {
  ...
  function clearScheduledCampaignBackendQuery(mount) {
    if (!mount || !mount._campaignBackendIdle) return;
    if (window.cancelIdleCallback) window.cancelIdleCallback(mount._campaignBackendIdle);
    else clearTimeout(mount._campaignBackendIdle);
    mount._campaignBackendIdle = null;
  }

  function scheduleCampaignBackendQuery(mount, data, ctx, state, renderToken) {
    clearScheduledCampaignBackendQuery(mount);
    if (!mount || !mount._campaignBackendEnabled || mount._campaignBackendActive) return;
    var run = function () {
      mount._campaignBackendIdle = null;
      if (!mount.isConnected || mount.hidden || renderToken !== mount._campaignRenderToken) {
        mount._dashboardNeedsRefresh = true;
        return;
      }
      // Local intelligence is already visible. The backend page is only a
      // refinement, so it must never hold up the first complete paint.
      requestBackendCampaigns(mount, data, ctx, state, false);
    };
    if (window.requestIdleCallback) {
      mount._campaignBackendIdle = window.requestIdleCallback(run, { timeout: 350 });
    } else {
      mount._campaignBackendIdle = setTimeout(run, 32);
    }
  }
  ...
  function scheduleCampaignsUIUpdate(mount, data, ctx, state, intel, options) {
    ...
    if (backendRefresh && options.delay == null) delay = 80;
    if (backendRefresh) {
  ...
  function requestBackendCampaigns(mount, data, ctx, state, fullRender) {
    ...
    clearScheduledCampaignUpdate(mount);
    var params = backendCampaignParams(mount, data, state);
    var queryKey = JSON.stringify(params || {});
    if (mount._campaignBackendPendingPromise && mount._campaignBackendPendingKey === queryKey) {
      if (window.DashboardTrace) {
        window.DashboardTrace.emit("campaigns", "query:reuse-inflight", {
          fullRender: !!fullRender,
          campaignPage: params.campaignPage,
          productPage: params.productPage,
          pageSize: params.pageSize
        });
      }
      return mount._campaignBackendPendingPromise;
    }
    var requestId = Number(mount._campaignBackendRequest || 0) + 1;
    mount._campaignBackendRequest = requestId;
    mount._campaignBackendPendingKey = queryKey;
    mount._campaignBackendPendingPromise = window.DashboardQueryRuntime.query("campaign-overview", params, data).then(function (result) {
      if (mount._campaignBackendRequest === requestId) {
        mount._campaignBackendPendingPromise = null;
        mount._campaignBackendPendingKey = "";
      }
      if (requestId !== mount._campaignBackendRequest || !mount.isConnected) return false;
      // A query may finish after the user has left Campaigns. Keep the result,
      // but never build two wide tables inside a hidden cached pane.
      if (mount.hidden) {
        mount._campaignDeferredBackendResult = result;
      ...
      if (mount._campaignLegacyIntel && mount.isConnected && !mount.hidden) renderMainCampaignsUI(mount, data, ctx, state, mount._campaignLegacyIntel);
      return false;
    });
    return mount._campaignBackendPendingPromise;
  }
  ...
  function renderMainCampaignsUI(mount, data, ctx, state, intel) {
    ...
    mount.innerHTML = '<section class="campaign-section">' +
      ...
      '<div class="campaign-panel wide"><div class="campaign-panel-title"><div><h3>Product Actions</h3><span>Unambiguously attributed products only, using KHOD WHAAT product-level results. Product orders can differ from whole-account unique orders.</span></div><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end"><button type="button" class="campaign-ai-chip" data-campaign-ai-review>' + icon("sparkles") + 'Analyze actions</button></div></div>' +
        ...
        '<div class="campaign-table-scroll"><table class="campaign-table campaign-table-products"><thead><tr>' +
          ...
          sortableTh("KHOD WHAAT orders", "taagerOrders", state, "products") +
  ...
  function renderSectionCampaigns(mount, data, ctx) {
    ...
    // Never make the wide campaign tables wait for an IPC round trip. Build
    // the cached local view first; an enabled backend query refines it later.
    var initialSnapshot = buildCampaignIntelSnapshot(mount, data, ctx);
    state = initialSnapshot.state;
    mount._campaignLegacyIntel = initialSnapshot.intel;
    renderMainCampaignsUI(mount, data, ctx, state, initialSnapshot.intel);
    renderToken = Number(mount._campaignRenderToken || 0);

    var resolvedQueryFlags = window.DashboardQueryRuntime && typeof window.DashboardQueryRuntime.currentFlags === "function"
      ? window.DashboardQueryRuntime.currentFlags()
      : null;

    if (resolvedQueryFlags) {
      mount._campaignBackendEnabled = !!resolvedQueryFlags.campaigns;
      if (mount._campaignBackendEnabled) {
        scheduleCampaignBackendQuery(mount, data, ctx, state, renderToken);
      }
    } else if (window.DashboardQueryRuntime && typeof window.DashboardQueryRuntime.flags === "function") {
      window.DashboardQueryRuntime.flags().then(function (flags) {
        ...
        if (mount.hidden) {
          mount._dashboardNeedsRefresh = mount._campaignBackendEnabled;
        } else if (mount._campaignBackendEnabled) {
          scheduleCampaignBackendQuery(mount, data, ctx, state, renderToken);
        }
      }).catch(function () {});
    }
    ...
    var campaignsLifecycleActive = true;
    mount._dashboardSectionDeactivate = function () {
      if (!campaignsLifecycleActive) return;
      campaignsLifecycleActive = false;
      clearScheduledCampaignUpdate(mount);
      clearScheduledCampaignBackendQuery(mount);
    ...
    mount._dashboardSectionActivate = function () {
      if (campaignsLifecycleActive) return;
      campaignsLifecycleActive = true;
      if (mount._campaignRoiListener && window.DashboardRoiState) {
        window.DashboardRoiState.unsubscribe(mount._campaignRoiListener);
        window.DashboardRoiState.subscribe(mount._campaignRoiListener);
      }
      var deferredResult = mount._campaignDeferredBackendResult;
      mount._campaignDeferredBackendResult = null;
      if (deferredResult && deferredResult.ok) {
        clearCampaignRowsBusy(mount);
        mount._campaignBackendActive = true;
        mount._campaignIntel = backendCampaignIntel(deferredResult, data);
        updateCampaignsUIOnly(mount, data, ctx, state, mount._campaignIntel, { backendReady: true });
      } else if (mount._dashboardNeedsRefresh) {
        refreshCampaignCurrencyUIOnly(mount, data, ctx);
        if (mount._campaignBackendEnabled && !mount._campaignBackendActive) {
          scheduleCampaignBackendQuery(mount, data, ctx, state, Number(mount._campaignRenderToken || 0));
        }
      } else if (mount._campaignBackendEnabled && !mount._campaignBackendActive) {
        scheduleCampaignBackendQuery(mount, data, ctx, state, Number(mount._campaignRenderToken || 0));
      }
      mount._dashboardNeedsRefresh = false;
    };

    return function cleanupCampaigns() {
      mount._dashboardSectionDeactivate();
      if (mount._campaignRoiListener && window.DashboardRoiState) {
        window.DashboardRoiState.unsubscribe(mount._campaignRoiListener);
        mount._campaignRoiListener = null;
      }
      ...
      mount._dashboardSectionActivate = null;
      mount._dashboardSectionDeactivate = null;
    };
  }
