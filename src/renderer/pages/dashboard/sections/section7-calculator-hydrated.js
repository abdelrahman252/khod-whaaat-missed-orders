...
window.renderSection7HydratedEntry = function (mountEl, data, ctx) {
  ...
  var calcVisualPending = false;
  var calcPersistTimer = null;
  var growthChartSignature = '';
  ...
  function isCalculatorInputActive() {
    var active = document.activeElement;
    return !!(active && mountEl.contains(active) && (
      active.matches('input, select, textarea') ||
      active.isContentEditable
    ));
  }

  function renderDeferredCalcVisuals(options) {
    options = options || {};
    if (!options.force && !calcVisualPending) return;
    if (!mountEl.isConnected || mountEl.hidden) {
      mountEl._dashboardNeedsRefresh = true;
      return;
    }
    if (!options.force && isCalculatorInputActive()) {
      scheduleCalcVisuals(700);
      return;
    }
    calcVisualPending = false;
    var res = compute();
    buildGrowthChart(res.cpaSAR, res.spendSAR);
    renderScenarios(res.cpaSAR, res.spendSAR);
  }

  function scheduleCalcVisuals(delay) {
    calcVisualPending = true;
    if (calcVisualTimer != null) clearTimeout(calcVisualTimer);
    calcVisualTimer = setTimeout(function () {
      calcVisualTimer = null;
      requestAnimationFrame(function () {
        renderDeferredCalcVisuals();
      });
    }, delay || 700);
  }

  function flushCalcVisuals() {
    if (calcVisualTimer != null) {
      clearTimeout(calcVisualTimer);
      calcVisualTimer = null;
    }
    if (!calcVisualPending) return;
    renderDeferredCalcVisuals({ force: true });
  }
  ...
  function scheduleCalcUI() {
    ...
    calcUpdateFrame = requestAnimationFrame(function () {
      ...
      calcUpdateFrame = requestAnimationFrame(function () {
        calcUpdateFrame = null;
        if (!mountEl.isConnected || mountEl.hidden) { mountEl._dashboardNeedsRefresh = true; return; }
        updateCalcUI({ deferVisuals: true, quickInput: true });
      });
    });
    scheduleCalcVisuals(700);
  }
  ...
  var themeObserveOptions = { attributes: true, attributeFilter: ["data-theme"] };
  if (sectionResources) sectionResources.observe(observer, document.documentElement, themeObserveOptions);
  else observer.observe(document.documentElement, themeObserveOptions);
  ...
  var realAvgCommission = d.averageCommission != null
    ? Number(d.averageCommission)
    : (d.avgCommission != null ? Number(d.avgCommission) : (d.averageProfit != null ? Number(d.averageProfit) : 0));
  var realEarnedCommission = d.actualEarnedCommission != null
    ? Number(d.actualEarnedCommission)
    : (d.actualEarnedProfitAfterTax != null
  ...
  function compute() {
    ...
    var commissionSAR = isExpectedRateMode ? (realAvgCommission * realExpectedDvlExact) : realEarnedCommission;
    var netSAR = commissionSAR - spendSAR;
    var roi = spendSAR > 0 ? (netSAR / spendSAR) * 100 : 0;
    var returnPerSar = spendSAR > 0 ? commissionSAR / spendSAR : 0;
    ...
    return {
      ...
      profit: convert(commissionSAR, activeReportCurrency, state.currency),
  ...
  function computeProjection(budgetSAR, cpaSAR) {
    ...
    var projection = financialCore.calculate({
      mode: "expected",
  ...
  function computeSim() {
    ...
    var projected = financialCore.calculate({
      mode: "expected",
      netOrders: projOrders,
      actualDeliveredOrders: 1,
      actualEarnedProfitAfterTax: s.avgCommission,
  ...
  function buildGrowthChart(cpaSAR, baseSpendSAR) {
    ...
    if (!canvas) return;
    var nextChartSignature = JSON.stringify([Number(cpaSAR || 0), Number(baseSpendSAR || 0), state.currency]);
    if (canvas._chartInstance && canvas._chartInstance.canvas === canvas && growthChartSignature === nextChartSignature) return;
    growthChartSignature = nextChartSignature;
  ...
  function renderSimScenarioTable(c) {
    ...
    var rows = scenarios
      .map(function (sc) {
        var calc = financialCore.calculate({
          mode: "expected",
          netOrders: sc.orders,
          actualDeliveredOrders: 1,
  ...
  function updateCalcUI(options) {
    options = options || {};
    var quickInput = !!options.quickInput;
    if (syncedSpendActive && sourceBreakdown.length) {
      state.budget = syncedBudgetInCurrency();
      d.adSpend = state.budget;
      refreshBudgetInputValue(true);
    ...
    if (!quickInput && assignedMarketingAccounts.length) {
  ...
  function render() {
    mountEl.innerHTML =
      ...
      _tip(
        ...
        s7Txt(
          isExpectedRateMode
            ? "Expected delivered orders multiplied by the editable average marketer commission per delivered order."
            : "Actual earned marketer commission counted from delivered sheet status before subtracting synced ad spend.",
          "الطلبات المسلمة المتوقعة × متوسط الربح لكل طلب.",
        ),
        isExpectedRateMode
          ? "commissionBeforeAdSpend = expectedDeliveriesExact * averageMarketerCommissionPerDeliveredOrder"
          : "commissionBeforeAdSpend = actualEarnedMarketerCommission",
      ) +
    ...
    if (budgetInput) {
      ...
      budgetInput.addEventListener("input", function () {
        if (syncedSpendActive) return;
        state.budget = Number(String(budgetInput.value || "").replace(/[^0-9.-]/g, "")) || 0;
  ...
  function initTooltips() {
    ...
    if (sectionResources) {
      sectionResources.listen(document, "mouseover", mountEl._s7TooltipOver);
      sectionResources.listen(document, "mouseout", mountEl._s7TooltipOut);
    } else {
  ...
  var calculatorLifecycleActive = true;
  mountEl._dashboardSectionDeactivate = function () {
    if (!calculatorLifecycleActive) return;
    calculatorLifecycleActive = false;
    if (calcUpdateFrame != null) { cancelAnimationFrame(calcUpdateFrame); calcUpdateFrame = null; }
    if (calcUpdateTimer != null) { clearTimeout(calcUpdateTimer); calcUpdateTimer = null; }
    if (calcVisualTimer != null) { clearTimeout(calcVisualTimer); calcVisualTimer = null; }
    calcVisualPending = false;
    if (mountEl._s7MarketingListener && window.DashboardMarketingState) {
      window.DashboardMarketingState.unsubscribe(mountEl._s7MarketingListener);
    }
  };
  mountEl._dashboardSectionActivate = function () {
    if (calculatorLifecycleActive) return;
    calculatorLifecycleActive = true;
    if (mountEl._s7MarketingListener && window.DashboardMarketingState) {
      window.DashboardMarketingState.subscribe(mountEl._s7MarketingListener);
    }
  };
