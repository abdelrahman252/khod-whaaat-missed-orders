(function () {
  ...
  function getAiSessionId() {
    try {
      ...
      var legacyKey = "taager_ai_session_id";
  ...
  function genericBusinessAnswer(value) {
    return /\bI am (?:Taager|KHOD|Khod Whaat) AI\b|How can I assist|I can help you with|As an AI|أنا (?:Taager|KHOD) AI|أستطيع مساعدتك|يمكنني مساعدتك/i.test(String(value || ""));
  }
  ...
  function validScalePlanAnswer(value) {
    ...
    var metricTypes = [
      ...
      /Earned marketer commission|\u0627\u0644\u0639\u0645\u0648\u0644\u0629 \u0627\u0644\u0645\u062d\u0642\u0642\u0629/i.test(text)
  ...
  function defaultChatMessages() {
    return [{
      sender: "assistant",
      state: "complete",
      text: tr("aii.chatWelcome", null, "Ask Khod Whaat AI about products, cities, ROI, COD, marketer commission, scaling, forecasting, or operations."),
    }];
  }
  ...
  function logAiDiagnostics() {
    ...
    console.log("[KHOD AI] Developer diagnostics", snapshot);
  }
  ...
  function renderChat() {
    ...
    var controlStrip = '<div class="aii-control-strip">' +
      ...
      '<button type="button" class="aii-diagnostics-btn" data-aii-diagnostics="copy">' + esc(uiText("Copy diagnostics", "نسخ التشخيصات")) + '</button>' +
      '<button type="button" class="aii-diagnostics-btn" data-aii-diagnostics="logs">' + esc(uiText("Open logs", "فتح السجلات")) + '</button>' +
    '</div>';
      ...
      '<div class="khod-ai-section taager-ai-section" dir="' + (isRtl ? "rtl" : "ltr") + '">' +
        '<div class="aii-hero-header">' +
          '<div class="aii-hero-title">' +
            ...
            '<div>' +
              '<h1>' + esc(tr("nav.taagerAi", null, "Khod Whaat AI Copilot")) + '</h1>' +
  ...
  window.renderSectionKhodAi = window.renderSectionTaagerAi;
    ...
    root.querySelectorAll("[data-aii-diagnostics]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        if (action === "copy") {
          ...
          console.log("[KHOD AI] Diagnostics copied", snapshot);
          return;
        }
      });
    });
  ...
  async function ask(prompt) {
    ...
    if (typeof window.ensureFeatureScripts === "function") {
      await window.ensureFeatureScripts("dashboardAiEngine").catch(function () {});
    }
