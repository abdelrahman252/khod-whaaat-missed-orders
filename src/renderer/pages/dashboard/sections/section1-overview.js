...
window.renderSection1 = function (mountEl, data, ctx) {
  ...
  var cards = [
    { label: s1Txt('Earned Commission', 'العمولة المحققة'), value: d.earnedCommission.value,   unit: d.earnedCommission.unit,   delta: d.earnedCommission.delta,   color: 'green',  spark: d.sparklines.earned,   iconType: 'green', tooltip: tx('kpi.earned.tooltip', 'العمولة من الطلبات التي تم تسليمها بنجاح.') },
    { label: s1Txt('Incoming Commission', 'عمولة قادمة'),    value: d.incomingCommission.value,  unit: d.incomingCommission.unit,  delta: d.incomingCommission.delta,  color: 'orange', spark: d.sparklines.incoming,  iconType: 'orange', tooltip: tx('kpi.incoming.tooltip', 'عمولة متوقعة من الطلبات التي ما زالت قيد التنفيذ أو لم تتم تسويتها بالكامل.') },
    { label: s1Txt('Lost Commission', 'عمولة ضائعة'),   value: d.lostCommission.value,      unit: d.lostCommission.unit,      delta: d.lostCommission.delta,      color: 'red',    spark: d.sparklines.lost,      iconType: 'red', tooltip: tx('kpi.lost.tooltip', 'عمولة غالبا ضاعت بسبب طلبات فاشلة أو ملغاة أو غير مسلمة.') },
  ...
  /* ── Health bar data ─────────────────────────────────────────────────────── */
  var barSegments = [
    { pct: d.health.earned.pct,   color: '#00e676', label: s1Txt('Earned', 'عمولة محققة'),  sar: d.health.earned.sar   },
    { pct: d.health.incoming.pct, color: '#f59e0b', label: s1Txt('Incoming', 'عمولة قادمة'),  sar: d.health.incoming.sar },
    { pct: d.health.lost.pct,     color: '#ef4444', label: s1Txt('Lost', 'عمولة ضائعة'), sar: d.health.lost.sar     },
  ];
  ...
  var cardsHtml = cards.map(function (c, i) {
    return '<div class="fade-up" style="flex:1;min-width:0;animation-delay:' + (i * 100) + 'ms;">' +
  ...
  var newCardsHtml = newCards.map(function (c, i) {
    return '<div class="fade-up" style="flex:1;min-width:0;animation-delay:' + ((i + 4) * 100) + 'ms;">' +
      window.kpiCard({ label: c.label, value: c.value, unit: c.unit, delta: c.delta, color: c.color, sparklineData: c.spark, iconType: c.iconType, tooltip: c.tooltip }) +
      '</div>';
  }).join('');
  ...
  /* ── Dynamic Performance evaluation based on lost commission percentage ── */
  var lostPct = (d.health && d.health.lost) ? Number(d.health.lost.pct || 0) : 0;
  ...
  if (perfStatus === 'critical') {
    ...
    perfSub = s1Txt('High leakage rate', 'نسبة عمولة ضائعة مرتفعة');
    ...
    '<div class="dash-scroll" style="flex:1;overflow-y:auto;display:flex;flex-direction:column;background:#080b12; " id="s1-root">' +
      ...
      '<div class="s1-body" style="padding:20px 22px;flex:1;">' +
        ...
        '<div class="fade-up s1-title-row" style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:28px;">' +
          ...
          '<div id="s1-title-block">' +
            ...
            '<p id="s1-subtitle" style="font-size:13px;color:rgba(255,255,255,0.4);margin:7px 0 0;display:flex;align-items:center;gap:6px;opacity:0;transition:opacity 0.4s ease 0.12s;">' +
              s1Txt('Comprehensive overview of your store performance on Khod Whaat platform', 'ملخص شامل لأداء متجرك على منصة Khod Whaat') +
              '<span style="color:#3b82f6;font-size:15px;margin-right:6px;filter:drop-shadow(0 0 4px rgba(59,130,246,0.5));">✦</span>' +
            '</p>' +
          '</div>' +

          /* Compare button (visual left = last child) */
          '<button class="s1-compare-btn" style="display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:8px;' +
            'border:1px solid rgba(255,255,255,0.13);background:rgba(255,255,255,0.05);' +
            'color:rgba(255,255,255,0.65);font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;transition:all 0.2s ease;height:30px;min-height:30px;align-self:flex-start;" onmouseover="this.style.background=\'rgba(255,255,255,0.08)\'" onmouseout="this.style.background=\'rgba(255,255,255,0.05)\'">' +
              ...
              '<div class="s1-health-row" style="background:#0d1220;border:1px solid rgba(255,255,255,0.07);border-radius:16px;padding:24px 28px;direction:ltr;display:flex;flex-wrap:wrap;align-items:center;gap:32px;width:100%;">' +
                ...
                '<div style="flex:2;min-width:280px;">' +
                  ...
                  '<div class="health-title" style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">' +
                    ...
                    '</svg>' +
                    '<span style="font-size:18px;font-weight:800;color:#fff;">' + s1Txt('Commission Health Index', 'مؤشر صحة العمولة') + '</span>' +
                  '</div>' +

                  /* Subtitle */
                  '<div class="health-subtitle" style="font-size:13px;color:rgba(255,255,255,0.45);margin-bottom:18px;">' +
                    s1Txt('Distribution of commission between earned, incoming, and lost', 'توزيع العمولة بين المحققة والقادمة والضائعة') +
                  '</div>' +
        ...
        '<p style="text-align:center;font-size:11px;color:rgba(255,255,255,0.35);margin-top:22px;display:flex;align-items:center;justify-content:center;gap:6px; ">' +
          s1Txt('Earned commission is calculated from delivered orders only', 'تحسب العمولة المحققة من الطلبات التي تم تسليمها فقط') +
