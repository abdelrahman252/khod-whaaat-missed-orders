...
window.renderSectionProductForecastHydratedEntry = function (mountEl, data, ctx) {
  ...
  var themeObserverOptions = { attributes: true, attributeFilter: ['data-theme'] };
  if (sectionResources) sectionResources.observe(observer, document.documentElement, themeObserverOptions);
  else observer.observe(document.documentElement, themeObserverOptions);
  ...
  function initTooltips() {
    ...
    if (sectionResources) {
      sectionResources.listen(document, 'mouseover', mountEl._s9TooltipOver);
      sectionResources.listen(document, 'mouseout', mountEl._s9TooltipOut);
    } else {
  ...
  function isForecastInputActive() {
    var active = document.activeElement;
    return !!(active && active.classList && (
      active.classList.contains('s9-spend-input') ||
      active.classList.contains('s9-sim-spend-input') ||
      active.classList.contains('s9-total-orders-input') ||
      active.classList.contains('s9-delivered-orders-input') ||
      active.classList.contains('s9-ndr-input') ||
      active.classList.contains('s9-comm-input')
    ));
  }

  function flushScheduledRender(options) {
    options = options || {};
    if (tableRenderTimer) {
    ...
    if (!options.force && isForecastInputActive()) return;
  ...
  function scheduleRenderAll(delay) {
    ...
    tableRenderTimer = setTimeout(function () {
      ...
      if (isForecastInputActive()) return;
      flushScheduledRender();
    }, delay || 180);
  }

  function markTableRenderPending() {
    tableRenderPending = true;
    if (tableRenderTimer) {
      clearTimeout(tableRenderTimer);
      tableRenderTimer = null;
  ...
  function cancelPendingTableSearchRender() {
    if (!tableSearchTimer) return;
    clearTimeout(tableSearchTimer);
    tableSearchTimer = null;
  }

  function syncTableRowProfit(idx) {
    if (idx == null || idx < 0) return;
    var row = mountEl.querySelector('.s9-row[data-idx="' + idx + '"]');
    var rs = simulations[idx];
    if (!row || !rs) return;
  ...
  function updateSimulatorOnly() {
    ...
    syncTableRowProfit(selectedIdx);
  }
  ...
  function bindEvents(rootEl) {
    ...
    eventRoot.querySelectorAll('.s9-spend-input').forEach(function (inp) {
      inp.addEventListener('input', function (e) {
        ...
        cancelPendingTableSearchRender();
        invalidateSimulationRowsCache();
        updateSimulatorOnly();
        markTableRenderPending();
      });
      inp.addEventListener('blur', function () {
        flushScheduledRender({ force: true });
      });
    });
    ...
    if (simSpendInp) {
      ...
      simSpendInp.addEventListener('blur', function () {
        flushScheduledRender({ force: true });
      });
    }
    ...
    if (totalOrdersInp) {
      ...
      totalOrdersInp.addEventListener('blur', function () {
        flushScheduledRender({ force: true });
      });
    }
    ...
    if (deliveredOrdersInp) {
      ...
      deliveredOrdersInp.addEventListener('blur', function () {
        flushScheduledRender({ force: true });
      });
    }
    ...
    if (ndrInp) {
      ...
      ndrInp.addEventListener('blur', function (e) {
        var v = parseFloat(e.target.value);
        if (isNaN(v) || v < 1) {
          e.target.value = p9RatioPctValue(simulations[selectedIdx].ndr);
          flushScheduledRender({ force: true });
          return;
        }
  ...
  if (window.DashboardRoiState) {
    ...
    if (sectionResources) sectionResources.observe(forecastSettingsObserver, document.body, { childList: true, subtree: true });
    else forecastSettingsObserver.observe(document.body, { childList: true, subtree: true });
  ...
  var forecastLifecycleActive = true;
  mountEl._dashboardSectionDeactivate = function () {
    if (!forecastLifecycleActive) return;
    forecastLifecycleActive = false;
    if (tableSearchTimer) { clearTimeout(tableSearchTimer); tableSearchTimer = null; }
    if (tableRenderTimer) { clearTimeout(tableRenderTimer); tableRenderTimer = null; }
    tableRenderPending = false;
    if (mountEl._s9RoiListener && window.DashboardRoiState) window.DashboardRoiState.unsubscribe(mountEl._s9RoiListener);
    if (mountEl._s9MarketingListener && window.DashboardMarketingState) window.DashboardMarketingState.unsubscribe(mountEl._s9MarketingListener);
  };
  mountEl._dashboardSectionActivate = function () {
    if (forecastLifecycleActive) return;
    forecastLifecycleActive = true;
    if (mountEl._s9RoiListener && window.DashboardRoiState) window.DashboardRoiState.subscribe(mountEl._s9RoiListener);
    if (mountEl._s9MarketingListener && window.DashboardMarketingState) window.DashboardMarketingState.subscribe(mountEl._s9MarketingListener);
  };
};
