...
window.renderSection4HydratedEntry = function (mountEl, data, ctx) {
  var codTrace = function (event, detail, level) {
    if (window.DashboardTrace) window.DashboardTrace.emit("cod", event, detail || {}, level);
  };
  var codRenderStartedAt = window.performance && performance.now ? performance.now() : Date.now();
  var sectionResources = mountEl && mountEl._dashboardResources;
  var scheduleCodTimeout = sectionResources ? sectionResources.timeout.bind(sectionResources) : setTimeout;
  var scheduleCodFrame = sectionResources ? sectionResources.raf.bind(sectionResources) : requestAnimationFrame;
  ...
  const D = cod
    ...
    : DEFAULTS;
  codTrace("model:ready", {
    cityCount: Array.isArray(D.allCities) ? D.allCities.length : 0,
    mapCityCount: Array.isArray(D.cities) ? D.cities.length : 0,
    paymentMethods: Array.isArray(D.payMethods) ? D.payMethods.length : 0,
    amountRepairRows: D.amountRepairReport && Array.isArray(D.amountRepairReport.rows) ? D.amountRepairReport.rows.length : 0,
    salesRepairRows: D.totalSalesRepairReport && Array.isArray(D.totalSalesRepairReport.rows) ? D.totalSalesRepairReport.rows.length : 0
  });
  ...
  function cityBreakdownHTML() {
    ...
    const anchoredMapCities = D.cities.map((city, index) => {
      const country = String(city.country || activeCodCountry() || "sa").toLowerCase();
      const point = window.TaagerGeo && typeof window.TaagerGeo.cityPoint === "function"
        ? window.TaagerGeo.cityPoint(city.name, country, index)
        : null;
      return Object.assign({}, city, point ? {
        x: point.x,
        y: point.y,
        exact: point.exact === true,
        exactPoint: point.exact === true
      } : {});
    });
    const saMap = String(activeCodCountry() || "sa").toLowerCase() === "sa";
    const mapCitiesForDisplay = window.TaagerGeo && typeof window.TaagerGeo.spreadCityPoints === "function"
      ? window.TaagerGeo.spreadCityPoints(anchoredMapCities, {
          minDistance: saMap ? 12 : 22,
          maxShift: saMap ? 10 : 28,
          iterations: saMap ? 8 : 30,
          preserveExact: true
        })
      : anchoredMapCities;
  ...
  function amountRepairReportHTML() {
    ...
    return `<div class="fade-up" style="margin-top:18px;background:var(--dash-surface);border:1px solid rgba(96,165,250,0.16);border-radius:var(--dash-radius-xl);padding:22px;box-shadow:0 18px 60px rgba(0,0,0,0.18);content-visibility:auto;contain-intrinsic-size:auto 900px">
  ...
  (function initAmountRepairReport() {
    ...
    var sortState = {
      filled: { key: null, dir: "asc" },
      unfilled: { key: null, dir: "asc" },
    };
    var pageSize = 7;
    ...
    input.addEventListener("input", function () {
      ...
      searchTimer = scheduleCodTimeout(function () {
        searchTimer = null;
        pageState.filled = 1;
  ...
  codTrace("dom:build:end", {
    nodeCount: mountEl.getElementsByTagName("*").length,
    elapsedMs: Math.round(((window.performance && performance.now ? performance.now() : Date.now()) - codRenderStartedAt) * 10) / 10
  });
  ...
  (function animateDonut() {
    ...
    codTrace("background:visual-frame", { target: "cod-donut", delayRemovedMs: 400 });
    scheduleCodFrame(() => {
  ...
  (function animatePayment() {
    ...
    codTrace("background:visual-frame", { target: "payment-donut", delayRemovedMs: 450 });
    scheduleCodFrame(() => {
