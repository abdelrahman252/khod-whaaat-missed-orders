...
window.renderSection8 = function (mountEl, data, ctx) {
  ...
  function clearSection8RenderTimer() {
    if (mountEl._s8RenderTimer != null) {
      window.clearTimeout(mountEl._s8RenderTimer);
      mountEl._s8RenderTimer = null;
    }
  }
  function scheduleSection8Render(reason) {
    if (!mountEl.isConnected || mountEl.hidden) {
      mountEl._dashboardNeedsRefresh = true;
      return;
    }
    if (mountEl._s8RenderTimer != null) return;
    mountEl._s8RenderReason = reason || 'state';
    mountEl._s8RenderTimer = window.setTimeout(function () {
      mountEl._s8RenderTimer = null;
      if (!mountEl.isConnected || mountEl.hidden) {
        mountEl._dashboardNeedsRefresh = true;
        return;
  ...
  function markSection8Stale(reason) {
    mountEl._s8StaleReason = reason || 'state';
    if (mountEl.hidden) mountEl._dashboardNeedsRefresh = true;
  }
  ...
  var roiLive = {
    ...
    avgCommission:  _roiLiveRaw.avgCommission != null
      ? _roiLiveRaw.avgCommission
      : (d.roi && d.roi.avgCommission != null
        ? d.roi.avgCommission
        : (d.roi && d.roi.averageProfit != null ? d.roi.averageProfit : 0)),
  ...
  var financials = d.financials || {};
  var comparisonLabel = d.meta && d.meta.comparisonLabel
    ? d.meta.comparisonLabel
    : s8Txt('vs previous period', 'مقارنة بالفترة السابقة');

  function khodFinancialValue(primaryKey, overviewKey, overviewAlias, fallback) {
    if (Object.prototype.hasOwnProperty.call(financials, primaryKey)) return num(financials[primaryKey], 0);
    var metric = overview[overviewKey];
    if (metric && overviewAlias && Object.prototype.hasOwnProperty.call(metric, overviewAlias)) {
      return num(metric[overviewAlias], 0);
    }
    if (metric && Object.prototype.hasOwnProperty.call(metric, 'value')) return num(metric.value, 0);
  ...
  var salesValue = khodFinancialValue('salesValue', 'totalSales', 'salesValue', 0);
  var codValue = khodFinancialValue('codValue', 'codValue', 'codValue', cod.totalDue);
  var commissionValue = khodFinancialValue('commissionValue', 'earnedCommission', 'commissionValue', 0);
  var deliveredSales = khodFinancialValue('deliveredSales', 'totalDeliveredSales', 'deliveredSales', 0);
  var failedSales = khodFinancialValue('failedSales', 'failedSales', 'failedSales', 0);
  var canceledSales = khodFinancialValue('canceledSales', 'canceledSales', 'canceledSales', 0);

  // Commission-named aggregator fields remain compatibility aliases for the
  // existing trend data; Section 8 presents them with KHOD commission meaning.
  var earnedCommission = commissionValue;
  var incomingCommission = overview.incomingCommission ? overview.incomingCommission.value : 0;
  var failedStage = (Array.isArray(pipeline.stages) ? pipeline.stages : []).find(function (stage) {
    return stage && (stage.bucket === 'failed' || stage.exactBucket === 'failed' || stage.id === 'failed');
  });
  var failedCommission = failedStage && failedStage.commissionValue != null
    ? num(failedStage.commissionValue, 0)
    : (overview.lostCommission ? overview.lostCommission.value : 0);

  // KPI Cards mapping. KHOD-ready aggregate fields are authoritative; old
  // overview names above are compatibility aliases for historical snapshots.
  var KPI_CARDS = [
    { label: s8Txt('Earned Commission', 'العمولة المكتسبة'), value: earnedCommission, unit: nativeCurrency, delta: overview.earnedCommission ? overview.earnedCommission.delta : 0, color: 'green', spark: (overview.sparklines && overview.sparklines.earned) || [0], iconType: 'green', tooltip: s8Txt('Earned Commission uses KHOD marketer commission for delivered orders.', 'العمولة المكتسبة تستخدم عمولة المسوق في KHOD للطلبات المسلمة.') },
    { label: s8Txt('Incoming Commission', 'العمولة القادمة'), value: incomingCommission, unit: nativeCurrency, delta: overview.incomingCommission ? overview.incomingCommission.delta : 0, color: 'orange', spark: (overview.sparklines && overview.sparklines.incoming) || [0], iconType: 'orange', tooltip: s8Txt('Incoming Commission is the commission still progressing through the order pipeline.', 'العمولة القادمة هي عمولة الطلبات التي ما زالت قيد التنفيذ.') },
    { label: s8Txt('Lost Commission', 'العمولة المفقودة'), value: failedCommission, unit: nativeCurrency, delta: overview.lostCommission ? overview.lostCommission.delta : 0, color: 'red', spark: (overview.sparklines && overview.sparklines.lost) || [0], iconType: 'red', tooltip: s8Txt('Lost Commission is the commission attached to failed orders.', 'العمولة المفقودة هي عمولة الطلبات الفاشلة.') },
    { label: s8Txt('Total Orders', 'إجمالي الطلبات'), value: netTotalOrders, unit: s8Txt('orders', 'طلبات'), delta: overview.totalOrders ? overview.totalOrders.delta : 0, color: 'blue', spark: (overview.sparklines && overview.sparklines.orders) || [0], iconType: 'blue', tooltip: tx('kpi.orders.tooltip', 'Total KHOD lifecycle orders in the selected period.') },
  ...
  var deliveredSalesInTarget = convert(deliveredSales, nativeCurrency, targetCurrency);
  ...
  var NEW_KPI_CARDS = [
    { label: s8Txt('Total Sales', 'إجمالي المبيعات'), value: salesValue, unit: nativeCurrency, delta: overview.totalSales ? overview.totalSales.delta : 0, color: 'green', spark: [], iconType: 'green', tooltip: s8Txt('Total Sales uses KHOD order value fields for all lifecycle orders.', 'إجمالي المبيعات يستخدم حقول قيمة الطلب في KHOD لكل طلبات دورة الحياة.') },
    { label: s8Txt('AOV', 'متوسط قيمة الطلب'), value: overview.overallAov ? overview.overallAov.value : 0, unit: nativeCurrency, delta: overview.overallAov ? overview.overallAov.delta : 0, color: 'blue', spark: [], iconType: 'blue', tooltip: tx('kpi.overallAov.tooltip', 'Average order value = total sales / total orders.') },
    { label: s8Txt('Total Delivered Sales', 'إجمالي مبيعات الطلبات المسلمة'), value: deliveredSales, unit: nativeCurrency, delta: overview.totalDeliveredSales ? overview.totalDeliveredSales.delta : 0, color: 'green', spark: [], iconType: 'green', tooltip: s8Txt('Sales value for delivered orders only.', 'قيمة المبيعات للطلبات المسلمة فقط.') },
    { label: s8Txt('Delivered AOV', 'متوسط قيمة الطلب المسلم'), value: overview.deliveredAov ? overview.deliveredAov.value : 0, unit: nativeCurrency, delta: overview.deliveredAov ? overview.deliveredAov.delta : 0, color: 'blue', spark: [], iconType: 'blue', tooltip: tx('kpi.deliveredAov.tooltip', 'Delivered AOV = net delivered sales / delivered orders.') },
    { label: s8Txt('Net ROAS', 'العائد الصافي على الإعلان'), value: netRoasUnavailable ? 0 : netRoas.toFixed(2), displayValue: netRoasUnavailable ? '—' : netRoas.toFixed(2), unit: 'x', delta: netRoasDelta, hideDelta: netRoasUnavailable, color: 'purple', spark: [], iconType: 'purple', tooltip: tx('kpi.netRoas.tooltip', s8Txt('Net ROAS = delivered sales divided by ad spend. It uses only successfully delivered order revenue.', 'العائد الصافي على الإعلان = المبيعات المسلمة مقسومة على الإنفاق الإعلاني.')) },
    { label: s8Txt('Delivery Rate (DR)', 'معدل التسليم (DR)'), value: overview.drRate ? overview.drRate.value : 0, unit: '%', delta: overview.drRate ? overview.drRate.delta : 0, color: 'blue', spark: [], iconType: 'blue', tooltip: tx('kpi.drRate.tooltip', 'DR = delivered orders / confirmed orders.') }
  ];

  var KHOD_STAGE_META = [
    { id: 'pending', label: s8Txt('Pending', 'بانتظار التأكيد'), color: '#f59e0b' },
    { id: 'confirmed', label: s8Txt('Confirmed', 'مؤكد'), color: '#22c55e' },
    { id: 'processing', label: s8Txt('Under processing', 'قيد المعالجة'), color: '#3b82f6' },
    { id: 'waiting', label: s8Txt('Waiting', 'قيد الانتظار'), color: '#06b6d4' },
    { id: 'shipping', label: s8Txt('In shipping', 'قيد الشحن'), color: '#f97316' },
    { id: 'delivered', label: s8Txt('Delivered', 'تم التسليم'), color: '#10b981', highlight: 'green' },
    { id: 'failed', label: s8Txt('Failed', 'فشل'), color: '#ef4444', highlight: 'red' },
    { id: 'canceled', label: s8Txt('Canceled', 'ملغي'), color: '#94a3b8' }
  ];

  function khodStageBucket(stage) {
    if (!stage) return null;
    var allowed = KHOD_STAGE_META.map(function (meta) { return meta.id; });
    var direct = [stage.bucket, stage.exactBucket, stage.id];
    for (var i = 0; i < direct.length; i++) {
      var candidate = String(direct[i] == null ? '' : direct[i]).toLowerCase();
      if (allowed.indexOf(candidate) !== -1) return candidate;
    }
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.normalize === 'function') {
      var normalized = window.KhodDashboardLogic.normalize(stage.label || '');
      if (normalized && allowed.indexOf(normalized.bucket) !== -1) return normalized.bucket;
    }
    return null;
  }

  function khodStageSales(stage) {
    if (!stage) return 0;
    if (stage.salesValue != null) return num(stage.salesValue, 0);
    if (stage.salesSar != null) return num(stage.salesSar, 0);
    return 0;
  }

  function buildKhodStages(inputStages) {
    var grouped = {};
    KHOD_STAGE_META.forEach(function (meta) { grouped[meta.id] = { count: 0, salesValue: 0 }; });
    (Array.isArray(inputStages) ? inputStages : []).forEach(function (stage) {
      var bucket = khodStageBucket(stage);
      if (!bucket || !grouped[bucket]) return;
      grouped[bucket].count += num(stage.count, 0);
      grouped[bucket].salesValue += khodStageSales(stage);
    });
    var lifecycleTotal = KHOD_STAGE_META.reduce(function (sum, meta) {
      return sum + grouped[meta.id].count;
    }, 0);
    return KHOD_STAGE_META.map(function (meta) {
      var values = grouped[meta.id];
      var share = lifecycleTotal > 0 ? parseFloat(((values.count / lifecycleTotal) * 100).toFixed(1)) : 0;
      return {
        id: meta.id,
        bucket: meta.id,
        label: meta.label,
        ...
        sar: values.salesValue ? values.salesValue.toLocaleString('en-US', { maximumFractionDigits: 2 }) : undefined,
        sarLabel: values.salesValue ? s8Txt('Sales value', 'قيمة المبيعات') : ''
      };
    });
  }

  var STAGES = buildKhodStages(pipeline.stages);
  ...
  var shippingCount = stageCountBy(function (stage) { return stage.id === 'shipping'; });
  var deliveredCount = stageCountBy(function (stage) { return stage.id === 'delivered'; });
  var failedCount = stageCountBy(function (stage) { return stage.id === 'failed'; });
  var canceledCount = stageCountBy(function (stage) { return stage.id === 'canceled'; });
  ...
  PIPELINE_SUMMARY[1].tooltip = s8Txt(
    'NDR = Delivered orders divided by KHOD lifecycle orders excluding Canceled.',
    'NDR = الطلبات المسلمة مقسومة على طلبات دورة حياة KHOD مع استبعاد الملغاة.'
  );
  PIPELINE_SUMMARY[3].tooltip = s8Txt(
    'DR = Delivered orders divided by the confirmed KHOD lifecycle base.',
    'DR = الطلبات المسلمة مقسومة على قاعدة الطلبات المؤكدة في دورة حياة KHOD.'
  );
  ...
  COD_METRICS[2].tooltip = s8Txt(
    'Collected = sum of KHOD Amount Due for Delivered COD orders in the selected period.',
    'المحصل = مجموع المبلغ المستحق في KHOD لطلبات COD المسلمة في الفترة المحددة.'
  );
  COD_METRICS[3].tooltip = s8Txt(
    'Gap = incoming COD still in active KHOD lifecycle stages; Failed and Canceled are excluded.',
    'الفجوة = تحصيل COD القادم داخل مراحل KHOD النشطة؛ وتستبعد الطلبات الفاشلة والملغاة.'
  );
  ...
  function productCommissionValue(p) {
    var values = [
      p && p.expectedTotalProfitBeforeAdSpend,
      p && p.commissionValue,
      p && p.marketerCommission,
      p && p.commission,
      p && p.actualEarnedCommission,
      p && p.actualEarnedProfitAfterTax
    ];
    for (var i = 0; i < values.length; i++) {
      if (values[i] != null && isFinite(Number(values[i]))) return Number(values[i]);
    }
    return 0;
  }
  function productDeliveredValue(p) {
    if (p && p.expectedDeliveriesExact != null) return Number(p.expectedDeliveriesExact) || 0;
    if (p && p.deliveredCount != null) return Number(p.deliveredCount) || 0;
    return Number(p && p.units) || 0;
  }
  function productHasTopSignal(p) {
    return productDeliveredValue(p) > 0 || productCommissionValue(p) > 0;
  }
  function productDeliveredLabel(p) {
    var exact = productDeliveredValue(p);
    var displayed = p && p.expectedDeliveriesDisplay != null ? Number(p.expectedDeliveriesDisplay) : Math.round(exact);
    if (displayed <= 0 && exact > 0) return '<1';
    return String(Math.max(0, displayed || 0));
  }
  var RANK_META = {
  ...
  // Commission trend period mapping. The old "lost" key is a compatibility
  // alias for KHOD Failed commission and never includes Canceled.
  ...
  if (s8CommissionType === 'incoming') {
    ...
    displayColor = '#f59e0b';
    displayTitle = s8Txt('Pipeline Commission', 'عمولة الطلبات قيد التنفيذ');
  } else if (s8CommissionType === 'lost') {
    activePeriodsObj = trend.lostPeriods || {};
    displayCommissionVal = failedCommission;
    displayDeltaVal = overview.lostCommission ? overview.lostCommission.delta : 0;
    displayColor = '#ef4444';
    displayTitle = s8Txt('Failed Commission', 'عمولة الطلبات الفاشلة');
  } else {
    ...
    displayColor = '#22c55e';
    displayTitle = s8Txt('Delivered Commission', 'عمولة الطلبات المسلمة');
  }
  ...
  var CHART_STATS = [
    { label: s8Txt('Average Daily Commission', 'متوسط العمولة اليومية'), value: CHART_AVG.toLocaleString("en-US") + ' ' + nativeCurrency, color: 'rgba(255,255,255,0.6)', sub: null             },
  ...
  var SUMMARY_ITEMS = [
    ...
    { id: 7, label: s8Txt('Delivered commission growth', 'نمو عمولة الطلبات المسلمة'), iconColor: '#84cc16', value: deltaSign + Math.abs(earnedDelta).toFixed(1) + '%', sub: s8Txt('vs previous period', 'مقارنة بالفترة السابقة'), valueColor: deltaColor, iconType: 'trend' },
  ...
  SUMMARY_ITEMS[1].tooltip = s8Txt(
    'NDR = Delivered orders / KHOD lifecycle orders excluding Canceled.',
    'NDR = الطلبات المسلمة / طلبات دورة حياة KHOD مع استبعاد الملغاة.'
  );
  ...
  SUMMARY_ITEMS[5].tooltip = s8Txt(
    'DR = Delivered orders / confirmed KHOD lifecycle base.',
    'DR = الطلبات المسلمة / قاعدة الطلبات المؤكدة في دورة حياة KHOD.'
  );
  ...
  function sectionBadge(num, title, color) {
    ...
    return '<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;justify-content:flex-end;">' +
      ...
      '<div style="width:26px;height:26px;border-radius:50%;background:' + color + '22;border:1.5px solid ' + color + '80;' +
        'display:flex;align-items:center;justify-content:center;font-size:var(--type-caption);font-weight:var(--weight-bold);color:' + color + ';flex-shrink:0;">' +
        num +
      '</div>' +
    '</div>';
  }
  ...
  function pipelineStageCard(s, isFirst, isLast) {
    ...
    var stageIcons = {
      pending: '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="'+s.color+'" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',
      ...
      waiting: '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="'+s.color+'" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',
      ...
      canceled: '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="'+s.color+'" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><path d="M8 12h8"/></svg>'
    };
    var iconSvg = stageIcons[s.id] || stageIcons.pending;
    ...
    if (s.sar) {
      sarHtml = '<div style="display:flex;flex-direction:column;align-items:center;gap:1px;line-height:1.05;">' +
        ...
        (sarLabel ? '<span style="font-family:var(--font-ui);font-size:var(--type-micro);font-weight:var(--weight-semibold);color:var(--dash-text-faint);text-align:center;max-width:72px;">' + sarLabel + '</span>' : '') +
      '</div>';
    }
  ...
  var kpiRowHtml = KPI_CARDS.map(function (c, i) {
    return '<div style="flex:1 1 calc((100% - 50px) / 6);min-width:170px;">' +
      window.s8KpiCard({
  ...
  var financialKpiCards = [
    { label: s8Txt('Average Commission', 'متوسط العمولة'), value: avgCommissionInTargetCurrency, decimals: 2, color: 'green', iconType: 'green', tooltip: s8Txt('Average Commission = average KHOD marketer commission per delivered order.', 'متوسط العمولة = متوسط عمولة المسوق في KHOD لكل طلب مسلم.') },
    ...
    { label: s8Txt('Total Revenue', 'إجمالي الإيرادات'), value: accountRevenue, decimals: 0, color: 'green', iconType: 'green', tooltip: s8Txt('Total Revenue = delivered orders multiplied by average KHOD commission per delivered order.', 'عائد العمولة = الطلبات المسلمة مضروبة في متوسط عمولة KHOD لكل طلب مسلم.') },
    { label: s8Txt('Net Profit', 'صافي الربح'), value: accountNetProfit, decimals: 0, color: accountNetProfit >= 0 ? 'green' : 'red', iconType: accountNetProfit >= 0 ? 'green' : 'red', tooltip: s8Txt('Net Profit = total revenue minus total spend.', 'صافي عائد العمولة = عائد العمولة ناقص إجمالي الإنفاق.') }
  ];
  ...
  var pipelineSummaryRow1Html = PIPELINE_SUMMARY.slice(0, 3).map(function (s) {
    ...
    return '<div ' + tipAttr(s.tooltip) + ' style="display:flex;flex-direction:column;align-items:center;gap:3px;min-width:0;">' +
      '<span style="font-family:var(--font-ui);font-size:var(--type-micro);color:var(--dash-text-muted);font-weight:var(--weight-semibold);white-space:nowrap;text-align:center;">' + s.label + '</span>' +
    ...
    return '<div ' + tipAttr(m.tooltip) + ' style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:' + m.color + '0a;' +
      ...
      '<div style="flex:1;min-width:0;direction:' + (isAr ? 'rtl' : 'ltr') + ';display:flex;flex-direction:column;justify-content:center;">' +
        ...
        '<div style="display:flex;align-items:baseline;gap:3px;justify-content:flex-start;direction:ltr;flex-direction:row-reverse;">' +
          '<span style="font-size:var(--type-control);font-weight:var(--weight-bold);color:#fff;line-height:1;white-space:nowrap;">' + m.value + '</span>' +
  ...
  var citiesHtml = CITIES.map(function (c, i) {
    ...
    return '<div class="city-bar-row" style="display:flex;flex-direction:column;gap:6px;direction:ltr;" data-pct="' + c.pct + '" data-delay="' + (i * 40) + '">' +
      ...
      '<div style="width:100%;height:4px;background:rgba(59,130,246,0.1);border-radius:2px;overflow:hidden;">' +
        '<div class="city-bar-fill" style="width:' + c.pct + '%;height:100%;background:#3b82f6;border-radius:2px;box-shadow:0 0 8px #3b82f6;' +
          'transition:none;"></div>' +
      '</div>' +
    '</div>';
  }).join('');

  var TOP_PRODUCTS = PRODUCTS.filter(productHasTopSignal).slice(0, 15);
  ...
  for (var pageIdx = 0; pageIdx < numPages; pageIdx++) {
    ...
    var pageHtml = pageItems.map(function (p, i) {
      ...
      var earnedProfitAfterTax = productCommissionValue(p);
      return '<div style="display:flex;align-items:center;gap:10px;padding:10px 4px;border-bottom:' + border + ';direction:' + (isAr ? 'rtl' : 'ltr') + ';">' +
        '<div style="width:22px;height:22px;border-radius:50%;background:' + meta.bg + ';display:flex;align-items:center;justify-content:center;' +
          'font-size:var(--type-micro);font-weight:var(--weight-bold);color:#fff;flex-shrink:0;box-shadow:' + (meta.glow !== 'none' ? '0 0 8px ' + meta.glow : 'none') + ';">' +
          p.rank +
        '</div>' +
        ...
        '<div data-i18n-preserve style="flex:1;min-width:0;font-size:10px;font-weight:700;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:right;">' +
          p.name +
        '</div>' +
        '<div style="font-size:var(--type-body);font-weight:var(--weight-bold);color:#fff;width:26px;text-align:center;flex-shrink:0;">' + productDeliveredLabel(p) + '</div>' +
        '<div style="text-align:left;flex-shrink:0;min-width:52px;">' +
          '<div style="font-size:var(--type-label);font-weight:var(--weight-bold);color:#fff;line-height:1;">' + earnedProfitAfterTax.toLocaleString("en-US") + '</div>' +
  ...
  var chartStatsHtml = CHART_STATS.map(function (s, i) {
    ...
    return '<div style="flex:1;text-align:center;border-left:' + bl + ';padding:0 6px;">' +
      ...
      '<div style="font-size:var(--type-caption);font-weight:var(--weight-bold);color:' + s.color + ';line-height:1;">' + s.value + '</div>' +
  ...
  var summaryItemsHtml = SUMMARY_ITEMS.map(function (item, i) {
    ...
    if (item.stars) {
      leftBlock = '<div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex-shrink:0;padding-left:4px;">' +
        '<span style="font-size:var(--type-metric-sm);font-weight:var(--weight-bold);color:#fff;line-height:1;">' + item.value + '</span>' +
    ...
    return '<div ' + tipAttr(item.tooltip) + ' style="flex:1;min-width:0;display:flex;align-items:center;gap:8px;direction:ltr;' +
  ...
  var roiAdvice = roiPct >= 50
    ...
    : roiPct >= 0
      ? s8Txt('Returns are positive but tight. Use Section 7 to test NDR, simulator marketer commission, and budget scenarios.', 'العوائد إيجابية لكنها محدودة. استخدم القسم 7 لاختبار NDR وعمولة المسوق في المحاكاة وسيناريوهات الميزانية.')
  ...
  function roiMetricCard(label, value, sub, color, iconSvg) {
    return '<div style="display:flex;flex-direction:column;align-items:flex-start;gap:8px;background:' + color + '0d;border:1px solid ' + color + '33;padding:12px;border-radius:14px;min-width:0;width:100%;box-sizing:border-box;">' +
      ...
      '<div style="min-width:0;width:100%;">' +
        ...
        '<div style="font-size:var(--type-body);font-weight:var(--weight-bold);color:#fff;line-height:1.1;direction:ltr;text-align:left;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + value + '</div>' +
  ...
  var roiWidgetHtml = '<div id="s8-roi-preview" style="background:#0a0f18;border:1px solid rgba(255,255,255,0.06);border-radius:16px;padding:20px;height:100%;display:flex;flex-direction:column;box-shadow:inset 0 0 40px rgba(0,0,0,0.5);">' +
    '<div style="display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:18px;">' +
      ...
      '<div style="width:20px;height:20px;border-radius:50%;border:1px solid #a855f7;background:rgba(168,85,247,0.1);display:flex;align-items:center;justify-content:center;color:#a855f7;font-size:10px;font-weight:800;box-shadow:0 0 10px rgba(168,85,247,0.3);">6</div>' +
        ...
        '</div>' +
      '</div>' +
      '<span style="font-size:var(--type-micro);font-weight:var(--weight-bold);color:' + roiProfitColor + ';background:' + roiProfitColor + '18;border:1px solid ' + roiProfitColor + '38;border-radius:var(--radius-pill);padding:6px 10px;white-space:nowrap;">' + roiStateLabel + '</span>' +
    '</div>' +
    '<div style="display:grid;grid-template-columns:minmax(0,1.05fr) minmax(260px,.95fr);gap:18px;align-items:stretch;flex:1;" class="s8-roi-preview-grid">' +
      '<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;align-content:start;">' +
        ...
        roiMetricCard(s8Txt('Revenue', 'الإيرادات'), roiMoney(roiRevenue), s8Txt('delivered × simulator marketer commission per delivered order', 'المسلم × عمولة المسوق في المحاكاة لكل طلب مسلم'), '#06b6d4', '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3v18h18"/><path d="m7 15 4-4 3 3 5-7"/></svg>') +
  ...
  var gmvSnapshot = window.DashboardGmvTargetState && typeof window.DashboardGmvTargetState.snapshot === 'function'
    ...
    : null;
  var gmvStatus = (function () {
    if (!gmvSnapshot || !gmvSnapshot.hasTarget) return { label: s8Txt('No target set', 'لا يوجد هدف محدد'), tone: 'empty' };
    if (gmvSnapshot.status === 'overachieved') return { label: s8Txt('Overachieved', 'تم تجاوز الهدف'), tone: 'good' };
    if (gmvSnapshot.status === 'achieved') return { label: s8Txt('Achieved', 'تم تحقيق الهدف'), tone: 'good' };
    if (gmvSnapshot.status === 'on_track') return { label: s8Txt('On track', 'على المسار الصحيح'), tone: 'info' };
    if (gmvSnapshot.status === 'watch') return { label: s8Txt('Needs a small push', 'يحتاج دفعة بسيطة'), tone: 'warn' };
    if (gmvSnapshot.status === 'ended') return { label: s8Txt('Period ended', 'انتهت الفترة'), tone: 'muted' };
    return { label: s8Txt('Behind pace', 'أقل من الوتيرة المطلوبة'), tone: 'bad' };
  })();
  function gmvMoney(value) {
    if (window.formatDashboardMoney) return window.formatDashboardMoney(value, gmvSnapshot ? gmvSnapshot.currency : nativeCurrency, 0, { compact: true });
    return Number(value || 0).toLocaleString('en-US') + ' ' + nativeCurrency;
  }
  function gmvRate(value) {
    return Number(value || 0).toFixed(1).replace(/\.0$/, '');
  }
  var gmvTargetIcon = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/><path d="M12 2v3M22 12h-3M12 22v-3M2 12h3"/></svg>';
  var gmvPreviewHtml = '';
  if (gmvSnapshot && gmvSnapshot.hasTarget) {
    var gmvProgress = Math.max(0, Math.min(100, Number(gmvSnapshot.progressPct || 0)));
    gmvPreviewHtml = '<section id="s8-gmv-preview" class="s8-gmv-preview" aria-label="' + s8Txt('GMV Target Progress', 'تقدم هدف GMV') + '">' +
      '<div class="s8-gmv-head"><div class="s8-gmv-title"><span class="s8-gmv-icon">' + gmvTargetIcon + '</span><div><strong>' + s8Txt('GMV Target Progress', 'تقدم هدف GMV') + '</strong><span>' + s8Txt('Net Total Delivered Sales goal', 'هدف صافي مبيعات الطلبات المسلمة') + '</span></div></div>' +
      '<span class="s8-gmv-status is-' + gmvStatus.tone + '">' + gmvStatus.label + '</span></div>' +
      '<div class="s8-gmv-content"><div class="s8-gmv-main"><div class="s8-gmv-value"><strong>' + gmvMoney(gmvSnapshot.currentGmv) + ' / ' + gmvMoney(gmvSnapshot.targetGmv) + '</strong><span>' + gmvRate(gmvSnapshot.progressPct) + '%</span></div>' +
      '<div class="s8-gmv-track"><span style="width:' + gmvProgress.toFixed(2) + '%"></span></div>' +
      '<div class="s8-gmv-meta"><span>' + s8Txt('Missing', 'المتبقي') + ': <b>' + gmvMoney(gmvSnapshot.remainingGmv) + '</b></span><span>' + s8Txt('Need', 'المطلوب') + ': <b>' + gmvRate(gmvSnapshot.dailyOrdersNeeded) + s8Txt('/day', '/يوم') + '</b></span><span>' + s8Txt('Current pace', 'الوتيرة الحالية') + ': <b>' + gmvRate(gmvSnapshot.runRate) + s8Txt('/day', '/يوم') + '</b></span></div></div>' +
      '<button id="s8-btn-gmv-target" type="button">' + s8Txt('Open Planner', 'فتح المخطط') + '</button></div></section>';
  } else {
    gmvPreviewHtml = '<section id="s8-gmv-preview" class="s8-gmv-preview is-empty"><div class="s8-gmv-title"><span class="s8-gmv-icon">' + gmvTargetIcon + '</span><div><strong>' + s8Txt('GMV Target Progress', 'تقدم هدف GMV') + '</strong><span>' + s8Txt('No GMV target set yet', 'لم يتم تحديد هدف GMV بعد') + '</span></div></div><button id="s8-btn-gmv-target" type="button">' + s8Txt('Set Goal', 'تحديد الهدف') + '</button></section>';
  }
    ...
    '<div style="padding:22px 28px 32px;flex:1;">' +
      ...
      '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:22px;">' +
        '<div>' +
          ...
          '<p style="font-size:var(--type-label);color:var(--dash-text-faint);margin:5px 0 0;">' + s8Txt('Comprehensive overview of current bot performance', 'نظرة شمولية على أداء عمل البوت الحالي') + '</p>' +
        '</div>' +
      ...
      '<div class="s8-row2" style="margin-bottom:22px;display:flex;flex-wrap:wrap;gap:18px;width:100%;direction:ltr;">' +
      ...
      '<div class="s8-row3" style="display:grid;grid-template-columns:1.2fr 0.8fr 1.6fr;gap:18px;margin-bottom:22px;align-items:stretch;width:100%;">' +

        /* Section 5: KHOD commission chart (flex: 1.2; min-width: 280px) */
        '<div style="flex:1.2;min-width:280px;display:flex;flex-direction:column;">' +
          sectionBadge('5', s8Txt('Daily Commission Trend', 'اتجاه العمولة اليومي'), '#22c55e') +
          '<div style="flex:1;background:#0d1220;border:1px solid rgba(255,255,255,0.07);border-radius:16px;padding:18px 20px;display:flex;flex-direction:column;gap:12px;box-sizing:border-box;">' +
            '<div style="display:flex;justify-content:space-between;align-items:center;">' +
              '<div style="display:flex;align-items:center;gap:8px;">' +
                '<span style="font-size:var(--type-control);font-weight:var(--weight-semibold);color:#fff;direction:' + (isAr ? 'rtl' : 'ltr') + ';font-family:var(--font-ui);">' + s8Txt('Daily Commission Trend', 'اتجاه العمولة اليومي') + '</span>' +
            ...
            '<div style="direction:' + (isAr ? 'rtl' : 'ltr') + ';">' +
              ...
              '<div style="display:flex;align-items:baseline;gap:6px;">' +
                '<span id="s8-commission-val" style="font-size:var(--type-metric-sm);font-weight:var(--weight-bold);color:#fff;line-height:1;">' + displayCommissionVal.toLocaleString("en-US") + '</span>' +
        ...
        '<div style="flex:0.8;min-width:230px;display:flex;flex-direction:column;">' +
          ...
          '<div style="flex:1;background:#0d1220;border:1px solid rgba(255,255,255,0.07);border-radius:16px;padding:18px 20px;display:flex;flex-direction:column;gap:10px;box-sizing:border-box;">' +
            ...
            '<div style="display:flex;justify-content:space-between;align-items:center;padding-bottom:8px;border-bottom:1px solid rgba(255,255,255,0.06);direction:' + (isAr ? 'rtl' : 'ltr') + ';">' +
              '<span style="font-size:8px;color:rgba(255,255,255,0.28);font-weight:600;">' + s8Txt('Delivered Commission', 'عمولة الطلبات المسلمة') + '</span>' +
  ...
  // Motion is disabled for dashboard panes; KPI values are rendered final.
  ...
  (function renderScalingLeaderboard() {
    ...
    var rows = topScaling.map(function (c, i) {
      ...
      return '<div style="display:grid;grid-template-columns:28px 1fr 70px 60px 60px 60px;' +
        ...
        '<div style="font-size:var(--type-label);font-weight:var(--weight-bold);color:' + rankColor + ';text-align:center">#' + rank + '</div>' +
        '<div>' +
          ...
          '<div style="margin-top:4px;height:4px;background:rgba(255,255,255,0.06);border-radius:4px;overflow:hidden">' +
            '<div style="height:100%;width:' + barW + '%;border-radius:4px;background:linear-gradient(90deg,#7c3aed,#14b8a6);transition:none;' +
    ...
    mount.innerHTML =
      '<div style="margin:0 28px 28px;background:#0b1120;border:1px solid rgba(255,255,255,0.07);' +
        ...
        '<div style="padding:0 12px 16px;display:flex;align-items:center;gap:10px;">' +
          ...
          '</div>' +
          '<div>' +
            '<div style="font-size:var(--type-body);font-weight:var(--weight-bold);color:#fff">' + s8Txt('Best Expansion List', 'قائمة أفضل توسع') + '</div>' +
  ...
  /* KHOD commission chart (Chart.js) dynamically updating */
  mountEl._updateCommissionChart = function (type, isInitial) {
    ...
    if (type === 'incoming') {
      ...
      dispColor = '#f59e0b';
      dispTitle = s8Txt('Pipeline Commission', 'عمولة الطلبات قيد التنفيذ');
    } else if (type === 'lost') {
      activePeriodsObj = trend.lostPeriods || {};
      dispComm = failedCommission;
      dispDelta = overview.lostCommission ? overview.lostCommission.delta : 0;
      dispColor = '#ef4444';
      dispTitle = s8Txt('Failed Commission', 'عمولة الطلبات الفاشلة');
    } else {
      activePeriodsObj = trend.periods || {};
      dispComm = earnedCommission;
      dispDelta = overview.earnedCommission ? overview.earnedCommission.delta : 0;
      dispColor = '#22c55e';
      dispTitle = s8Txt('Delivered Commission', 'عمولة الطلبات المسلمة');
    }
    ...
    if (!isInitial) {
      ...
      var cStats = [
        { label: s8Txt('Average Daily Commission', 'متوسط العمولة اليومية'), value: cAvg.toLocaleString("en-US") + ' ' + nativeCurrency, color: 'rgba(255,255,255,0.6)', sub: null },
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              ...
              bodyColor:       dispColor,
              bodyFont:        { size: 10, family: getComputedStyle(mountEl).fontFamily },
              callbacks: {
                label: function (item) {
          ...
          scales: {
            x: {
              grid: { display: false },
              ticks: { color: theme.label, font: { size: 7, family: getComputedStyle(mountEl).fontFamily } },
            ...
            y: {
              ...
              ticks: {
                color: theme.label,
                font: { size: 7, family: getComputedStyle(mountEl).fontFamily },
                stepSize: stepSize,
  ...
  if (typeof dropdown === 'function') {
    ...
    if (typeWrap) {
      dropdown(typeWrap, [
        { value: 'earned', label: s8Txt('Delivered Commission', 'عمولة الطلبات المسلمة') },
        { value: 'incoming', label: s8Txt('Pipeline Commission', 'عمولة الطلبات قيد التنفيذ') },
        { value: 'lost', label: s8Txt('Failed Commission', 'عمولة الطلبات الفاشلة') }
      ], s8CommissionType, function (value) {
        mountEl._s8CommissionType = value;
        mountEl._updateCommissionChart(value, false);
      }, { ariaLabel: 'KHOD commission type' });
    }
  }
  ...
  if (window.DashboardGmvTargetState) {
    ...
    mountEl._s8GmvTargetKey = typeof window.DashboardGmvTargetState.get === 'function'
      ? JSON.stringify(window.DashboardGmvTargetState.get(_roiAccountId) || null)
      : '';
    mountEl._s8GmvTargetListener = function (next) {
      if (!next || String(next.accountId) !== String(_roiAccountId)) return;
      var nextKey = JSON.stringify(next || null);
      if (nextKey === mountEl._s8GmvTargetKey) return;
      mountEl._s8GmvTargetKey = nextKey;
      if (mountEl.hidden) {
        mountEl._dashboardNeedsRefresh = true;
        return;
      }
      markSection8Stale('gmv');
    };
  ...
  if (window.DashboardRoiState) {
    ...
    mountEl._s8RoiKey = typeof window.DashboardRoiState.get === 'function'
      ? JSON.stringify(window.DashboardRoiState.get(_roiAccountId) || null)
      : '';
    mountEl._s8RoiListener = function (next) {
      if (!next || String(next.accountId) !== String(_roiAccountId)) return;
      var nextKey = JSON.stringify(next || null);
      if (nextKey === mountEl._s8RoiKey) return;
      mountEl._s8RoiKey = nextKey;
      if (mountEl.hidden) {
        mountEl._dashboardNeedsRefresh = true;
        return;
      }
      markSection8Stale('roi');
    };
  ...
  function subscribeSection8() {
    ...
    if (window.DashboardGmvTargetState && mountEl._s8GmvTargetListener && typeof window.DashboardGmvTargetState.get === 'function') {
      mountEl._s8GmvTargetListener(window.DashboardGmvTargetState.get(_roiAccountId));
    }
    if (window.DashboardRoiState && mountEl._s8RoiListener && typeof window.DashboardRoiState.get === 'function') {
      mountEl._s8RoiListener(window.DashboardRoiState.get(_roiAccountId));
    }
    if (window.DashboardMarketingState && mountEl._s8MarketingListener && typeof window.DashboardMarketingState.get === 'function') {
      mountEl._s8MarketingListener(window.DashboardMarketingState.get(_roiAccountId));
    }
  }

  function cleanupSection8Subscriptions() {
    clearSection8RenderTimer();
    if (mountEl._s8ThemeObserver) {
      mountEl._s8ThemeObserver.disconnect();
      mountEl._s8ThemeObserver = null;
    }
  ...
  var section8LifecycleActive = true;
  mountEl._dashboardSectionDeactivate = function () {
    if (!section8LifecycleActive) return;
    section8LifecycleActive = false;
    clearSection8RenderTimer();
    unsubscribeSection8();
  };
  mountEl._dashboardSectionActivate = function () {
    if (section8LifecycleActive) return;
    section8LifecycleActive = true;
    subscribeSection8();
    if (mountEl._dashboardNeedsRefresh) {
      mountEl._dashboardNeedsRefresh = false;
      scheduleSection8Render('activate');
    }
  };
  mountEl._dashboardSectionCleanup = function () {
    cleanupSection8Subscriptions();
