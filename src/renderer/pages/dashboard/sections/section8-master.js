...
window.renderSection8 = function (mountEl, data, ctx) {
  ...
  var accountStages = Array.isArray(pipeline.stages) ? pipeline.stages : [];
  function s8StageById(id) {
    for (var i = 0; i < accountStages.length; i++) {
      if (accountStages[i] && accountStages[i].id === id) return accountStages[i];
    }
    return null;
  }
  var accountAwaitingStage = s8StageById('awaiting');
  var accountTotalOrders = pipeline.metrics && pipeline.metrics.totalOrders != null
    ? num(pipeline.metrics.totalOrders, 0)
    : num(totalOrders, 0);
  var accountPendingOrders = accountAwaitingStage ? num(accountAwaitingStage.count, 0) : 0;
  var accountConfirmationPct = overview.confirmationRate && overview.confirmationRate.value != null
    ? num(overview.confirmationRate.value, 0)
    : accountAwaitingStage && accountAwaitingStage.conv != null
    ? num(accountAwaitingStage.conv, 0)
    : (accountTotalOrders > 0 ? parseFloat((((accountTotalOrders - accountPendingOrders) / accountTotalOrders) * 100).toFixed(1)) : 0);
  var accountNdrPct = cod.ndrPct != null
    ? num(cod.ndrPct, 0)
    : (d.roi && d.roi.ndrPct != null ? num(d.roi.ndrPct, 0) : 0);
  var accountDrPct = cod.drPct != null
    ? num(cod.drPct, 0)
    : (cod.collectionRate != null ? num(cod.collectionRate, 0) : 0);
  ...
  var KPI_CARDS = [
    ...
    { label: s8Txt('Confirmation Rate', 'معدل التأكيد'), value: accountConfirmationPct, unit: '%', delta: 0, color: 'purple', spark: [], iconType: 'purple', tooltip: tx('kpi.confirmationRate.tooltip', s8Txt('Account-wide confirmation rate across all orders in the current dashboard period.', 'معدل التأكيد على مستوى الحساب لكل الطلبات في الفترة الحالية.')) },
    { label: s8Txt('Net Delivery Rate (NDR)', 'معدل التسليم الصافي (NDR)'), value: accountNdrPct, unit: '%', delta: 0, color: 'green', spark: [], iconType: 'green', tooltip: tx('kpi.ndr.tooltip', s8Txt('Account-wide net delivery rate based on delivered orders divided by total placed orders.', 'معدل التسليم الصافي على مستوى الحساب: الطلبات المسلمة مقارنة بإجمالي الطلبات.')) },
  ];
  ...
  var confirmationCount = (STAGES[1] ? num(STAGES[1].count, 0) : 0) + shippingCount + deliveredCount + (STAGES[2] ? num(STAGES[2].count, 0) : 0);
  var confirmationPct = totalOrders > 0 ? parseFloat(((confirmationCount / totalOrders) * 100).toFixed(1)) : 0;
  var confirmationMetricColor = window.dashboardRateColor ? window.dashboardRateColor(confirmationPct) : (confirmationPct >= 40 ? '#22d3ee' : confirmationPct >= 30 ? '#00e676' : confirmationPct >= 20 ? '#f59e0b' : '#ef4444');
  ...
  var SUMMARY_ITEMS = [
    ...
    { id: 5, label: s8Txt('Confirmation rate', 'نسبة التأكيد'), iconColor: confirmationMetricColor, value: confirmationPct.toFixed(1) + '%', sub: s8Txt(confirmationCount + ' of ' + totalOrders, confirmationCount + ' من ' + totalOrders), iconType: 'shield' },
  ...
  var citiesHtml = CITIES.map(function (c, i) {
    ...
    var cityConfirm = typeof c.confirmationPct === 'number' ? c.confirmationPct : null;
    var cityConfirmColor = cityConfirm == null
      ? 'rgba(255,255,255,0.35)'
      : window.dashboardRateColor ? window.dashboardRateColor(cityConfirm) : (cityConfirm >= 40 ? '#22d3ee' : cityConfirm >= 30 ? '#00e676' : cityConfirm >= 20 ? '#f59e0b' : '#ef4444');
    return '<div class="city-bar-row" style="display:flex;flex-direction:column;gap:6px;direction:ltr;" data-pct="' + c.pct + '" data-delay="' + (i * 40) + '">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;">' +
        ...
        '<div style="display:flex;align-items:baseline;gap:7px;">' +
          '<span style="font-size:10px;color:' + cityConfirmColor + ';font-weight:800;">' + (cityConfirm == null ? 'Confirm —' : 'Confirm ' + cityConfirm.toFixed(1) + '%') + '</span>' +
    ...
    var pageHtml = pageItems.map(function (p, i) {
      ...
      return '<div class="fade-up" style="display:flex;align-items:center;gap:10px;padding:10px 4px;border-bottom:' + border + ';direction:' + (isAr ? 'rtl' : 'ltr') + ';animation-delay:' + (i * 70) + 'ms;">' +
        ...
        '<div style="text-align:left;flex-shrink:0;min-width:52px;">' +
          '<div style="font-size:12px;font-weight:900;color:#fff;line-height:1;">' + num(p.commission, 0).toLocaleString() + '</div>' +
  ...
  (function renderScalingLeaderboard() {
    ...
    var rows = topScaling.map(function (c, i) {
      ...
      var confirmation = typeof c.confirmationPct === 'number' ? c.confirmationPct.toFixed(1) : '—';
      var confirmationColor = window.dashboardRateColor && confirmation !== '—' ? window.dashboardRateColor(parseFloat(confirmation)) : 'rgba(255,255,255,0.45)';
      var barW = Math.max(4, Math.round(c.scalingScore || 0));

      return '<div style="display:grid;grid-template-columns:28px 1fr 70px 60px 64px 60px 60px;' +
        ...
        '<div style="text-align:center;font-size:12px;font-weight:800;color:' + confirmationColor + '">' + confirmation + (confirmation === '—' ? '' : '%') + '</div>' +
    ...
    mount.innerHTML =
      '<div style="margin:0 28px 28px;background:#0b1120;border:1px solid rgba(255,255,255,0.07);' +
        ...
        '<div style="padding:0 12px 16px;display:flex;align-items:center;gap:10px;">' +
          ...
          '</div>' +
        '</div>' +
        '<div style="padding:0 12px 6px;display:grid;grid-template-columns:28px 1fr 70px 60px 64px 60px 60px;' +
          ...
          '<div style="text-align:center">' + s8Txt('Confirm', 'التأكيد') + '</div>' +
