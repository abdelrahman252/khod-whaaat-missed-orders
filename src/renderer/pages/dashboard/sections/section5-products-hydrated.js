...
function renderSection5Hydrated(mountEl, data, ctx) {
  var productTrace = function (event, detail, level) {
    if (window.DashboardTrace) window.DashboardTrace.emit('products', event, detail || {}, level);
  };
  productTrace('hydrate:body-start', { hidden: !!mountEl.hidden });
  ...
  function buildCampaignAssignments(campaignRows) {
    ...
    var attribution = window.KhodProductAttribution || window.TaagerProductAttribution;
  ...
  function productValue(product, field) {
    ...
    if (field === 'firstOrderCreatedAt') {
      var dateKey = String(value || '').slice(0, 10);
      if (!/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) return 0;
      var dateValue = new Date(dateKey + 'T00:00:00').getTime();
  ...
  function productsDataSignature() {
    ...
    return [
      ...
      first.firstOrderCreatedAt || '',
      last.key || last.sku || last.name || '',
      last.firstOrderCreatedAt || '',
      totals.orders,
      totals.delivered,
      totals.failed,
      totals.canceled,
  ...
  function requestBackendProductPage(force) {
    ...
    if (backendProductsPendingPromise && backendProductsPendingKey === key) {
      productTrace('query:reuse-inflight', { forced: !!force, page: params.page, pageSize: params.pageSize });
  ...
  var backendProductsRefreshTimer = null;
  var backendProductsRefreshAfterPending = false;
  function scheduleBackendProductsRefresh(reason, options) {
    if (!backendProductsEnabled) return false;
    options = options || {};
    if (backendProductsPendingPromise) {
      if (!backendProductsRefreshAfterPending) {
        backendProductsRefreshAfterPending = true;
        productTrace('query:refresh-after-inflight', { reason: reason || 'unknown' });
        backendProductsPendingPromise.then(function () {
          backendProductsRefreshAfterPending = false;
          if (!mountEl.isConnected || mountEl.hidden || mountEl._s5RenderToken !== renderToken) return;
          refreshBackendProducts(options.force !== false);
        });
      } else {
        productTrace('query:refresh-coalesced-after-inflight', { reason: reason || 'unknown' });
      }
      return true;
    }
    if (backendProductsRefreshTimer) {
      clearTimeout(backendProductsRefreshTimer);
      backendProductsRefreshTimer = null;
    }
    var delay = options.delay == null ? 80 : Math.max(0, Number(options.delay) || 0);
    productTrace('query:refresh-scheduled', { reason: reason || 'unknown', delayMs: delay });
    backendProductsRefreshTimer = setTimeout(function () {
      backendProductsRefreshTimer = null;
      if (!mountEl.isConnected || mountEl.hidden || mountEl._s5RenderToken !== renderToken) {
        productTrace('query:refresh-skipped', { reason: !mountEl.isConnected ? 'disconnected' : mountEl.hidden ? 'hidden' : 'stale-render' });
        return;
      }
      refreshBackendProducts(options.force !== false);
    }, delay);
...
<div class="s5-rate-track" style="width:100%;height:3px;background:rgba(255,255,255,0.07);border-radius:2px;overflow:hidden;margin-top:6px">
  ...
  function productRowHTML(p, i) {
    ...
    const firstOrderText = productDateText(p.firstOrderCreatedAt);
    ...
    return `<div class="s5-product-row s5-metrics-track" data-idx="${i}" data-product-key="${attr(productKey)}"
      ...
      <!-- Col 2c: First net order date -->
      <div class="s5-cell s5-cell-first-order" style="flex:0 0 88px;min-width:88px;text-align:center;padding:0 5px">
        <div class="s5-number-fit" title="${attr(firstOrderText)}" style="font-size:${compact?'12px':'13px'};font-weight:var(--weight-bold);color:rgba(255,255,255,0.72)">${esc(firstOrderText)}</div>
      </div>
      ${DIV}
  ...
  const SORT_OPTIONS = [
    ...
    { value: 'firstOrderCreatedAt', label: s5Txt('First Order', 'أول طلب'), icon: '📅' },
  ...
  function columnHeadersHTML() {
    ...
    return `<div class="s5-header-cols s5-metrics-track" style="display:flex;align-items:center;padding:0 0 10px 0;border-bottom:1px solid rgba(255,255,255,0.05);margin-bottom:10px;position:sticky;top:0;z-index:9;background:var(--dash-bg)">
      ...
      <div style="width:1px"></div>
      ${colHeaderBtn(s5Txt('First Order', 'أول طلب'),'firstOrderCreatedAt','flex:0 0 88px')}
      <div style="width:1px"></div>
  ...
  mountEl.innerHTML = `
    <style>
      ...
      .s5-cell-first-order { flex-basis:92px !important; min-width:92px !important; }
      ...
      .s5-header-cols .s5-sort-col[data-field="firstOrderCreatedAt"] { flex-basis:92px !important; min-width:92px !important; }
  ...
  if (typeof ResizeObserver === 'function') {
    ...
    if (sectionResources) sectionResources.observe(s5ResizeObserver, resizeTarget, {});
    else s5ResizeObserver.observe(resizeTarget);
  } else {
    if (sectionResources) sectionResources.listen(window, 'resize', syncProductDetailWidth);
    else window.addEventListener('resize', syncProductDetailWidth);
  }
  addProductCleanup(function () {
  ...
  function scheduleProductPageRender(options) {
    ...
    productTrace('background:page-render-scheduled', { keepFilterBar: !!(options && options.keepFilterBar) });
    _scheduleRenderTimer = setTimeout(function () {
      _scheduleRenderTimer = null;
      if (!mountEl.isConnected || mountEl.hidden) {
        productTrace('background:page-render-skipped', { reason: !mountEl.isConnected ? 'disconnected' : 'hidden' });
  ...
  function renderProductPage(options) {
    options = options || {};
    var productPageTimer = window.DashboardTrace ? window.DashboardTrace.start('products', 'page-render', {
      backendEnabled: !!backendProductsEnabled,
    ...
    if (backendProductsEnabled && !options.backendReady) {
      ...
      if (isCached) {
        ...
        if (productPageTimer && window.DashboardTrace) window.DashboardTrace.end(productPageTimer, { deferredForBackend: true });
        return;
      }
    }
    ...
    if (productPageTimer && window.DashboardTrace) window.DashboardTrace.end(productPageTimer, {
      visibleRows: visibleProducts.length,
      totalRows: backendProductsActive ? backendProductsTotal : list.length,
      nodeCount: mountEl.getElementsByTagName('*').length
    });
    ...
    if (window.KhodDashboardEagerProductDetailPrefetch === true &&
        window.DashboardQueryRuntime && typeof window.DashboardQueryRuntime.query === 'function') {
      ...
      if (_prefetchKeys.length) {
        productTrace('background:detail-prefetch-scheduled', { productCount: _prefetchKeys.length, delayMs: 250 });
        var _doPrefetch = function () {
          backendDetailPrefetchIdle = null;
          if (mountEl.isConnected && !mountEl.hidden && mountEl._s5RenderToken === renderToken) {
            productTrace('background:detail-prefetch-start', { productCount: _prefetchKeys.length });
            loadBackendProductDetails(_prefetchKeys);
          } else {
            productTrace('background:detail-prefetch-skipped', { reason: !mountEl.isConnected ? 'disconnected' : mountEl.hidden ? 'hidden' : 'stale-render' });
          }
        };
        ...
        var schedulePrefetch = sectionResources ? sectionResources.timeout.bind(sectionResources) : setTimeout;
        backendDetailPrefetchTimer = schedulePrefetch(function () {
          backendDetailPrefetchTimer = null;
          if (typeof requestIdleCallback === 'function') {
            backendDetailPrefetchIdle = sectionResources
              ? sectionResources.idle(_doPrefetch, { timeout: 1000 })
              : requestIdleCallback(_doPrefetch, { timeout: 1000 });
          } else {
            _doPrefetch();
          }
        }, 250);
  ...
  function refreshProductControls() {
    ...
    if (sectionResources) {
      sectionResources.listen(document, 'click', outsideClick);
      sectionResources.listen(document, 'scroll', outsideScroll, true);
      sectionResources.listen(window, 'resize', outsideScroll);
    } else {
  ...
  if (sectionResources) sectionResources.listen(window, 'dashboard-ai-filter-products', onAiProductFilter);
  else window.addEventListener('dashboard-ai-filter-products', onAiProductFilter);
  ...
  if (window.DashboardRoiState) {
    ...
    mountEl._s5RoiListener = function (settings) {
      ...
      if (String(settings.accountId) !== String(productAccountId)) return;
      if (mountEl.hidden) {
        productTrace('state:roi-ignored', { reason: 'hidden' });
        return;
      }
      productTrace('state:roi-update', { accountId: settings.accountId || '' });
      productFinancialSettings = settings;
      if (scheduleBackendProductsRefresh('roi-update')) {
        updateProductCurrencyUIOnly();
        refreshProductModal();
        refreshProductCompareModal();
        return;
    ...
    if (mountEl.parentNode) {
      if (sectionResources) sectionResources.observe(productSettingsObserver, mountEl.parentNode, { childList: true });
      else productSettingsObserver.observe(mountEl.parentNode, { childList: true });
    }
    addProductCleanup(function () {
      if (mountEl._s5RoiListener) {
        window.DashboardRoiState.unsubscribe(mountEl._s5RoiListener);
  ...
  if (window.DashboardMarketingState) {
    ...
    mountEl._s5MarketingListener = function (status) {
      ...
      if (mountEl.hidden) {
        productTrace('state:marketing-ignored', { reason: 'hidden' });
        return;
      }
      ...
      productTrace('state:marketing-update', { accountId: status.accountId || '' });
      productMarketingKey = nextMarketingKey;
      productMarketingState = status;
      if (scheduleBackendProductsRefresh('marketing-update')) return;
      applyProductFinancials();
      listCache = null;
      listCacheKey = '';
      clearProductDetailCache();
    ...
    if (mountEl.parentNode) {
      if (sectionResources) sectionResources.observe(productMarketingObserver, mountEl.parentNode, { childList: true });
      else productMarketingObserver.observe(mountEl.parentNode, { childList: true });
    }
    addProductCleanup(function () {
      if (mountEl._s5MarketingListener) {
        window.DashboardMarketingState.unsubscribe(mountEl._s5MarketingListener);
  ...
  var productsLifecycleActive = true;
  mountEl._dashboardSectionDeactivate = function () {
    if (!productsLifecycleActive) return;
    productsLifecycleActive = false;
    productTrace('lifecycle:internal-deactivate', {});
    cancelBackendDetailPrefetch();
    if (backendProductsRefreshTimer) { clearTimeout(backendProductsRefreshTimer); backendProductsRefreshTimer = null; }
    backendProductsRefreshAfterPending = false;
    if (_scheduleRenderTimer) { clearTimeout(_scheduleRenderTimer); _scheduleRenderTimer = null; }
    if (_searchDebounce) { clearTimeout(_searchDebounce); _searchDebounce = null; }
    if (mountEl._s5MarketingListener && window.DashboardMarketingState) {
      window.DashboardMarketingState.unsubscribe(mountEl._s5MarketingListener);
    }
    if (mountEl._s5RoiListener && window.DashboardRoiState) {
      window.DashboardRoiState.unsubscribe(mountEl._s5RoiListener);
    }
  };
  mountEl._dashboardSectionActivate = function () {
    if (productsLifecycleActive) return;
    productsLifecycleActive = true;
    productTrace('lifecycle:internal-activate', {});
    if (mountEl._s5RoiListener && window.DashboardRoiState) {
      window.DashboardRoiState.unsubscribe(mountEl._s5RoiListener);
      window.DashboardRoiState.subscribe(mountEl._s5RoiListener);
      if (typeof window.DashboardRoiState.get === 'function') {
        mountEl._s5RoiListener(window.DashboardRoiState.get(productAccountId, productFinancialSettings));
      }
    }
    if (mountEl._s5MarketingListener && window.DashboardMarketingState) {
      window.DashboardMarketingState.unsubscribe(mountEl._s5MarketingListener);
      window.DashboardMarketingState.subscribe(mountEl._s5MarketingListener);
      if (typeof window.DashboardMarketingState.get === 'function') {
        mountEl._s5MarketingListener(window.DashboardMarketingState.get(productAccountId));
      }
    }
  };
  ...
  productTrace('hydrate:body-end', { nodeCount: mountEl.getElementsByTagName('*').length });
}

window.renderSection5HydratedEntry = function (mountEl, data, ctx) {
  if (!mountEl) return;
  // The lightweight wrapper already waited for this hydrated bundle. A second
  // skeleton + animation-frame defer only makes the section visibly pop in.
  var wouldHaveDeferred = !window._dashboardInteractiveSectionsReady && !!(data && data.meta && data.meta.lazyHeavyModels);
  var shouldDefer = false;
  if (window.DashboardTrace) window.DashboardTrace.emit('products', 'hydrate:entry', { shouldDefer: shouldDefer, removedRedundantDefer: wouldHaveDeferred });
  if (!shouldDefer) {
