...
window.renderSectionProductForecast = function (mountEl, data, ctx) {
  ...
  var forecastRoiSettings = window.DashboardRoiState
    ...
    : { currency: 'SAR', egpRate: 52.0 };
  var viewCurrency = forecastRoiSettings.currency || 'SAR';
  ...
  var forecastMarketingState = window.DashboardMarketingState
    ? window.DashboardMarketingState.get(forecastAccountId)
    : null;
  ...
  function buildSimulator() {
    ...
    var currToggleHtml =
      '<div style="display:flex;align-items:center;gap:2px;">' +
        '<div class="s9-curr-btn ' + (viewCurrency === 'SAR' ? 's9-curr-active' : '') + '" data-curr="SAR" style="display:flex;align-items:center;gap:4px;padding:7px 16px;border-radius:20px;font-size:13px;font-weight:800;cursor:pointer;transition:.2s;' + (viewCurrency === 'SAR' ? 'background:rgba(59,130,246,0.18);border:1px solid rgba(59,130,246,0.45);color:#60a5fa;' : 'background:transparent;border:1px solid transparent;color:rgba(255,255,255,0.38);') + '"><span style="font-size:9px;font-weight:700;letter-spacing:.04em;opacity:.75">SA</span><span>SAR</span></div>' +
        '<div class="s9-curr-btn ' + (viewCurrency === 'USD' ? 's9-curr-active' : '') + '" data-curr="USD" style="display:flex;align-items:center;gap:4px;padding:7px 16px;border-radius:20px;font-size:13px;font-weight:800;cursor:pointer;transition:.2s;' + (viewCurrency === 'USD' ? 'background:rgba(59,130,246,0.18);border:1px solid rgba(59,130,246,0.45);color:#60a5fa;' : 'background:transparent;border:1px solid transparent;color:rgba(255,255,255,0.38);') + '"><span style="font-size:9px;font-weight:700;letter-spacing:.04em;opacity:.75">US</span><span>USD</span></div>' +
        '<div class="s9-curr-btn ' + (viewCurrency === 'EGP' ? 's9-curr-active' : '') + '" data-curr="EGP" style="display:flex;align-items:center;gap:4px;padding:7px 16px;border-radius:20px;font-size:13px;font-weight:800;cursor:pointer;transition:.2s;' + (viewCurrency === 'EGP' ? 'background:rgba(59,130,246,0.18);border:1px solid rgba(59,130,246,0.45);color:#60a5fa;' : 'background:transparent;border:1px solid transparent;color:rgba(255,255,255,0.38);') + '"><span style="font-size:9px;font-weight:700;letter-spacing:.04em;opacity:.75">EG</span><span>EGP</span></div>' +
      '</div>';

    var egpRateInputHtml = viewCurrency === 'EGP'
      ? '<div style="background:rgba(255,255,255,0.02);padding:16px;border-radius:12px;border:1px solid rgba(255,255,255,0.05);">' +
          '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;gap:8px;">' +
            '<label style="font-size:12px;font-weight:700;color:rgba(255,255,255,0.7);">' + p9Txt('EGP Exchange Rate', 'سعر صرف الجنيه') + '</label>' +
            '<span style="font-size:10px;color:#60a5fa;font-weight:700;">' + p9Txt('Shared with Account Calculator', 'مشترك مع حاسبة الحساب') + '</span>' +
          '</div>' +
          '<div class="sfe-input-wrap2">' +
            '<input type="number" class="s9-egp-rate-input sfe-input2" min="0.01" step="0.01" value="' + egpRate + '" style="direction:ltr;">' +
            '<span class="sfe-input-unit2">EGP / USD</span>' +
          '</div>' +
          '<div style="font-size:10px;color:rgba(255,255,255,0.48);line-height:1.6;margin-top:8px;">' + p9Txt('1 USD = ', '1 USD = ') + egpRate + ' EGP. ' + p9Txt('Changes here also update Account Calculator.', 'التعديل هنا يحدث حاسبة الحساب أيضا.') + '</div>' +
        '</div>'
      : '';
  ...
  function bindEvents() {
    ...
    var egpRateInp = mountEl.querySelector('.s9-egp-rate-input');
    if (egpRateInp) {
      egpRateInp.addEventListener('change', function (e) {
        var v = parseFloat(e.target.value);
        if (isNaN(v) || v <= 0) {
          e.target.value = egpRate;
          return;
        }
        egpRate = v;
