...
window.renderSection7 = function (mountEl, data, ctx) {
  ...
  function convert(val, from, to) {
    ...
    else if (from === "EGP") sar = (val / state.egpRate) * 3.75;
    if (to === "SAR") return sar;
    if (to === "USD") return sar / 3.75;
    if (to === "EGP") return (sar / 3.75) * state.egpRate;
    return val;
  }
  ...
  function simConvert(valSAR, to) {
    if (to === "SAR") return valSAR;
    if (to === "USD") return valSAR / 3.75;
    if (to === "EGP") return (valSAR / 3.75) * simState.egpRate;
    return valSAR;
  }
  function computeSim() {
    var s = simState;
  ...
  function updateSimUI() {
    ...
    if (commHint) commHint.textContent = simState.avgCommission + " SAR";
  ...
  function updateCalcUI() {
    ...
    if (realCommEl) {
      var convertedComm = convert(realAvgCommission, "SAR", state.currency);
      var sym =
        state.currency === "SAR"
          ? "SAR"
          : state.currency === "USD"
  ...
  function render() {
    mountEl.innerHTML =
      ...
      ".s7-input-num::-webkit-inner-spin-button,.s7-input-num::-webkit-outer-spin-button,#s7-in-egprate::-webkit-inner-spin-button,#s7-in-egprate::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}" +
      ".s7-input-num,#s7-in-egprate{-moz-appearance:textfield}" +
      ...
      '<div class="s7-lbl">' +
      s7Txt("EGP EXCHANGE RATE", "سعر صرف EGP") +
      "</div>" +
      ...
      ';border-radius:10px;overflow:hidden;height:42px;transition:.2s" id="s7-egprate-wrap">' +
      '<div style="display:flex;align-items:center;gap:5px;padding:0 10px;border-left:1px solid ' +
      (document.documentElement.getAttribute("data-theme") === "light"
        ? "#cbd5e1"
        : "rgba(255,255,255,0.07)") +
      ...
      ';letter-spacing:.06em">EGP</span>' +
      "</div>" +
      '<input type="number" id="s7-in-egprate" style="background:transparent;border:none;color:' +
      (document.documentElement.getAttribute("data-theme") === "light"
        ? "#1e293b"
        : "#fff") +
      ';font-family:Cairo,sans-serif;font-size:14px;font-weight:800;width:100%;outline:none;padding:0 10px;height:100%" value="' +
      state.egpRate +
      '" step="0.5" />' +
      "</div>" +
      "</div>" +
      ...
      '<div class="s7-tab ' +
      (state.currency === "SAR" ? "active" : "") +
      '" data-curr="SAR" ><span style="font-size:9px;font-weight:700;letter-spacing:.04em;opacity:.75">SA</span><span>SAR</span></div>' +
      '<div class="s7-tab ' +
      (state.currency === "USD" ? "active" : "") +
      '" data-curr="USD"><span style="font-size:9px;font-weight:700;letter-spacing:.04em;opacity:.75">US</span><span>USD</span></div>' +
      '<div class="s7-tab ' +
      (state.currency === "EGP" ? "active" : "") +
      ...
      "</div>" +
      '<div style="display:flex;align-items:center;gap:2px">' +
      '<div class="sfe-curr-tab ' +
      (state.currency === "SAR" ? "sfe-curr-active" : "") +
      '" data-sfecurr="SAR"><span style="font-size:9px;font-weight:700;letter-spacing:.04em;opacity:.75">SA</span><span>SAR</span></div>' +
      '<div class="sfe-curr-tab ' +
      (state.currency === "USD" ? "sfe-curr-active" : "") +
      '" data-sfecurr="USD"><span style="font-size:9px;font-weight:700;letter-spacing:.04em;opacity:.75">US</span><span>USD</span></div>' +
      '<div class="sfe-curr-tab ' +
      (state.currency === "EGP" ? "sfe-curr-active" : "") +
      '" data-sfecurr="EGP"><span style="font-size:9px;font-weight:700;letter-spacing:.04em;opacity:.75">EG</span><span>EGP</span></div>' +
      "</div>" +
      "</div>" +
      ...
      '<div class="sfe-control-row">' +
      '<label class="sfe-label">' +
      s7Txt("EGP Exchange Rate", "سعر صرف الجنيه") +
      ' <span class="sfe-label-hint" style="font-size:10px;color:#8b8fa8">' +
      s7Txt("for 3-currency results", "لعرض النتائج بثلاث عملات") +
      "</span></label>" +
      '<div class="sfe-input-wrap2"><input type="number" id="sfe-egprate" class="sfe-input2" min="1" step="0.5"><span class="sfe-input-unit2">EGP/USD</span></div>' +
      "</div>" +
      '<div class="sfe-control-row">' +
      '<label class="sfe-label">' +
      s7Txt("Net Delivery Rate", "نسبة التسليم") +
      ...
      ' <span class="sfe-label-hint" id="sfe-comm-hint">0 SAR</span></label>' +
      '<div class="sfe-input-wrap2"><input type="number" id="sfe-comm" class="sfe-input2" min="0" step="0.5" inputmode="decimal"><span class="sfe-input-unit2">SAR</span></div>' +
      "</div>" +
  ...
  function initCurrencySelect() {
    ...
    if (!wrap || typeof renderCustomSelect !== "function") {
      ...
      wrap.innerHTML =
        ...
        "</option>" +
        '<option value="EGP" ' +
        (state.currency === "EGP" ? "selected" : "") +
        ">" +
        s7Txt("Egyptian Pound (EGP)", "جنيه (EGP)") +
        "</option>" +
        "</select>";
      document
    ...
    var options = [
      {
        value: "SAR",
        label: s7Txt("SA Saudi Riyal (SAR)", "SA ريال سعودي (SAR)"),
      },
      {
        value: "USD",
        label: s7Txt("US Dollar (USD)", "US دولار أمريكي (USD)"),
      },
      {
        value: "EGP",
        label: s7Txt("EG Egyptian Pound (EGP)", "EG جنيه مصري (EGP)"),
      },
    ];
  ...
  function initSimInputs() {
    ...
    var inpEgpRate = document.getElementById("sfe-egprate");
    ...
    if (inpEgpRate) inpEgpRate.value = simState.egpRate;
  }

  // ── Wire Calculator Events ──────────────────────────────────────────────────
  function wireCalcEvents() {
    // Budget input event is now handled reactively by initMoneyInput formatter.
    var egpRateInput = document.getElementById("s7-in-egprate");
    var egpRateWrap = document.getElementById("s7-egprate-wrap");
    if (egpRateInput) {
      egpRateInput.addEventListener("input", function (e) {
        state.egpRate = parseFloat(e.target.value) || 52;
        persistCalculatorSettings();
        updateCalcUI();
        updateSimUI(); // Also update sim to keep egpRate in sync
      });
      egpRateInput.addEventListener("focus", function () {
        var isLight =
          document.documentElement.getAttribute("data-theme") === "light";
        if (egpRateWrap) {
          egpRateWrap.style.borderColor = "#3b82f6";
          egpRateWrap.style.background = isLight
            ? "#ffffff"
            : "rgba(15,23,42,0.9)";
          egpRateWrap.style.boxShadow = "0 0 0 2px rgba(59,130,246,0.1)";
        }
      });
      egpRateInput.addEventListener("blur", function () {
        var isLight =
          document.documentElement.getAttribute("data-theme") === "light";
        if (egpRateWrap) {
          egpRateWrap.style.borderColor = isLight
            ? "#cbd5e1"
            : "rgba(255,255,255,0.08)";
          egpRateWrap.style.background = isLight
            ? "#ffffff"
            : "rgba(15,23,42,0.6)";
          egpRateWrap.style.boxShadow = "none";
        }
      });
    }
  ...
  function wireSimEvents() {
    ...
    var egpRateEl = document.getElementById("sfe-egprate");
    ...
    if (egpRateEl)
      egpRateEl.addEventListener("input", function (e) {
        simState.egpRate = parseFloat(e.target.value) || 52;
        onChange();
      });
