...
window.renderSection6HydratedEntry = function (mountEl, data, ctx) {
  ...
  let areaChartSignature = '';
  let donutChartSignature = '';
  let PLATFORM_ROWS = [];
  ...
  function recommendationsHtml() {
    ...
    scheduleInitialCharts();
    animateTotalCountup();
  }

  function scheduleInitialCharts() {
    if (chartRenderFrame != null) cancelAnimationFrame(chartRenderFrame);
    chartRenderFrame = requestAnimationFrame(function () {
      chartRenderFrame = null;
      if (!mountEl.isConnected || mountEl.hidden) return;
      buildAreaChart();
      buildDonutChart();
    });
  }
  ...
  function buildAreaChart() {
    ...
    const chartData = getChartData(activePeriod);
    const nextSignature = JSON.stringify([activePeriod, activeMode, activeCpaPlatform, activeCurrency, dailyAdSpend, chartData]);
    if (chartInstance && chartInstance.canvas === canvas && areaChartSignature === nextSignature) return;
    if (chartInstance) { chartInstance.destroy(); chartInstance = null; }
    areaChartSignature = nextSignature;
    const labels = chartData.map(x => x.d);
  ...
  function buildDonutChart() {
    ...
    if (!canvas || typeof Chart === 'undefined') return;
    const nextSignature = JSON.stringify([activeCurrency, PERF_DATA.map(function (item) { return [item.label, item.pct, item.color]; })]);
    if (donutChartInstance && donutChartInstance.canvas === canvas && donutChartSignature === nextSignature) return;
    if (donutChartInstance) { donutChartInstance.destroy(); donutChartInstance = null; }
    donutChartSignature = nextSignature;
    const theme = window.dashboardThemeColors ? window.dashboardThemeColors() : {
      surface: 'rgba(11,17,32,0.95)',
      borderSoft: 'rgba(255,255,255,0.1)',
      text: '#fff'
  ...
  var commissionLifecycleActive = true;
  mountEl._dashboardSectionDeactivate = function () {
    if (!commissionLifecycleActive) return;
    commissionLifecycleActive = false;
    if (chartRenderFrame != null) {
      cancelAnimationFrame(chartRenderFrame);
      chartRenderFrame = null;
    }
    if (mountEl._s6RoiListener && window.DashboardRoiState) window.DashboardRoiState.unsubscribe(mountEl._s6RoiListener);
    if (mountEl._s6MarketingListener && window.DashboardMarketingState) window.DashboardMarketingState.unsubscribe(mountEl._s6MarketingListener);
  };
  mountEl._dashboardSectionActivate = function () {
    if (commissionLifecycleActive) return;
    commissionLifecycleActive = true;
    if (mountEl._s6RoiListener && window.DashboardRoiState) window.DashboardRoiState.subscribe(mountEl._s6RoiListener);
    if (mountEl._s6MarketingListener && window.DashboardMarketingState) window.DashboardMarketingState.subscribe(mountEl._s6MarketingListener);
    if (!chartInstance || !donutChartInstance) scheduleInitialCharts();
  };
