(function () {
  ...
  function accountMappingKeys(account) {
    ...
    // Compatibility only: old saved mappings may still be keyed by Taager aliases.
  ...
  window.renderSectionMarketingConnectionsHydratedEntry = function renderSectionMarketingConnections(mount, data, ctx) {
    ...
    function assignedAccountsMarkup(current, assigned, platform) {
      ...
      return '<section class="marketing-mapping-board">' +
        ...
        '<p class="marketing-map-empty" hidden>' + escapeHtml(tr('marketing.assignmentMemberBody', 'No ' + label + ' ad account has been assigned to this KHOD account yet. Connect ' + label + ' or ask an admin to assign the correct account.').replace(/\{platform\}/g, label)) + '</p>' +
      ...
      if (!allMode && currentMapping.length) {
        mappingContent =
          mappingDisclosureMarkup(
            ...
            tr('marketing.assignedBody', 'This KHOD account is linked only to the ' + label + ' account assigned to it.').replace(/\{platform\}/g, label),
        ...
        mappingContent =
          mappingDisclosureMarkup(
            platform,
            tr('marketing.mappingTitle', 'Map known ' + label + ' accounts to KHOD accounts').replace(/\{platform\}/g, label),
            tr('marketing.mappingBody', 'Only ad accounts known to this KHOD app appear here. Select all that belong to each account.').replace(/\{platform\}/g, label),
    ...
    function render() {
      ...
      if (!mount.isConnected || mount.hidden) return;
      mount.innerHTML =
    ...
    function saveAllMappings(platform) {
      ...
      var payloads = rows.map(function (row) {
        ...
        var dashboardAccount = shownAccounts.concat(khodAccounts).filter(function (account) {
    ...
    function bind() {
      ...
      Array.prototype.forEach.call(releaseButtons, function (releaseButton) {
        releaseButton.addEventListener('click', async function () {
          ...
          var confirmed = window.KhodUI && typeof window.KhodUI.confirm === 'function'
            ? await window.KhodUI.confirm({
      ...
      Array.prototype.forEach.call(syncButtons, function (syncButton) {
        syncButton.addEventListener('click', function () {
          ...
          var exchangeRates = window.KhodCurrency && typeof window.KhodCurrency.rates === 'function'
            ? window.KhodCurrency.rates()
            : { USD: 1, SAR: 3.75, EGP: currentGlobalEgpRate(roi.egpRate), AED: 3.6725, IQD: 1310, OMR: 0.385 };
          var syncPayload = {
      ...
      Array.prototype.forEach.call(syncAllButtons, function (syncAllButton) {
        syncAllButton.addEventListener('click', function () {
          ...
          var accountSettings = khodAccounts.map(function (account) {
            ...
            var accountCountry = account.khodCountry || account.khod_country || 'sa';
            var nativeCurrency = accountCountry && window.TaagerCountry && window.TaagerCountry.currency
              ? window.TaagerCountry.currency(accountCountry)
    ...
    if (mount._marketingConnectionsListener && store && typeof store.unsubscribe === 'function') {
      store.unsubscribe(mount._marketingConnectionsListener);
    }
    ...
    mount._marketingConnectionsListener = listener;
    ...
    function activateMarketingConnections() {
      if (lifecycleActive) return;
      lifecycleActive = true;
      window.removeEventListener('focus', refreshAfterAuthorizationFocus);
      window.removeEventListener('khod-lang-change', render);
      window.addEventListener('focus', refreshAfterAuthorizationFocus);
      window.addEventListener('khod-lang-change', render);
      if (store && typeof store.unsubscribe === 'function') store.unsubscribe(listener);
      if (store && typeof store.subscribe === 'function') store.subscribe(listener);
      if (hasActivatedOnce) {
        render();
      } else {
    ...
    function deactivateMarketingConnections() {
      if (!lifecycleActive) return;
      lifecycleActive = false;
      window.removeEventListener('focus', refreshAfterAuthorizationFocus);
      window.removeEventListener('khod-lang-change', render);
      stopAutoRefresh('inactive');
    ...
    mount._dashboardSectionActivate = activateMarketingConnections;
    mount._dashboardSectionDeactivate = deactivateMarketingConnections;
    activateMarketingConnections();

    return function cleanupMarketingConnections() {
      deactivateMarketingConnections();
      if (mount._marketingConnectionsListener && store && typeof store.unsubscribe === 'function') {
        store.unsubscribe(mount._marketingConnectionsListener);
      }
      mount._marketingConnectionsListener = null;
