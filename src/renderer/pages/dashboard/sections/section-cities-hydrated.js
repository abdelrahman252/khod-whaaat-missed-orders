// section-cities.js â€” Ø§Ù„Ù…Ø¯Ù† ÙˆØ§Ù„Ù…Ù†Ø§Ø·Ù‚
...
window.renderSectionCitiesHydratedEntry = function (mountEl, data, ctx) {
  ...
  var sectionResources = mountEl._dashboardResources || null;
  function scheduleCitiesTimeout(callback, delay) {
    return sectionResources ? sectionResources.timeout(callback, delay) : window.setTimeout(callback, delay);
  }
  function scheduleCitiesFrame(callback) {
    return sectionResources ? sectionResources.raf(callback) : window.requestAnimationFrame(callback);
  }
  ...
  if (mountEl._citiesFilterBusObserver) {
    ...
    mountEl._citiesFilterBusObserver = null;
  }
  if (mountEl._citiesVirtualObserver) {
    mountEl._citiesVirtualObserver.disconnect();
    mountEl._citiesVirtualObserver = null;
  }
  if (mountEl._citiesSecondaryVisibilityObserver) {
    mountEl._citiesSecondaryVisibilityObserver.disconnect();
    mountEl._citiesSecondaryVisibilityObserver = null;
  }
  if (mountEl._citiesLeaderboardResizeHandler) {
    window.removeEventListener("resize", mountEl._citiesLeaderboardResizeHandler);
    mountEl._citiesLeaderboardResizeActive = false;
  }
  ...
  if (mountEl._citiesQueryIdleHandle != null) {
    cancelCitiesIdle(mountEl._citiesQueryIdleHandle);
    mountEl._citiesQueryIdleHandle = null;
  }
  ...
  function whenCitiesIdle(callback, timeout) {
    var handle = { timeoutId: null, idleId: null };
    var deadline = Math.max(80, Number(timeout || 800) || 800);
    function guardedCallback(idleDeadline) {
    ...
    if (window.requestIdleCallback) {
      // The timeout is a maximum wait, not an artificial delay. Taager's
      // cities pane becomes complete quickly because this work starts at the
      // first idle opportunity instead of sleeping for multiple seconds.
      handle.idleId = window.requestIdleCallback(guardedCallback, { timeout: deadline });
    } else {
      handle.timeoutId = window.setTimeout(guardedCallback, 0);
    }
    return handle;
  }
  ...
  function cancelCitiesIdle(handle) {
    ...
    if (typeof handle === "object") {
      ...
      if (handle.idleId != null && window.cancelIdleCallback) window.cancelIdleCallback(handle.idleId);
      return;
    }
    if (window.requestIdleCallback) {
  ...
  function applyCitiesQueryResult(result) {
    ...
    if (window.DashboardTrace) {
      window.DashboardTrace.emit("cities", "query:result-applied-light", {
        cityCount: result.cities.length,
        skippedGeoProductMapRebuild: true
      });
    }
  }

  // Keep first paint interactive. The query-backed city snapshot can be slower
  // on large KHOD data, so refresh it in the background instead of blocking the
  // section search/filter UI from appearing.
  var citiesQueryFlags = window.DashboardQueryRuntime &&
      typeof window.DashboardQueryRuntime.currentFlags === "function"
  ...
  if (!mountEl._citiesQueryDone &&
    ...
    if (!mountEl._citiesQueryPending) {
      mountEl._citiesQueryPending = new Promise(function (resolve) {
        mountEl._citiesQueryIdleHandle = whenCitiesIdle(function () {
          mountEl._citiesQueryIdleHandle = null;
          resolve(window.DashboardQueryRuntime.flags().then(function (flags) {
        ...
        mountEl._citiesQueryPending = null;
        if (window.DashboardTrace) {
          window.DashboardTrace.emit("cities", "query:deferred-full-rerender-skipped", {
            active: !!(mountEl.isConnected && !mountEl.hidden && ctx && ctx.sectionId === "cities")
          });
        }
      });
    }
  ...
  function s6Txt(en, ar) {
    ...
    return String(value == null ? '' : value)
      ...
      .replace(/\bTiger profit(?: after tax)?\b/g, 'marketer commission')
      .replace(/Ø±Ø¨Ø­ ØªØ§Ø¬Ø±(?: Ø¨Ø¹Ø¯ Ø§Ù„Ø¶Ø±ÙŠØ¨Ø©)?/g, 'Ø¹Ù…ÙˆÙ„Ø© Ø§Ù„Ù…Ø³ÙˆÙ‚');
  }
  ...
  function openExclusionModal(cities, reason) {
    ...
    function platformBtn(key, label, active) {
      return (
        ...
        ";font-size:12px;font-weight:var(--weight-semibold);font-family:inherit;cursor:pointer;" +
        '">' +
        label +
        "</button>"
      );
    ...
    var preview = items.slice(0, 10).map(function (city) {
      ...
      return (
        ...
        'color:#fecaca;font-size:11px;font-weight:var(--weight-semibold);">' +
    ...
    overlay.innerHTML =
      ...
      '<div>' +
      '<div style="font-size:18px;font-weight:var(--weight-bold);color:#fff;margin-bottom:5px;">' +
      s6Txt("Ad Set Exclusion List", "Ù‚Ø§Ø¦Ù…Ø© Ø§Ø³ØªØ¨Ø¹Ø§Ø¯ Ø§Ù„Ù…Ø¯Ù† Ù„Ù„Ø¥Ø¹Ù„Ø§Ù†Ø§Øª") +
      "</div>" +
      '<div style="font-size:12px;color:rgba(255,255,255,0.52);line-height:1.7;">' +
      esc(reason || s6Txt("Cities with weak NDR are recommended for exclusion.", "Ø§Ù„Ù…Ø¯Ù† Ø°Ø§Øª NDR Ø§Ù„Ø¶Ø¹ÙŠÙ ÙŠÙØ¶Ù„ Ø§Ø³ØªØ¨Ø¹Ø§Ø¯Ù‡Ø§ Ù…Ù† Ø§Ù„Ø­Ù…Ù„Ø§Øª.")) +
      "</div>" +
      "</div>" +
      '<button id="sc-exclusion-close" style="width:34px;height:34px;border-radius:10px;border:1px solid rgba(255,255,255,0.08);' +
      'background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.72);font-size:18px;font-weight:var(--weight-bold);cursor:pointer;">x</button>' +
      "</div>" +
      '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px;">' +
      platformBtn("plain", s6Txt("Plain", "Ù‚Ø§Ø¦Ù…Ø© Ø¹Ø§Ø¯ÙŠØ©"), true) +
      ...
      (preview || '<span style="color:rgba(255,255,255,0.42);font-size:12px;">' + s6Txt("No weak-NDR cities found.", "Ù„Ø§ ØªÙˆØ¬Ø¯ Ù…Ø¯Ù† Ø¶Ø¹ÙŠÙØ© NDR.") + "</span>") +
      "</div>" +
      '<button id="sc-exclusion-copy" style="padding:10px 16px;border-radius:11px;border:1px solid rgba(20,184,166,0.35);' +
      'background:rgba(20,184,166,0.15);color:#99f6e4;font-size:12px;font-weight:var(--weight-bold);font-family:inherit;cursor:pointer;">' +
      s6Txt("Copy List", "Ù†Ø³Ø® Ø§Ù„Ù‚Ø§Ø¦Ù…Ø©") +
      "</button>" +
      "</div>" +
      "</div>";
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(function () {
          if (window.TaagerUI && window.TaagerUI.toast) {
            window.TaagerUI.toast(s6Txt("Exclusion list copied.", "ØªÙ… Ù†Ø³Ø® Ù‚Ø§Ø¦Ù…Ø© Ø§Ù„Ø§Ø³ØªØ¨Ø¹Ø§Ø¯."), "success");
          }
        });
      } else {
        output.select();
  ...
  if (false) {
    (function debugCitiesSection() {
      ...
      mountEl.innerHTML =
        '<div dir="rtl" style="flex:1;overflow-y:auto;background:#080b12;color:#fff;font-family:var(--font-ui);padding:24px 28px 38px;">' +
        ...
        s6Txt(
          ...
          "Ù…Ø±ØªØ¨Ø© Ø­Ø³Ø¨ ÙØ¬ÙˆØ© Ø§Ù„ØªØ­ØµÙŠÙ„ ÙˆØ¹Ø¯Ø¯ Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ù…Ù† Ù„Ù‚Ø·Ø© Dashboard Ø§Ù„Ø­Ø§Ù„ÙŠØ©",
        ) +
        "</div>" +
        '<div><h1 style="margin:0;font-size:28px;font-weight:var(--weight-bold);">' +
        s6Txt("Cities and Regions", "Ø§Ù„Ù…Ø¯Ù† ÙˆØ§Ù„Ù…Ù†Ø§Ø·Ù‚") +
        '</h1><p style="margin:6px 0 0;color:rgba(255,255,255,0.42);font-size:12px;">' +
        s6Txt(
          "Real data based on the selected account",
          "Ø¨ÙŠØ§Ù†Ø§Øª Ø­Ù‚ÙŠÙ‚ÙŠØ© Ø­Ø³Ø¨ Ø§Ù„Ø­Ø³Ø§Ø¨ Ø§Ù„Ù…Ø­Ø¯Ø¯",
        ) +
        "</p></div>" +
        "</div>" +
        ...
        realCities
          .map(function (city, idx) {
            ...
            return (
              ...
              '<div style="font-size:12px;color:#a855f7;font-weight:var(--weight-semibold);">#' +
              (idx + 1) +
              "</div>" +
              '<div style="font-size:16px;font-weight:var(--weight-bold);text-align:right;">' +
              city.name +
              "</div>" +
              "</div>" +
              ...
              s6Txt("Orders", "Ø§Ù„Ø·Ù„Ø¨Ø§Øª") +
              "</div><strong>" +
              (city.count || 0).toLocaleString("en-US") +
              "</strong></div>" +
              '<div style="background:rgba(255,255,255,0.035);border-radius:10px;padding:10px;"><div style="color:rgba(255,255,255,0.42);margin-bottom:4px;">' +
              s6Txt("Collection", "Ø§Ù„ØªØ­ØµÙŠÙ„") +
              "</div><strong>" +
              (city.pct || 0).toLocaleString("en-US") +
              "%</strong></div>" +
              '<div style="background:rgba(255,255,255,0.035);border-radius:10px;padding:10px;"><div style="color:rgba(255,255,255,0.42);margin-bottom:4px;">' +
              s6Txt("Remaining", "Ø§Ù„Ù…ØªØ¨Ù‚ÙŠ") +
              "</div><strong>" +
              fmtSAR(city.gap || 0) +
              "</strong></div>" +
              '<div style="background:rgba(255,255,255,0.035);border-radius:10px;padding:10px;"><div style="color:rgba(255,255,255,0.42);margin-bottom:4px;">' +
              s6Txt("Due", "Ø§Ù„Ù…Ø³ØªØ­Ù‚") +
              "</div><strong>" +
              fmtSAR(city.due || 0) +
              "</strong></div>" +
              "</div>" +
  ...
  // â”€â”€ Province + city data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if (realCities.length > 0) {
    ...
    realCities.forEach(function (city, idx) {
      ...
      pDef.cities.push({
        name: city.name || s6Txt("Unknown", "ØºÙŠØ± Ù…Ø¹Ø±ÙˆÙ"),
        ...
        exact: !!(geoPoint && geoPoint.exact),
        exactPoint: !!(geoPoint && geoPoint.exact),
  ...
  PROVINCES.forEach(function (p) {
    p.cities.forEach(function (c) {
      ALL_CITIES.push({
        ...
        exact: c.exact === true || c.exactPoint === true,
        exactPoint: c.exactPoint === true,
        x: c.x,
        y: c.y,
        provinceId: p.id,
        provinceName: p.name,
  ...
  if (window.TaagerGeo && typeof window.TaagerGeo.spreadCityPoints === "function") {
    var spreadCountry = String(activeMapCountry() || "sa").toLowerCase();
    var saSpread = spreadCountry === "sa";
    ALL_CITIES = window.TaagerGeo.spreadCityPoints(ALL_CITIES, {
      minDistance: saSpread ? 12 : 18,
      maxShift: saSpread ? 10 : 18,
      iterations: saSpread ? 8 : 12,
      preserveExact: true
    });
  }

  var MAX_ORDERS = (ALL_CITIES[0] && ALL_CITIES[0].orders) || 1;
  var MAP_CITY_LIMIT = String(activeMapCountry() || "sa").toLowerCase() === "sa" ? 70 : 60;
  var MAP_CITIES = ALL_CITIES.slice(0, Math.min(ALL_CITIES.length, MAP_CITY_LIMIT));
  ...
  })[0] || ALL_CITIES[0] || { name: 'â€”', deliveryRate: 0 };
  var WATCH_CITY = ELIGIBLE_CITIES.slice().sort(function (a, b) {
    return a.deliveryRate - b.deliveryRate;
  })[0] || ALL_CITIES[ALL_CITIES.length - 1] || { name: 'â€”', deliveryRate: 0 };
  ...
  // â”€â”€ Phase 5: Global Filter Bar HTML â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function filterBarHTML() {
    ...
    var provinceOpts =
      '<option value="">' +
      s6Txt("Region: All", "Ø§Ù„Ù…Ù†Ø·Ù‚Ø©: Ø§Ù„ÙƒÙ„") +
      "</option>" +
    ...
    var productOpts =
      '<option value="">' + s6Txt("Product: All", "Ø§Ù„Ù…Ù†ØªØ¬: Ø§Ù„ÙƒÙ„") + "</option>";
    ...
    var selStyle =
      ...
      "color:#fff;font-family:var(--font-ui);font-size:12px;font-weight:var(--weight-bold);" +
      paddingStr +
    ...
    function pillStyle(active) {
      return (
        "padding:7px 14px;border-radius:9px;border:none;font-size:11px;font-weight:var(--weight-semibold);" +
    ...
    return (
      ...
      '">' +
      s6Txt("Prepaid", "Ù…Ø³Ø¨Ù‚ Ø§Ù„Ø¯ÙØ¹") +
      "</button>" +
      ...
      s6Txt("Search city...", "Ø¨Ø­Ø« Ø¹Ù† Ù…Ø¯ÙŠÙ†Ø©...") +
      '" value="' + esc(searchVal) + '" style="' +
      "width:100%;background:#0b1120;border:1px solid rgba(255,255,255,0.1);border-radius:10px;" +
      "color:#fff;font-family:var(--font-ui);font-size:12px;padding:8px 12px;outline:none;" +
      ...
      s6Txt("Reset city analysis filters", "Ø¥Ø¹Ø§Ø¯Ø© Ø¶Ø¨Ø· ÙÙ„Ø§ØªØ± ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ù…Ø¯Ù†") +
      '" title="' + s6Txt("Reset city analysis filters", "Ø¥Ø¹Ø§Ø¯Ø© Ø¶Ø¨Ø· ÙÙ„Ø§ØªØ± ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ù…Ø¯Ù†") + '">â†º ' +
      s6Txt("Reset", "Ø¥Ø¹Ø§Ø¯Ø© Ø¶Ø¨Ø·") +
      "</button>" +
      '<button id="sc-fb-exclude-export" style="padding:7px 12px;border-radius:9px;border:1px solid rgba(168,85,247,0.3);' +
      "background:rgba(168,85,247,0.1);color:#e9d5ff;font-size:11px;font-weight:var(--weight-semibold);" +
      'cursor:pointer;font-family:inherit;transition:all 0.18s;white-space:nowrap;">' +
      s6Txt("Export Exclusions", "ØªØµØ¯ÙŠØ± Ø§Ù„Ø§Ø³ØªØ¨Ø¹Ø§Ø¯") +
      "</button>" +
      "</div>"
    );
  }

  // â”€â”€ KPI cards HTML â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function kpiRowHTML() {
    var cards = [
      {
        icon: "package",
        color: "#a855f7",
        label: s6Txt("Net Orders", "ØµØ§ÙÙŠ Ø§Ù„Ø·Ù„Ø¨Ø§Øª"),
      ...
      },
      {
        icon: "trendingUp",
        color: "#14b8a6",
        label: s6Txt("NDR (Total)", "NDR (Ø¥Ø¬Ù…Ø§Ù„ÙŠ)"),
      ...
      },
      {
        icon: "trendingUp",
        color: "#7c3aed",
        label: s6Txt("DR (Active)", "DR (Ø§Ù„Ù†Ø´Ø·Ø©)"),
      ...
      },
      {
        icon: "checkCircle",
        color: "#22d3ee",
        label: s6Txt("Delivered Orders", "Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„Ù…Ø³Ù„Ù…Ø©"),
      ...
      },
      {
        icon: "clipboardCheck",
        color: "#3b82f6",
        label: s6Txt("Confirmed Orders", "Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„Ù…Ø¤ÙƒØ¯Ø©"),
      ...
      },
      {
        icon: "award",
        color: "#00e676",
        label: s6Txt("Best Delivery City", "Ø£ÙØ¶Ù„ Ù…Ø¯ÙŠÙ†Ø© ØªØ³Ù„ÙŠÙ…Ø§Ù‹"),
        value: BEST_CITY.name,
        sub:
          formatPercent(BEST_CITY.deliveryRate) + "%" + s6Txt(" delivery rate", " Ù…Ø¹Ø¯Ù„ ØªØ³Ù„ÙŠÙ…"),
        trend: null,
      },
      {
        icon: "alertTriangle",
        color: "#ef4444",
        label: s6Txt("Needs Attention", "ØªØ­ØªØ§Ø¬ Ù…ØªØ§Ø¨Ø¹Ø©"),
        value: WATCH_CITY.name,
        sub:
          formatPercent(WATCH_CITY.deliveryRate) +
          "%" +
          s6Txt(" delivery rate", " Ù…Ø¹Ø¯Ù„ ØªØ³Ù„ÙŠÙ…"),
        trend: null,
      },
    ];
    ...
    return cards
      .map(function (c, i) {
        var trendHTML = c.trend
          ? '<div style="display:flex;align-items:center;gap:4px;font-size:11px;font-weight:var(--weight-bold);' +
        ...
        return (
          ...
          '<div style="font-size:22px;font-weight:var(--weight-bold);color:#fff;line-height:1">' +
          c.value +
          "</div>" +
  ...
  // â”€â”€ Map HTML (SVG) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function mixedCountryMapsHTML(countryGroups) {
    ...
    return '<div class="fade-up" style="animation-delay:80ms;background:#0b1120;border:1px solid rgba(255,255,255,0.06);border-radius:18px;padding:20px;display:flex;flex-direction:column;gap:12px;">' +
      '<div style="font-size:15px;font-weight:var(--weight-semibold);color:#fff;">' + s6Txt("Geographical Distribution by Country", "Ø§Ù„ØªÙˆØ²ÙŠØ¹ Ø§Ù„Ø¬ØºØ±Ø§ÙÙŠ Ø­Ø³Ø¨ Ø§Ù„Ø¯ÙˆÙ„Ø©") + '</div>' +
      ...
      countryGroups.map(function (group, groupIdx) {
        ...
        var dots = countryCities.slice(0, 12).map(function (city, idx) {
          ...
          return '<circle cx="' + point.x + '" cy="' + point.y + '" r="' + radius + '" fill="' + (city.color || "#a855f7") + '" stroke="rgba(255,255,255,.7)" stroke-width="1"><title>' + esc(city.name) + ' Â· ' + s7Num(city.orders || 0) + '</title></circle>';
        }).join("");
        ...
        return '<div style="border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.025);border-radius:14px;padding:10px;">' +
          '<div style="display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:6px;"><strong style="font-size:11px;color:#fff;">' + esc(label) + '</strong><span style="font-size:9px;color:rgba(255,255,255,.48);">' + esc(group.currency || "") + ' Â· ' + s7Num(group.orderCount || group.rows || 0) + '</span></div>' +
  ...
  function mapHTML() {
    // Match the Taager fast path. SVG blur/drop-shadow filters keep Chromium's
    // compositor busy after the section is visible and can slow every later
    // dashboard interaction, even though the map itself is static.
    var fastMapPaint = true;
    ...
    var countryLabel = activeCountry === "mixed"
      ? s6Txt("Mixed countries", "Ø¹Ø¯Ø© Ø¯ÙˆÙ„")
    ...
    var countryTitleSuffix = String(activeCountry || "sa").toLowerCase() === "sa"
      ? ""
      : " Â· " + esc(countryLabel);
        ...
        countryGroups.map(function (group) {
          ...
          return '<div style="border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.035);border-radius:10px;padding:8px 10px;display:flex;align-items:center;justify-content:space-between;gap:8px;">' +
            '<span style="display:flex;align-items:center;gap:7px;min-width:0;">' +
              ...
              '<span style="font-size:11px;font-weight:var(--weight-semibold);color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + esc(label) + '</span>' +
            '</span>' +
            '<span style="font-size:10px;font-weight:var(--weight-semibold);color:rgba(255,255,255,0.58);white-space:nowrap;">' +
              esc(group.currency || "") + ' Â· ' + s7Num(group.orderCount || group.rows || 0) +
            '</span>' +
          '</div>';
        }).join("") +
      '</div>'
    ...
    var cityDots = MAP_CITIES.map(function (c) {
    ...
    var legendChips = PROVINCES.map(function (p) {
      return (
        ...
        p.color +
        ";" +
        "font-size:10px;font-weight:var(--weight-bold);cursor:pointer;font-family:inherit;" +
        'transition:all 0.2s;">' +
    ...
    var isAr = window.dashboardI18n
      ...
      : false;
    var heatModes = [
      { id: "default", label: s6Txt("Default", "Ø§ÙØªØ±Ø§Ø¶ÙŠ"), icon: "ðŸ—ºï¸" },
      { id: "ndr", label: s6Txt("NDR Index", "Ù…Ø¤Ø´Ø± NDR"), icon: "âš ï¸" },
      { id: "revenue", label: s6Txt("Revenue", "Ø§Ù„Ø¥ÙŠØ±Ø§Ø¯Ø§Øª"), icon: "ðŸ’°" },
    ];
    var heatPills = heatModes
      .map(function (m, i) {
        ...
        return (
          ...
          m.id +
          '" style="' +
          "padding:5px 11px;border-radius:9px;border:none;font-size:11px;font-weight:var(--weight-bold);" +
    ...
    return (
      ...
      '<span style="font-size:15px;font-weight:var(--weight-semibold);color:#fff">' +
      s6Txt("Geographical Distribution Map", "Ø®Ø±ÙŠØ·Ø© Ø§Ù„ØªÙˆØ²ÙŠØ¹ Ø§Ù„Ø¬ØºØ±Ø§ÙÙŠ") +
      countryTitleSuffix +
      "</span>" +
      "</div>" +
      '<button id="sc-clear-btn" style="display:none;font-size:11px;font-weight:var(--weight-bold);' +
      ...
      s6Txt("Show All âœ•", "Ø¹Ø±Ø¶ Ø§Ù„ÙƒÙ„ âœ•") +
      "</button>" +
      "</div>" +
      mixedCountryCards +
      ...
      s6Txt(
        ...
        "ÙŠØ¹Ø±Ø¶ Ø§Ù„Ù…Ù†Ø§Ø·Ù‚ Ù…Ø¬Ù…Ø¹Ø© Ø­Ø³Ø¨ Ø§Ù„Ù„ÙˆÙ† Ù„Ø¥Ø¸Ù‡Ø§Ø± Ø§Ù„Ù†Ø´Ø§Ø· Ø§Ù„Ø¹Ø§Ù….",
      ) +
      "</div>" +
      ...
      '<div id="sc-tt-name" style="font-size:14px;font-weight:var(--weight-semibold);line-height:1.45;margin-bottom:3px"></div>' +
      '<div id="sc-tt-info" style="font-size:12px;font-weight:var(--weight-bold);line-height:1.45"></div>' +
      "</div>" +
      "</div>" +
  ...
  // â”€â”€ Leaderboard HTML â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function leaderboardHTML() {
    ...
    var rows = ALL_CITIES.map(function (c, i) {
      ...
      // T-07: Resolve NDR% â€” prefer aggregator cityStats, fall back to returnRate
      ...
      var ndrColor =
        ndrVal === null
          ...
          : rateColor(ndrVal);
      var ndrDisplay = ndrVal === null ? "â€”" : pctValue(ndrVal) + "%";

      // Sum of marketer commission for unique delivered orders in the selected period.
      ...
      function ndrBadge(val) {
        if (val === null)
          return '<span style="font-size:11px;font-weight:var(--weight-semibold);color:rgba(255,255,255,0.25);background:rgba(255,255,255,0.05);padding:2px 6px;border-radius:6px;">â€”</span>';
        var nc = rateColor(val);
        return (
          '<span style="font-size:11px;font-weight:var(--weight-semibold);color:' +
          nc +
          ";background:" +
          nc +
      ...
      // Delivered orders: prefer direct field, fallback to orders Ã— NDR%
      var deliveredVal =
      ...
      var deliveredDisplay =
        deliveredVal !== null ? deliveredVal.toLocaleString("en-US") : "â€”";
      var confirmedVal =
      ...
      return (
        ...
        '<span style="color:#fff;font-weight:var(--weight-bold);font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="' +
        esc(c.name) +
        '">' +
        c.name +
        "</span>" +
        ...
        c.color +
        '"></span>' +
        '<span class="sc-lb-rank" style="font-size:10px;color:rgba(255,255,255,0.2);font-weight:var(--weight-bold);width:16px;text-align:center">' +
        (i + 1) +
        "</span>" +
        "</div>" +
        /* Orders â€” switchable by pill */
        ...
        "</div>" +
        '<div class="sc-lb-confirmed-cell" style="color:#3b82f6;font-size:12px;font-weight:var(--weight-bold);text-align:center">' +
        ...
        "%</span></div>" +
        /* DR% (Active) + bar â€” uses drRate (active orders only) */
        '<div class="sc-lb-dr-cell" style="display:flex;flex-direction:column;gap:3px">' +
        '<span style="font-size:12px;font-weight:var(--weight-bold);text-align:right;color:' +
        drColor2 +
        '">' +
        ...
        "</div>" +
        "</div>" +
        /* NDR% â€” switchable by pill */
        '<div class="sc-lb-ndr-cell" style="text-align:center;">' +
        ndrBadge(rowTotalNdr) +
        "</div>" +
        /* Earned marketer commission */
        '<div class="sc-lb-commission-cell" style="text-align:center;">' +
        (commDisplay
          ? '<span style="font-size:11px;font-weight:var(--weight-bold);color:#a855f7">' +
            commDisplay + window.supposedBadgeHtml('commission') +
            "</span>"
          : '<span style="font-size:10px;font-weight:var(--weight-bold);color:rgba(255,255,255,0.18)">â€”</span>') +
        "</div>" +
        "</div>"
      );
    }).join("");
    ...
    return (
      ...
      '<span id="sc-lb-title" style="font-size:15px;font-weight:var(--weight-semibold);color:#fff" dir="' +
      (isAr ? "rtl" : "ltr") +
      '">' +
      s6Txt("City Leaderboard", "ØªØ±ØªÙŠØ¨ Ø§Ù„Ù…Ø¯Ù†") +
      ...
      "padding:8px 12px;font-size:10px;font-weight:var(--weight-bold);color:rgba(255,255,255,0.3);" +
      "border-bottom:1px solid rgba(255,255,255,0.05);gap:8px;text-align:" +
      alignStr +
      '">' +
      "<div>" +
      s6Txt("City", "Ø§Ù„Ù…Ø¯ÙŠÙ†Ø©") +
      "</div>" +
      '<div class="sc-lb-orders-hdr cool-tooltip" data-tooltip="' +
      s6Txt("Net order count", "Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„ØµØ§ÙÙŠØ©") +
      '">' +
      s6Txt("Net Orders", "Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„ØµØ§ÙÙŠØ©") +
      "</div>" +
      '<div class="cool-tooltip" data-tooltip="' +
      s6Txt(
        "Number of successfully delivered orders",
        "Ø¹Ø¯Ø¯ Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„Ù…ÙˆØµÙ„Ø© Ø¨Ù†Ø¬Ø§Ø­",
      ) +
      '">' +
      s6Txt("Delivered", "Ù…ÙˆØµÙ‘Ù„") +
      "</div>" +
      '<div class="cool-tooltip" data-tooltip="' +
      s6Txt("Confirmed orders in this city", "Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„Ù…Ø¤ÙƒØ¯Ø© ÙÙŠ Ù‡Ø°Ù‡ Ø§Ù„Ù…Ø¯ÙŠÙ†Ø©") +
      '">' +
      s6Txt("Confirmed", "Ù…Ø¤ÙƒØ¯") +
      "</div>" +
      '<div style="text-align:center" class="cool-tooltip" data-tooltip="' +
      s6Txt("Confirmation rate: confirmed orders divided by net orders", "Ù†Ø³Ø¨Ø© Ø§Ù„ØªØ£ÙƒÙŠØ¯: Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„Ù…Ø¤ÙƒØ¯Ø© Ù…Ù‚Ø³ÙˆÙ…Ø© Ø¹Ù„Ù‰ ØµØ§ÙÙŠ Ø§Ù„Ø·Ù„Ø¨Ø§Øª") +
      '">CR</div>' +
      '<div class="cool-tooltip" data-tooltip="' +
      s6Txt(
        "Delivery rate from active orders only",
        "Ù†Ø³Ø¨Ø© Ø§Ù„ØªØ³Ù„ÙŠÙ… Ù…Ù† Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„Ù†Ø´Ø·Ø© ÙÙ‚Ø·",
      ) +
      '">' +
      s6Txt("DR% (Active)", "DR% (Ù†Ø´Ø·)") +
      "</div>" +
      '<div style="text-align:center" class="sc-lb-ndr-hdr cool-tooltip" data-tooltip="' +
      s6Txt(
        "Delivery rate from total orders (NDR)",
        "Ù†Ø³Ø¨Ø© Ø§Ù„ØªØ³Ù„ÙŠÙ… Ù…Ù† Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ø·Ù„Ø¨Ø§Øª (NDR)",
      ) +
      '">NDR%</div>' +
      '<div style="text-align:center" class="cool-tooltip" data-tooltip="' +
      // The commission DOM key remains for compatibility with existing sort code.
      s6Txt("Sum of marketer commission for delivered orders", "Ù…Ø¬Ù…ÙˆØ¹ Ø¹Ù…ÙˆÙ„Ø© Ø§Ù„Ù…Ø³ÙˆÙ‚ Ù„Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„Ù…Ø³Ù„Ù…Ø©") +
      '">' +
      s6Txt("Earned marketer commission", "Ø§Ù„Ø¹Ù…ÙˆÙ„Ø© Ø§Ù„Ù…Ø­Ù‚Ù‚Ø©") +
      "</div>" +
      "</div>" +
  ...
  // â”€â”€ Province cards HTML â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function provinceCardsHTML() {
    var cards = PROVINCES.map(function (p, i) {
      ...
      return (
        ...
        '<span style="font-size:14px;font-weight:var(--weight-bold);color:' +
        p.color +
        '">' +
        shortProvName(p.name) +
        "</span>" +
        ...
        s6Txt(
          ...
          "Ù…Ø¹Ø¯Ù„ Ù†Ù…Ùˆ Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ù…Ù‚Ø§Ø±Ù†Ø© Ø¨Ø§Ù„Ø´Ù‡Ø± Ø§Ù„Ù…Ø§Ø¶ÙŠ",
        ) +
        '" style="display:flex;align-items:center;gap:4px;font-size:12px;font-weight:var(--weight-semibold);' +
        "color:" +
        ...
        "</div>" +
        '<div style="width:100%">' +
        '<div style="font-size:28px;font-weight:var(--weight-bold);color:#fff;line-height:1">' +
        p.orders.toLocaleString("en-US") +
        "</div>" +
        '<div style="font-size:12px;font-weight:600;color:rgba(255,255,255,0.4);margin-top:6px">' +
        s6Txt("Net Orders Â· ", "ØµØ§ÙÙŠ Ø§Ù„Ø·Ù„Ø¨Ø§Øª Â· ") +
        p.cities.length +
        s6Txt(" cities", " Ù…Ø¯Ù†") +
        "</div>" +
        "</div>" +
        ...
        s6Txt("NDR (Total)", "NDR (Ø¥Ø¬Ù…Ø§Ù„ÙŠ)") +
        "</span>" +
        '<span style="font-size:14px;font-weight:var(--weight-semibold);color:' +
        p.color +
        '">' +
        ...
        '">' +
        '<span style="font-size:12px;font-weight:600;color:rgba(255,255,255,0.45)">' +
        s6Txt("DR (Active)", "DR (Ø§Ù„Ù†Ø´Ø·Ø©)") +
        "</span>" +
        '<span style="font-size:14px;font-weight:var(--weight-semibold);color:' +
        p.color +
        '">' +
        ...
        s6Txt(
          ...
          "Ù†Ø³Ø¨Ø© Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø¨Ø§Ù„Ø¯ÙØ¹ Ø¹Ù†Ø¯ Ø§Ù„Ø§Ø³ØªÙ„Ø§Ù… (COD)",
        ) +
        '">' +
        ...
        '<div style="font-size:14px;font-weight:var(--weight-semibold);color:#06b6d4;text-align:center;width:100%;">' +
        formatPercent(p.codRate) +
        "%</div>" +
        "</div>" +
        '<div style="display:flex;flex-direction:column;align-items:center;text-align:center;" class="cool-tooltip" data-tooltip="' +
        s6Txt("Percentage of returned orders", "Ù†Ø³Ø¨Ø© Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„Ù…Ø±ØªØ¬Ø¹Ø©") +
        '">' +
        '<div style="font-size:11px;color:rgba(255,255,255,0.35);font-weight:600;margin-bottom:4px;text-align:center;width:100%">' +
        s6Txt("Returns", "Ù…Ø±ØªØ¬Ø¹Ø§Øª") +
        "</div>" +
        '<div style="font-size:14px;font-weight:var(--weight-semibold);color:#ef4444;text-align:center;width:100%;">' +
        ...
        s6Txt(
          ...
          "Ù…ØªÙˆØ³Ø· Ø£ÙŠØ§Ù… Ø§Ù„ØªÙˆØµÙŠÙ„ (Ù…Ù† Ø¥Ù†Ø´Ø§Ø¡ Ø§Ù„Ø·Ù„Ø¨ Ø­ØªÙ‰ Ø¢Ø®Ø± ØªØ­Ø¯ÙŠØ« Ù„Ù„ØªØ³Ù„ÙŠÙ…)",
        ) +
        '">' +
        ...
        s6Txt("days", "Ø£ÙŠØ§Ù…") +
        "</div>" +
        '<div style="font-size:14px;font-weight:var(--weight-semibold);color:rgba(255,255,255,0.75);text-align:center;width:100%;">' +
        (p.avgDays == null ? "â€”" : p.avgDays) +
        "</div>" +
        "</div>" +
        "</div>" +
        "</button>"
    ...
    return (
      ...
      '<div style="font-size:10px;font-weight:var(--weight-bold);color:rgba(255,255,255,0.28);' +
      'letter-spacing:3px;text-align:right;margin-bottom:10px">' +
      s6Txt("Administrative Regions", "Ø§Ù„Ù…Ù†Ø§Ø·Ù‚ Ø§Ù„Ø¥Ø¯Ø§Ø±ÙŠØ©") +
      "</div>" +
  ...
  // â”€â”€ Time-filter tabs HTML â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  var TIME_TABS = [
    s6Txt("This Month", "Ù‡Ø°Ø§ Ø§Ù„Ø´Ù‡Ø±"),
    s6Txt("Last 7 Days", "Ø¢Ø®Ø± 7 Ø£ÙŠØ§Ù…"),
    s6Txt("This Quarter", "Ù‡Ø°Ø§ Ø§Ù„Ø±Ø¨Ø¹"),
  ];
    ...
    return (
      ...
      TIME_TABS.map(function (t, i) {
        ...
        return (
          ...
          i +
          '" style="' +
          "padding:7px 14px;border-radius:9px;border:none;font-size:11px;font-weight:var(--weight-bold);" +
          "cursor:pointer;font-family:inherit;transition:all 0.15s;" +
          "background:" +
  ...
  // â”€â”€ Assemble full section â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  var isAr = window.dashboardI18n
    ? window.dashboardI18n.currentLocale === "ar"
    : false;
  var dirStr = isAr ? "rtl" : "ltr";
  ...
  mountEl.innerHTML =
    ...
    "<div>" +
    '<h1 class="fade-up" style="font-size:30px;font-weight:var(--weight-bold);color:#fff;margin:0;text-align:' +
    alignStr +
    ';display:inline-flex;align-items:center;gap:12px;">' +
    s6Txt("Cities & Regions", "Ø§Ù„Ù…Ø¯Ù† ÙˆØ§Ù„Ù…Ù†Ø§Ø·Ù‚") +
    ...
    s6Txt("â³ Updating...", "â³ Ø¬Ø§Ø±ÙŠ Ø§Ù„ØªØ­Ø¯ÙŠØ«...") +
    '</span>' +
    "</h1>" +
    ...
    s6Txt(
      ...
      ALL_CITIES.length + " Ù…Ø¯ÙŠÙ†Ø© ÙÙŠ " + PROVINCES.length + " Ù…Ù†Ø§Ø·Ù‚ Ø¥Ø¯Ø§Ø±ÙŠØ©",
    ) +
    "</p>" +
    "</div>" +
    "</div>" +
    ...
    provinceCardsHTML() +
    "</div>" +
    /* T-23: Product Ã— City Matrix mount point */
  ...
  // â”€â”€ Post-mount: animate bars â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  scheduleCitiesTimeout(function () {
  ...
  // â”€â”€ Province selection logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function updateSelection() {
    ...
    /* leaderboard rows â€” let applyFilters handle visibility (it respects all filters) */
    ...
    if (titleEl) {
      var provLabel = sel
        ? (
          ...
          ).name || s6Txt("Region Cities", "Ù…Ø¯Ù† Ø§Ù„Ù…Ù†Ø·Ù‚Ø©")
        : s6Txt("City Leaderboard", "ØªØ±ØªÙŠØ¨ Ø§Ù„Ù…Ø¯Ù†");
      titleEl.innerHTML =
        provLabel +
        ' <span style="color:rgba(255,255,255,0.3);font-weight:600;font-size:13px">(' +
  ...
  function showCityTooltip(el) {
    ...
    tooltipFrame = scheduleCitiesFrame(function () {
      tooltipFrame = 0;
      var cx = parseFloat(el.dataset.cx);
  ...
  /* â”€â”€ T-15: Heatmap mode pills â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  ...
  function applyHeatMode(mode) {
    ...
    scheduleCitiesFrame(function () {
      ...
      dots.forEach(function (el) {
        ...
        var color;
        if (mode === "ndr") {
          /* NDR: green (safe) â†’ red (danger) */
          color = rateColor(v);
        } else {
          /* Revenue: purple (low) â†’ teal (high) */
          ...
          descEl.innerText =
            typeof s6Txt === "function"
              ? s6Txt(
                  ...
                  "ØªØ´ÙŠØ± Ø§Ù„Ø£Ù„ÙˆØ§Ù† Ø¥Ù„Ù‰ Ø£Ø¯Ø§Ø¡ Ù…Ø¹Ø¯Ù„ Ø¹Ø¯Ù… Ø§Ù„ØªØ³Ù„ÙŠÙ…: Ù…Ù† Ø§Ù„Ø£Ø®Ø¶Ø± (Ø¬ÙŠØ¯) Ø¥Ù„Ù‰ Ø§Ù„Ø£Ø­Ù…Ø± (Ø­Ø±Ø¬).",
                )
              : "Colors indicate NDR performance: Cyan (>=40%), green (>=30%), amber (>=20%), red (<20%).";
          ...
          descEl.innerText =
            typeof s6Txt === "function"
              ? s6Txt(
                  ...
                  "ØªØ¨Ø±Ø² Ø§Ù„Ø£Ù„ÙˆØ§Ù† ØªÙˆÙ„ÙŠØ¯ Ø§Ù„Ø¥ÙŠØ±Ø§Ø¯Ø§Øª: Ù…Ù† Ø§Ù„ØªØ±ÙƒÙˆØ§Ø²ÙŠ (Ù…Ø±ØªÙØ¹) Ø¥Ù„Ù‰ Ø§Ù„Ø£Ø±Ø¬ÙˆØ§Ù†ÙŠ (Ù…Ù†Ø®ÙØ¶).",
                )
              : "Colors highlight revenue generation: Teal (High) to Purple (Low).";
        } else {
          descEl.innerText =
  ...
  /* Phase 5: Virtual scroll â€” lazy-reveal city rows via IntersectionObserver */
  (function initVirtualScroll() {
    ...
    /* Mark rows beyond the first 15 as pending reveal â€” use a data attribute
    ...
    if ("IntersectionObserver" in window) {
      ...
      mountEl._citiesVirtualObserver = observer;
    } else {
  ...
  (function bindResponsiveCityLeaderboardPagination() {
    var resizeHandler = function () {
      window.clearTimeout(mountEl._citiesLeaderboardResizeTimer);
      mountEl._citiesLeaderboardResizeTimer = scheduleCitiesTimeout(function () {
        mountEl._citiesLeaderboardResizeTimer = null;
    ...
    if (sectionResources) sectionResources.listen(window, "resize", resizeHandler);
    else window.addEventListener("resize", resizeHandler);
    mountEl._citiesLeaderboardResizeActive = true;
  })();

  function parseLeaderboardNumber(value) {
    var raw = String(value == null ? "" : value).trim();
    if (!raw || raw === "-" || raw === "â€”") return -Infinity;
  ...
  /* (CSS transition on .sc-dot-inner is in dashboard-styles.css â€” ensures smooth fill changes) */
  ...
  (function wireFilterBar() {
    ...
    function enhanceSelect(selectEl, activeColor, withSearch) {
      ...
      if (sectionResources) sectionResources.listen(document, "click", onCustomSelectDocumentClick);
      else document.addEventListener("click", onCustomSelectDocumentClick);
    ...
    /* Normalize Arabic + English for search:
       ...
       - lowercases English
       - strips leading "Ù…Ù†Ø·Ù‚Ø© " / "Ø§Ù„Ù…Ù†Ø·Ù‚Ø© " prefixes so "Ø±ÙŠØ§Ø¶" matches "Ù…Ù†Ø·Ù‚Ø© Ø§Ù„Ø±ÙŠØ§Ø¶"
    */
    function normalizeSearch(str) {
      return str
        .replace(/[\u064B-\u065F\u0670]/g, "") /* strip diacritics */
        .replace(/[Ø£Ø¥Ø¢Ø§]/g, "Ø§") /* normalize alef */
        .replace(/Ø©/g, "Ù‡") /* ta marbuta */
        .replace(/Ù‰/g, "ÙŠ") /* alef maqsura */
        .replace(
          /^(Ù…Ù†Ø·Ù‚Ù‡|Ø§Ù„Ù…Ù†Ø·Ù‚Ù‡|Ù…Ù†Ø·Ù‚Ø©|Ø§Ù„Ù…Ù†Ø·Ù‚Ø©)\s+/g,
          "",
        ) /* strip region prefix */
    ...
    /* Aliases: each group = set of strings that all mean the same place.
       The real data uses PROVINCE names like "Ù…Ù†Ø·Ù‚Ø© Ø§Ù„Ø±ÙŠØ§Ø¶", "Ø§Ù„Ù…Ù†Ø·Ù‚Ø© Ø§Ù„Ø´Ø±Ù‚ÙŠØ©",
       "Ø§Ù„Ù…Ù†Ø·Ù‚Ø© Ø§Ù„ØºØ±Ø¨ÙŠØ© (Ù…ÙƒØ© Ø§Ù„Ù…ÙƒØ±Ù…Ø©)", "Ø¬ÙŠØ²Ø§Ù†", "Ø­Ø§Ø¦Ù„", etc.
       After normalizeSearch strips "Ù…Ù†Ø·Ù‚Ø©/Ø§Ù„Ù…Ù†Ø·Ù‚Ø©" prefix, the haystack becomes
       e.g. "Ø§Ù„Ø±ÙŠØ§Ø¶", "Ø§Ù„Ø´Ø±Ù‚ÙŠÙ‡", "Ø§Ù„ØºØ±Ø¨ÙŠÙ‡", "Ø¬ÙŠØ²Ø§Ù†", "Ø­Ø§Ø¦Ù„".
       Aliases let English terms map to those normalised Arabic strings. */
    var SEARCH_ALIASES = [
      /* Riyadh */
      ["riyadh", "riyad", "Ø§Ù„Ø±ÙŠØ§Ø¶", "Ø±ÙŠØ§Ø¶"],
      /* Makkah / Western */
      [
        "makkah",
        "mecca",
      ...
      /* Asir */
      ["asir", "aseer", "abha", "khamis", "Ø¹Ø³ÙŠØ±", "Ø£Ø¨Ù‡Ø§", "Ø§Ø¨Ù‡Ø§", "Ø®Ù…ÙŠØ³"],
      /* Madinah */
      [
        "madinah",
        "medina",
      ...
      /* Qassim */
      ["qassim", "qasim", "buraidah", "Ø§Ù„Ù‚ØµÙŠÙ…", "Ø¨Ø±ÙŠØ¯Ù‡", "Ø¨Ø±ÙŠØ¯Ø©"],
      /* Tabuk */
      ["tabuk", "ØªØ¨ÙˆÙƒ"],
      /* Hail */
      ["hail", "hael", "Ø­Ø§Ø¦Ù„", "Ø­Ø§ÙŠÙ„"],
      /* Jazan */
      ["jazan", "jizan", "gizan", "Ø¬Ø§Ø²Ø§Ù†", "Ø¬ÙŠØ²Ø§Ù†"],
      /* Baha */
      ["baha", "Ø§Ù„Ø¨Ø§Ø­Ù‡", "Ø§Ù„Ø¨Ø§Ø­Ø©", "Ø¨Ø§Ø­Ù‡", "Ø¨Ø§Ø­Ø©"],
      /* Najran */
      ["najran", "Ù†Ø¬Ø±Ø§Ù†"],
      /* Jawf */
      ["jawf", "jouf", "Ø§Ù„Ø¬ÙˆÙ"],
      /* Northern */
      ["northern", "arar", "Ø§Ù„Ø´Ù…Ø§Ù„ÙŠÙ‡", "Ø§Ù„Ø´Ù…Ø§Ù„ÙŠØ©", "Ø¹Ø±Ø¹Ø±"],
    ];
    ...
    function applyFilters() {
      ...
      /* â”€â”€ DEBUG: product filter diagnostics â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
      if (false && productVal) {
        console.log(
      ...
      mountEl.querySelectorAll(".sc-lb-row").forEach(function (el) {
        ...
        if (productVal) {
          if (!geoMap) {
            prodOk = true; /* geoMap not ready â€” fail open */
          } else {
            /* geoMap is keyed by city name (same as data-city / cityStats key).
               ...
               that may have parenthetical suffixes (e.g. 'Ø§Ù„Ø±ÙŠØ§Ø¶ ( Ø§Ù„Ø±ÙŠØ§Ø¶ )'). */
      ...
      if (false && productVal) {
        console.log(
          "[Cities][applyFilters] result â€” shown:",
          shownCount,
          "hidden:",
          hiddenCount,
        );
        if (shownCount === 0 && hiddenCount === 0) {
          console.warn(
            "[Cities][applyFilters] NO .sc-lb-row elements found in mountEl â€” leaderboard may not be rendered yet or selector is wrong",
          );
          /* Try broader search */
    ...
    function applyProductView(productKey) {
      ...
      mountEl.querySelectorAll(".sc-lb-row").forEach(function (row) {
        ...
        if (!productKey || !geoMap) {
          ...
          if (ndrCell) {
            ...
            ndrCell.innerHTML = nc0
              ? '<span style="font-size:11px;font-weight:var(--weight-semibold);color:' +
                ...
                pctValue(ndrNum) +
                "%</span>"
              : '<span style="font-size:11px;font-weight:var(--weight-semibold);color:rgba(255,255,255,0.25);background:rgba(255,255,255,0.05);padding:2px 6px;border-radius:6px;">&mdash;</span>';
          }
          ...
          if (deliveredCellR) {
            ...
            deliveredCellR.textContent = (delivRaw0 !== '' && delivRaw0 !== undefined)
              ? parseInt(delivRaw0, 10).toLocaleString('en-US') : 'â€”';
          }
          row.style.display = "grid";
          row.style.opacity = "1";
          return;
        ...
        /* Delivered cell â€” update to product-specific delivered count */
        ...
        if (deliveredCell && cell) {
          ...
          deliveredCell.textContent = prodDelivered != null ? prodDelivered.toLocaleString('en-US') : 'â€”';
        }
      });
    }
    ...
    syncLeaderboardProductFilter = function (state) {
      ...
      if (false) console.log(
        "[Cities][syncLeaderboardProductFilter] called â€” productKey:",
      ...
      if (changed) {
        ...
        /* Verify the setter actually updated â€” the option must exist in the <select> */
        if (false && fbProduct.value !== productKey) {
          console.warn(
            "[Cities][syncLeaderboardProductFilter] VALUE MISMATCH after set! fbProduct.value=",
            JSON.stringify(fbProduct.value),
            "â€” productKey not found as an <option> value. Available options:",
    ...
    if (fbSearch) {
      fbSearch.addEventListener("input", function () {
        ...
        citiesSearchTimer = scheduleCitiesTimeout(function () {
          citiesSearchTimer = null;
          if (mountEl.isConnected && !mountEl.hidden) applyFilters();
        }, 120);
      });
    ...
    /* Payment pills â€” switch displayed Orders and NDR columns per payment type */
    function applyPaymentView(payVal) {
      ...
      if (ordersHdr) {
        ordersHdr.textContent =
          payVal === "cod"
            ? s6Txt("COD Net Orders", "Ø·Ù„Ø¨Ø§Øª COD Ø§Ù„ØµØ§ÙÙŠØ©")
            : payVal === "prepaid"
              ? s6Txt("Prepaid Net Orders", "Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„ØµØ§ÙÙŠØ© Ù…Ø³Ø¨Ù‚Ø© Ø§Ù„Ø¯ÙØ¹")
              : s6Txt("Net Orders", "Ø§Ù„Ø·Ù„Ø¨Ø§Øª Ø§Ù„ØµØ§ÙÙŠØ©");
      }
      ...
      mountEl.querySelectorAll(".sc-lb-row").forEach(function (row) {
        ...
        if (ndrCell) {
          ...
          if (ndrNum === null) {
            ndrCell.innerHTML =
              '<span style="font-size:11px;font-weight:var(--weight-semibold);color:rgba(255,255,255,0.25);background:rgba(255,255,255,0.05);padding:2px 6px;border-radius:6px;">â€”</span>';
          } else {
            var nc =
              rateColor(ndrNum);
            ndrCell.innerHTML =
    ...
    refreshResetButtonState = function () {
      ...
      fbReset.title = active
        ? s6Txt("Reset active city analysis filters", "Ø¥Ø¹Ø§Ø¯Ø© Ø¶Ø¨Ø· ÙÙ„Ø§ØªØ± ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ù…Ø¯Ù† Ø§Ù„Ù†Ø´Ø·Ø©")
        : s6Txt("No active city analysis filters", "Ù„Ø§ ØªÙˆØ¬Ø¯ ÙÙ„Ø§ØªØ± Ù†Ø´Ø·Ø© Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ù…Ø¯Ù†");
    };
    refreshResetButtonState();
    ...
    if (fbExcludeBtn) {
      ...
      fbExcludeBtn.addEventListener("click", function () {
        ...
        openExclusionModal(lowNdrCities, s6Txt('Cities with Net Delivery Rate (NDR) below 40%', 'Ø§Ù„Ù…Ø¯Ù† Ø§Ù„ØªÙŠ ÙŠÙ‚Ù„ ÙÙŠÙ‡Ø§ Ù…Ø¹Ø¯Ù„ Ø§Ù„ØªØ³Ù„ÙŠÙ… Ø§Ù„ØµØ§ÙÙŠ (NDR) Ø¹Ù† 40%'));
      });
    }
  ...
  /* Phase 5: Mount Product Ã— City Matrix */
  function mountCitiesSecondaryWidgets() {
    ...
    if (!mountEl._citiesInsightStripMounted && insightMount && typeof window.renderInsightStrip === "function") {
      mountEl._citiesInsightStripMounted = true;
    ...
    if (!matrixMount || typeof window.renderProductMatrix !== "function" || mountEl._citiesProductMatrixLifecycle) {
      mountEl._citiesSecondaryMounted = !!mountEl._citiesProductMatrixLifecycle;
      return;
    }

    function startMatrix() {
      if (!mountEl.isConnected || mountEl.hidden || mountEl._citiesProductMatrixLifecycle) return;
      if (mountEl._citiesSecondaryVisibilityObserver) {
        mountEl._citiesSecondaryVisibilityObserver.disconnect();
        mountEl._citiesSecondaryVisibilityObserver = null;
      }
      ...
      if (window.DashboardTrace) {
        window.DashboardTrace.emit("cities", "product-matrix:mount", { mode: "visible-or-near" });
      }
      mountEl._citiesProductMatrixLifecycle = window.renderProductMatrix(
        matrixMount,
        window.dashboardGeoData,
        window.DashboardFilterBus,
      );
    ...
    var rect = matrixMount.getBoundingClientRect ? matrixMount.getBoundingClientRect() : null;
    var nearViewport = rect
      ? rect.top < ((window.innerHeight || 900) + 360) && rect.bottom > -120
      : true;
    ...
    if (!mountEl._citiesSecondaryVisibilityObserver) {
      if (window.DashboardTrace) {
        window.DashboardTrace.emit("cities", "product-matrix:deferred-until-visible", {});
      }
      mountEl._citiesSecondaryVisibilityObserver = new IntersectionObserver(function (entries) {
        var visible = entries.some(function (entry) { return entry.isIntersecting; });
        if (visible) startMatrix();
      }, { root: null, rootMargin: "360px 0px", threshold: 0.01 });
      mountEl._citiesSecondaryVisibilityObserver.observe(matrixMount);
    }
  }

  mountEl._citiesInsightStripMounted = false;
  mountEl._citiesSecondaryMounted = false;
  mountEl._citiesSecondaryIdleHandle = whenCitiesIdle(mountCitiesSecondaryWidgets, 1000);

  /* â”€â”€ T-22: Product Focus Mode â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  ...
  /* â”€â”€ T-22: Subscribe to FilterBus for cross-section product focus â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  ...
  if (window.DashboardFilterBus) {
    ...
    if (mountEl.parentNode) {
      if (sectionResources) sectionResources.observe(_filterBusObserver, mountEl.parentNode, { childList: true });
      else _filterBusObserver.observe(mountEl.parentNode, { childList: true });
    }
  ...
  var citiesLifecycleActive = true;
  function stopCitiesHiddenWork() {
    if (window.DashboardFilterBus && mountEl._onCitiesFilterBusChange) {
      window.DashboardFilterBus.unsubscribe(mountEl._onCitiesFilterBusChange);
    }
    cancelCitiesIdle(mountEl._citiesSecondaryIdleHandle);
    ...
    if (mountEl._citiesProductMatrixLifecycle &&
        typeof mountEl._citiesProductMatrixLifecycle.deactivate === "function") {
      mountEl._citiesProductMatrixLifecycle.deactivate();
    }
  }

  mountEl._dashboardSectionDeactivate = function () {
    if (!citiesLifecycleActive) return;
    citiesLifecycleActive = false;
    stopCitiesHiddenWork();
  };
  mountEl._dashboardSectionActivate = function () {
    if (citiesLifecycleActive) return;
    citiesLifecycleActive = true;
    if (window.DashboardFilterBus && mountEl._onCitiesFilterBusChange) {
      window.DashboardFilterBus.subscribe(mountEl._onCitiesFilterBusChange);
      mountEl._onCitiesFilterBusChange(window.DashboardFilterBus.getState());
    }
    if (mountEl._citiesProductMatrixLifecycle &&
        typeof mountEl._citiesProductMatrixLifecycle.activate === "function") {
      mountEl._citiesProductMatrixLifecycle.activate();
    }
    if (!mountEl._citiesSecondaryMounted) {
      mountEl._citiesSecondaryIdleHandle = whenCitiesIdle(mountCitiesSecondaryWidgets, 500);
    }
  };
  return function () {
    mountEl._dashboardSectionDeactivate();
    if (mountEl._citiesProductMatrixLifecycle &&
        typeof mountEl._citiesProductMatrixLifecycle.destroy === "function") {
      mountEl._citiesProductMatrixLifecycle.destroy();
      mountEl._citiesProductMatrixLifecycle = null;
    }
    if (window.DashboardFilterBus && mountEl._onCitiesFilterBusChange) {
      window.DashboardFilterBus.unsubscribe(mountEl._onCitiesFilterBusChange);
    }
