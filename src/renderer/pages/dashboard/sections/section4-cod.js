...
window.renderSection4 = function (mountEl, data, ctx) {
  ...
  if (tooltip) {
    ...
    const moveTooltip = () => {
      tooltipFrame = 0;
      tooltip.style.transform = `translate3d(${pointerX}px, ${pointerY - 15}px, 0) translate(-50%, -100%)`;
    };

    mountEl.querySelectorAll(".s4-city-dot").forEach((dot) => {
      dot.addEventListener("mouseenter", (e) => {
        mapRect = mapEl ? mapEl.getBoundingClientRect() : null;
        if (mapRect) {
          pointerX = e.clientX - mapRect.left;
          pointerY = e.clientY - mapRect.top;
          if (!tooltipFrame) tooltipFrame = requestAnimationFrame(moveTooltip);
        }
