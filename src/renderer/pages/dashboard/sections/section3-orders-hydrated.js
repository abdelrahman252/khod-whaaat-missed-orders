...
window.renderSection3HydratedEntry = function (mountEl, data, ctx) {
  ...
  // Certified status rows already come from the aggregator here. Reading only
  // data.stages made this section reconstruct them from every order again.
  var stages  = (data && data.pipeline && Array.isArray(data.pipeline.stages))
    ? data.pipeline.stages
    : ((data && data.stages) ? data.stages : STAGES_DEFAULT);
  ...
  var normalizeOrderForPreview = null;
  if (data && data.orders) {
    var dateCache = {};
    var phoneCache = {};
    ...
    normalizeOrderForPreview = normalizeOrder;
  ...
  var currentQueryFlags = window.DashboardQueryRuntime && typeof window.DashboardQueryRuntime.currentFlags === 'function'
    ? window.DashboardQueryRuntime.currentFlags()
    : null;
  var backendOrdersEnabled = !!(currentQueryFlags && currentQueryFlags.orders);
  var backendOrdersRequest = 0;
  var backendOrdersHasPage = false;
  ...
  if (window.DashboardQueryRuntime && typeof window.DashboardQueryRuntime.flags === 'function') {
    window.DashboardQueryRuntime.flags().then(function (flags) {
      ...
      if (backendOrdersEnabled && isActiveOrdersMount()) {
  ...
  var hasCertifiedStages = Array.isArray(stages) && stages.length > 0 && stages !== STAGES_DEFAULT;
  ...
  var productCityOptionsWarmTimer = null;
  var productCityOptionsWarmIdle = null;
  ...
  function exactStatusStages() {
    ...
    if (hasCertifiedStages) {
      exactStatusStagesCache = stages.map(function (stage) {
        var bucket = stage.exactBucket || stage.bucket || stage.id || 'other';
        var dashboardBucket = window.TaagerStatus.dashboardBucket(bucket);
        var isAggregateConfirmation = bucket === 'confirmed' && Number.isFinite(confirmationMetricCount());
        var count = isAggregateConfirmation ? confirmationMetricCount() : Number(stage.count || 0);
        var total = isAggregateConfirmation
          ? confirmationMetricTotal()
          : Number(certifiedMetrics && certifiedMetrics.totalOrders || orders.length || 0);
        var displayLabel = isAggregateConfirmation
          ? confirmationStageLabel()
          : (stage.label || window.TaagerStatus.display(bucket, { locale: isRtl ? 'ar' : 'en' }));
        return Object.assign({}, stage, {
          id: isAggregateConfirmation ? 'confirmation' : exactStatusId(bucket),
          exactBucket: bucket,
          label: displayLabel,
          shortLabel: stage.shortLabel || displayLabel,
          count: count,
          pct: stage.pct != null ? Number(stage.pct) : (total > 0 ? parseFloat(((count / total) * 100).toFixed(1)) : 0),
          color: stage.color || window.TaagerStatus.color(bucket),
          iconSvg: stage.iconSvg || statusIconSvg(bucket, dashboardBucket)
        });
      });
      return exactStatusStagesCache;
    }
  ...
  function scheduleProductCityOptionsWarm() {
    ...
    var run = function () {
      productCityOptionsWarmTimer = null;
      productCityOptionsWarmIdle = null;
      productCityOptionsWarmScheduled = false;
      if (!isActiveOrdersMount() || productCityOptionsReady) return;
    ...
    if (window.requestIdleCallback) {
      productCityOptionsWarmIdle = window.requestIdleCallback(run, { timeout: 900 });
    } else {
      productCityOptionsWarmTimer = window.setTimeout(run, 16);
    }
  }
  ...
  function buildInitialOrdersPreview() {
    if (activeStageId !== 'all' || searchTerm || productFilter || cityFilter || statusFilter || dateFilter ||
        sortVal !== 'date_desc' || currentPage !== 1 || !normalizeOrderForPreview || !Array.isArray(orders)) {
      return null;
    }
    // The initial table needs only one page. Normalizing and sorting the whole
    // order history here was the main Section 3 cold-click cost. Keep an exact
    // top-N while scanning raw rows, then normalize only the visible records.
    var top = [];
    var limit = Math.max(1, Number(perPage) || 10);
    orders.forEach(function (row, index) {
      var rawDate = row && (row.createdAt || row.date || row.dashboardDate) || '';
      var time = rawDate ? Date.parse(rawDate) : 0;
      if (!isFinite(time)) time = 0;
      var candidate = { row: row, time: time, index: index };
      var insertAt = top.length;
      while (insertAt > 0) {
        var previous = top[insertAt - 1];
        if (previous.time > time || (previous.time === time && previous.index < index)) break;
        insertAt--;
      }
      if (insertAt < limit) {
    ...
    return {
      rows: top.map(function (entry) { return normalizeOrderForPreview(entry.row); }),
      total: Number(certifiedMetrics && certifiedMetrics.totalOrders || orders.length || 0)
    };
  }
  /* ── 1. Pipeline Module ──────────────────────────────────────────────────── */
  ...
  function refreshDetails() {
    ...
    // Backend pagination should not be defeated by the summary cards. For an
    // unfiltered stage the aggregator already supplied exact counts/financials,
    // so avoid normalizing and reducing the complete order history here.
    var useCertifiedDetails = backendOrdersEnabled && isFullStage && hasCertifiedStages;
    var stageOrders = useCertifiedDetails ? [] : getFilteredOrders();
    var stageOrderCount = useCertifiedDetails
      ? Number(activeStageId === 'all'
        ? (certifiedMetrics && certifiedMetrics.totalOrders || orders.length || 0)
        : (activeStage.count || 0))
      : stageOrders.length;
    var certifiedAllSales = Number(overviewSummary.totalSales && (
      overviewSummary.totalSales.value != null ? overviewSummary.totalSales.value : overviewSummary.totalSales
    ) || 0);
    var certifiedAllProfit = hasCertifiedStages ? stages.reduce(function (sum, stage) {
      return sum + Number(stage.profitAfterTax != null ? stage.profitAfterTax : stage.commissionValue || 0);
    }, 0) : 0;
    var totalValue = useCertifiedDetails
      ? Number(activeStageId === 'all' ? certifiedAllSales : (activeStage.salesValue != null ? activeStage.salesValue : activeStage.salesSar || 0))
      : stageOrders.reduce(function(sum, o) { return sum + o.total; }, 0);
    var totalCommission = useCertifiedDetails
      ? Number(activeStageId === 'all' ? certifiedAllProfit : (activeStage.profitAfterTax != null ? activeStage.profitAfterTax : activeStage.commissionValue || 0))
      : stageOrders.reduce(function(sum, o) { return sum + o.commission; }, 0);
    var avgValue = stageOrderCount > 0 ? (totalValue / stageOrderCount) : 0;
    ...
    if (!useCertifiedDetails && avgDurationDays == null && (isDeliveredScope || isShippingScope)) {
    ...
    var kpisDynamic = [
      {
        ...
        value: totalValue,
        unit: activeCurrency,
        sub: (isRtl ? 'من ' : 'from ') + formatReadableNumber(stageOrderCount, 0) + (isRtl ? ' طلب في هذه المرحلة' : ' orders in this stage'),
        decimals: 0,
        positive: false,
        iconSvg: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="13" height="13"><polygon points="12 2 2 7 12 12 22 7 12 2"/><polyline points="2 17 12 22 22 17"/><polyline points="2 12 12 17 22 12"/></svg>'
      },
  ...
  function refreshTableContainer() {
    ...
    if (searchInput) {
      ...
      var refreshSearchResults = debounceFn(function() {
        refreshTableData();
        setTimeout(function () {
      ...
      searchInput.addEventListener('input', function(e) {
        ...
        currentPage = 1;
        invalidateFilteredCache();
        var tbody = mountEl.querySelector('.explorer-table tbody');
        if (tbody && searchTerm) tbody.innerHTML = '';
  ...
  async function refreshTableData(options) {
    ...
    if (!options.forceLocal && canUseBackendOrdersPage()) {
      ...
      if (requestId !== backendOrdersRequest || !isActiveOrdersMount()) return;
      setOrdersTableBusy(false, rowsEl);
      if (!backendPage || !backendPage.ok) backendPage = null;
      else backendOrdersHasPage = true;
    } else {
      setOrdersTableBusy(false, rowsEl);
    }
    var initialPreview = !backendPage && options.initialPreview ? buildInitialOrdersPreview() : null;
    var allFiltered = backendPage
      ? (backendPage.rows || []).map(normalizeBackendOrder)
      : (initialPreview ? initialPreview.rows : getFilteredAndSortedOrders());
    var totalFiltered = backendPage && backendPage.pagination
      ? Number(backendPage.pagination.total || 0)
      : (initialPreview ? initialPreview.total : allFiltered.length);
    ...
    var pageOrders = backendPage || initialPreview ? allFiltered : allFiltered.slice(start, end);
  ...
  function render() {
    ...
    // Render a complete usable section from the already-aggregated local rows.
    // Backend pagination refines the table after paint, but the user must not
    // wait on IPC to see the detail cards or first page of orders.
    var hydrationSeq = ++denseHydrationSeq;
    refreshPipeline();
    refreshTableContainer();
    refreshDetails();
    refreshTableData({ forceLocal: true, initialPreview: true });
  ...
  var ordersLifecycleActive = true;
  mountEl._dashboardSectionDeactivate = function () {
    if (!ordersLifecycleActive) return;
    ordersLifecycleActive = false;
    denseHydrationSeq += 1;
    productCityOptionsWarmScheduled = false;
    if (productCityOptionsWarmTimer != null) {
      window.clearTimeout(productCityOptionsWarmTimer);
      productCityOptionsWarmTimer = null;
    }
    if (productCityOptionsWarmIdle != null && window.cancelIdleCallback) {
      window.cancelIdleCallback(productCityOptionsWarmIdle);
      productCityOptionsWarmIdle = null;
    }
    if (mountEl._s3ThemeObserver) mountEl._s3ThemeObserver.disconnect();
  };
  mountEl._dashboardSectionActivate = function () {
    if (ordersLifecycleActive) return;
    ordersLifecycleActive = true;
    if (mountEl._s3ThemeObserver) {
      mountEl._s3ThemeObserver.observe(document.documentElement, { attributes: true, attributeFilter: ['data-theme'] });
    }
    if (!productCityOptionsReady) scheduleProductCityOptionsWarm();
    if (backendOrdersEnabled && !backendOrdersHasPage) refreshTableData({ preserveRows: true });
  };
