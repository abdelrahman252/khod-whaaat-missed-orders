(function () {
  ...
  // Preserve the precise legacy KOD anchors for the five primary Saudi hubs.
  // The aliases on each existing row remain intact; only coordinates change.
  const SAUDI_EXACT_POINTS = {
    riyadh: [230.6, 164.1],
    dammam: [294.7, 129.4],
    jeddah: [87.1, 232.2],
    makkah: [99.1, 233.5],
  ...
  CITY_POINTS.sa.forEach((entry) => {
    const aliases = entry[2] || [];
    Object.keys(SAUDI_EXACT_POINTS).some((alias) => {
      if (aliases.indexOf(alias) === -1) return false;
      entry[0] = SAUDI_EXACT_POINTS[alias][0];
      entry[1] = SAUDI_EXACT_POINTS[alias][1];
      return true;
    });
  });
  ...
  window.KhodCountryAtlas = COUNTRY_ATLAS;
  window.TaagerCountryAtlas = COUNTRY_ATLAS;
})();
