...
(function () {
  ...
  if (typeof window.KhodDashboardMotionDisabled === 'undefined') {
    window.KhodDashboardMotionDisabled = true;
  }

  function dashboardMotionDisabled() {
    return window.KhodDashboardMotionDisabled !== false;
  }

  window.disableDashboardChartMotion = function () {
    if (!dashboardMotionDisabled() || !window.Chart || window.Chart.__khodDashboardMotionDisabled) return;
    window.Chart.__khodDashboardMotionDisabled = true;
  ...
  // Heavy dashboard panes are cached instead of being destroyed on every
  // navigation. Keep their asynchronous work in one lifecycle-aware bag so a
  // hidden pane cannot continue polling, observing, or handling global events.
  (function installSectionResourceBag() {
    if (window.DashboardSectionResources) return;
    var bags = [];
    ...
    function create(owner) {
      ...
      function forget(list, value) {
        var index = list.indexOf(value);
        if (index >= 0) list.splice(index, 1);
      }
      function clearAsync() {
        timers.splice(0).forEach(function (entry) {
          if (entry.repeat) window.clearInterval(entry.id);
          else window.clearTimeout(entry.id);
        });
        rafIds.splice(0).forEach(function (id) { window.cancelAnimationFrame(id); });
        idleIds.splice(0).forEach(function (id) {
      ...
      function detach() {
        ...
        observers.forEach(function (entry) {
          if (entry.attached) entry.observer.disconnect();
          entry.attached = false;
        });
      }
      function attach() {
        ...
        observers.forEach(function (entry) {
          if (entry.attached || !entry.target) return;
          entry.observer.observe(entry.target, entry.options);
          entry.attached = true;
        });
      }
      ...
      var bag = {
        ...
        timeout: function (fn, delay) {
          ...
          var entry = { id: null, repeat: false };
        ...
        interval: function (fn, delay) {
          if (!active || destroyed) return null;
          var entry = { id: window.setInterval(function () {
            if (active && !destroyed) fn();
          }, delay || 0), repeat: true };
          timers.push(entry);
          return entry.id;
        ...
        idle: function (fn, options) {
          if (!active || destroyed) return null;
          var request = window.requestIdleCallback || function (callback) { return window.setTimeout(callback, 0); };
          var id = request(function (deadline) {
            forget(idleIds, id);
            if (active && !destroyed) fn(deadline);
          }, options || {});
          idleIds.push(id);
          return id;
        },
        listen: function (target, type, handler, options) {
          ...
          var entry = { target: target, type: type, handler: handler, options: options, attached: false };
        ...
        observe: function (observer, target, options) {
          if (!observer || typeof observer.observe !== 'function') return observer;
          var entry = { observer: observer, target: target, options: options || {}, attached: false };
          observers.push(entry);
          if (active && !destroyed && target) {
            observer.observe(target, entry.options);
            entry.attached = true;
          }
          return observer;
        },
        chart: function (chart) { if (chart) charts.push(chart); return chart; },
        cleanup: function (fn) { if (typeof fn === 'function') cleanupTasks.push(fn); return fn; },
        activate: function () { if (!destroyed) { active = true; attach(); } },
        deactivate: function () { if (!destroyed) { active = false; clearAsync(); detach(); } },
        destroy: function () {
          if (destroyed) return;
          bag.deactivate();
          destroyed = true;
          ...
          cleanupTasks.splice(0).forEach(function (fn) {
            try { fn(); } catch (err) { console.warn('[Dashboard] Resource cleanup failed:', err); }
          });
          removeBag(bag);
        },
        snapshot: function () {
          return {
            sectionId: owner && owner.dataset ? owner.dataset.sectionId || '' : '',
            active: active && !destroyed,
            timers: timers.length,
            rafIds: rafIds.length,
            idleIds: idleIds.length,
            listeners: listeners.filter(function (entry) { return entry.attached; }).length,
            observers: observers.filter(function (entry) { return entry.attached; }).length,
    ...
    window.DashboardSectionResources = {
      create: create,
      snapshot: function () { return bags.map(function (bag) { return bag.snapshot(); }); }
    };
  })();
  ...
  window.ICONS = {
    ...
    refreshCw:
      ...
      '<path d="M4 13a8 8 0 0 0 14.9 4M20 20v-5h-5"/></svg>',
    wallet:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
    ...
    sparkles:
      ...
      '<path d="M5 14l.8 1.8L8 17l-2.2.8L5 20l-.8-2.2L2 17l2.2-1.2L5 14z"/></svg>',
  };
  ...
  function gmvConvert(value, from, to) {
    ...
    if (window.KhodCurrency && typeof window.KhodCurrency.convert === 'function') {
      return window.KhodCurrency.convert(amount, source, target);
    }
    return amount;
  }
  ...
  function gmvStorageKey(accountId, country) {
    return 'khod_gmv_target_v1:' + encodeURIComponent(gmvAccountId(accountId)) + ':' + encodeURIComponent(String(country || 'mixed').toLowerCase());
  }
  ...
  window.animateNumber = function (el, to, opts) {
    ...
    var suppressMotion = dashboardMotionDisabled() ||
      (window.TaagerIsUiStabilizing && window.TaagerIsUiStabilizing()) ||
