...
(function () {
  ...
  function convertDashboardCurrency(value, from, to) {
  ...
  var _aggregatorCacheByHash = window.__dashboardAggregatorCache || new Map();
  window.__dashboardAggregatorCache = _aggregatorCacheByHash;
  ...
  window.runDashboardAggregator = function (callback) {
    ...
    Promise.all([
      ...
      var keyedCache = _aggregatorCacheByHash.get(rowHash);
      if (keyedCache && keyedCache.preview === !!previewActive) {
        _aggregatorCache = keyedCache.result;
        _aggregatorCacheAt = Date.now();
        _aggregatorCacheHash = rowHash;
        _aggregatorCachePreview = !!previewActive;
        callback(keyedCache.result);
        return;
      }
      ...
      _aggregatorCacheHash = rowHash;
      _aggregatorCachePreview = !!previewActive;
      if (_aggregatorCacheByHash.size > 12) _aggregatorCacheByHash.clear();
      _aggregatorCacheByHash.set(rowHash, { result: result, preview: !!previewActive, cachedAt: _aggregatorCacheAt });
      callback(result);
    }).catch(function (err) {
  ...
  function processSnapshotRows(rows, accountId, meta) {
    ...
    var placedCount = 0;
    var deliveredCount = 0, shippingCount = 0, failedCount = 0, realFailedCount = 0;
    ...
    rows.forEach(function (row) {
      ...
      if (cityName) {
        ...
        if (isDeliveredInPeriod) {
          ...
          cs.canceledCount++;
          if (rowIsRealFailed) cs.realFailedCount++;
        ...
        if (cityProductKey) {
          if (!cs.productMap[cityProductKey]) {
            cs.productMap[cityProductKey] = {
              ...
              confirmed: 0, shipping: 0, processing: 0, realFailed: 0,
              prepaidCount: 0, codCount: 0,
              prepaidNdrBaseOrders: 0, codNdrBaseOrders: 0,
          ...
          if (isDeliveredInPeriod) {
            ...
            if (rowIsRealFailed) cp.realFailed++;
            ...
            if (bucket === 'confirmed') cp.confirmed++;
            else if (bucket === 'shipping') cp.shipping++;
            else if (bucket === 'processing') cp.processing++;
          }
        }
      }
      ...
      if (productKey) {
        ...
        if (cityKey) {
          ...
          if (isDeliveredInPeriod) {
            ...
            pcm.canceled++;
            if (rowIsRealFailed) pcm.realFailed++;
          } else if (bucket !== 'delivered' && bucket !== 'failed' && inCreatedPeriod) {
            if (bucket === 'confirmed') pcm.confirmed++;
            else if (bucket === 'shipping') pcm.shipping++;
            else if (bucket === 'processing') pcm.processing++;
          }
        ...
        if (inCreatedPeriod || isDeliveredInPeriod) {
          ...
          if (cityKey) {
            ...
            if (inCreatedPeriod) {
              _qcEntry.count++;
              if (bucket === 'confirmed') _qcEntry.confirmed++;
              else if (bucket === 'shipping') _qcEntry.shipping++;
              else if (bucket === 'processing') _qcEntry.processing++;
              else if (bucket === 'failed' && rowIsRealFailed) _qcEntry.realFailed++;
            }
            if (isDeliveredInPeriod) {
              _qcEntry.delivered++;
            }
    ...
    var sortedCities = Object.keys(cityStats).map(function (name) {
      ...
      var drPctCity = stat.drBaseOrders > 0
        ...
        : 0;
      var confirmationPctCity = stat.count > 0
        ? parseFloat((((stat.confirmedCount + stat.shippingCount + stat.processingCount + stat.deliveredOrders + (stat.realFailedCount || 0)) / stat.count) * 100).toFixed(1))
        : 0;
      ...
      stat.confirmationPct = confirmationPctCity;
      ...
      return {
        ...
        confirmationPct: confirmationPctCity,
    ...
    var rankedProducts = Object.keys(productStats).map(function (key) {
      ...
      function confirmationRateFromCounts(confirmed, shipping, processing, delivered, realFailed, totalCount) {
        totalCount = Number(totalCount || 0);
        if (totalCount <= 0) return 0;
        return parseFloat((((Number(confirmed || 0) + Number(shipping || 0) + Number(processing || 0) + Number(delivered || 0) + Number(realFailed || 0)) / totalCount) * 100).toFixed(1));
      }
      ...
      var productCities = Object.keys(p.cityMap)
        .map(function (c) {
          ...
          var cityConfirmed = cm.confirmed || 0;
          var cityShipping = cm.shipping || 0;
          var cityProcessing = cm.processing || 0;
          var cityRealFailed = cm.realFailed || 0;
          return {
            name:      c,
            ...
            shipping: cityShipping,
            processing: cityProcessing,
            confirmationPct: confirmationRateFromCounts(cityConfirmed, cityShipping, cityProcessing, cityDelivered, cityRealFailed, cityOrders),
      ...
      var piecesBreakdown = Object.keys(p.piecesMap)
        .map(function (k) {
          ...
          var qtyConfirmed = typeof entry === 'object' ? entry.confirmed || 0 : 0;
          var qtyShipping = typeof entry === 'object' ? entry.shipping || 0 : 0;
          var qtyProcessing = typeof entry === 'object' ? entry.processing || 0 : 0;
          var qtyRealFailed = typeof entry === 'object' ? entry.realFailed || 0 : 0;
          return {
            qty: k,
            count: qtyCount,
            delivered: qtyDelivered,
            confirmed: qtyConfirmed,
            shipping: qtyShipping,
            processing: qtyProcessing,
            confirmationPct: confirmationRateFromCounts(qtyConfirmed, qtyShipping, qtyProcessing, qtyDelivered, qtyRealFailed, qtyCount),
            ...
            cities: Object.keys(p.quantityCityMap[qtyKey])
              .map(function (cityName) {
                ...
                return {
                  ...
                  confirmed: typeof entry === 'object' ? (entry.confirmed || 0) : 0,
                  shipping: typeof entry === 'object' ? (entry.shipping || 0) : 0,
                  processing: typeof entry === 'object' ? (entry.processing || 0) : 0,
                  confirmationPct: typeof entry === 'object'
                    ? confirmationRateFromCounts(entry.confirmed, entry.shipping, entry.processing, entry.delivered, entry.realFailed, entry.count)
                    : 0
                };
              })
              .sort(function (a, b) { return b.count - a.count; })
    ...
    var confirmationRatePct = placedCount > 0
      ? parseFloat((((confirmedCount + shippingCount + processingCount + deliveredCount + realFailedCount) / placedCount) * 100).toFixed(1))
      : 0;
    ...
    var roiCurrency = ['SAR', 'USD'].indexOf(roiCurrencyRaw) !== -1 ? roiCurrencyRaw : 'SAR';
    ...
    var deliveredSalesInRoiCurrency = convertDashboardCurrency(totalDeliveredSales, 'SAR', roiCurrency);
    var previousDeliveredSalesInRoiCurrency = convertDashboardCurrency(previousOverview.totalDeliveredSales, 'SAR', roiCurrency);
    ...
    return {
      ...
      overview: {
        ...
        confirmationRate:    { value: confirmationRatePct, delta: 0,                                                                    unit: '%',           color: 'purple' },
        ndrRate:             { value: ndrPct,              delta: 0,                                                                    unit: '%',           color: 'green'  },
        drRate:              { value: collectionRate,      delta: 0,                                                                    unit: '%',           color: 'blue'   },
      ...
      pipeline: {
        ...
        insights: null,
        stages: [
          stage('awaiting',    'بانتظار التأكيد', pendingCount,    placedCount, '#a855f7', 'معدل التأكيد',  confirmationRatePct),
      ...
      roi: {
        adSpend: roiAdSpend, currency: roiCurrency, sarRate: 3.75,
