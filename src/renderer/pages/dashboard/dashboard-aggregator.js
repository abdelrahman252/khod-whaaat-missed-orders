/*
   ...
   Loads Dashboard snapshots, preserves account identity, and converts raw dashboard
   rows into the section data consumed by dashboard-shell.js.

   FIXES (v2):
   - Sort: deliveredCount descending (b - a), commission as tiebreaker
   - Dashboard NDR: delivered / (created orders - canceled-status orders)
...
(function () {
  ...
  function getStatusBucket(status) {
    if (isEasyOrdersSourceStatus(status)) return 'other';
    if (window.KhodDashboardLogic) {
      return window.KhodDashboardLogic.dashboardBucket(status);
    }
    ...
    if (s === 'ملغى' || s === 'ملغي' || s === 'ملغاة') return 'canceled';
    if (s === 'failed' || s === 'مرتجع' || s === 'فشلت') return 'failed';
  ...
  function lastUpdatedDashboardDate(row) {
    return normalizeDateKey(row && (row.lastUpdatedAt || row.updatedAt || row.lastUpdated || row.updateDate));
  }

  function deliveredDateMode() {
    if (window.DashboardDeliveredDateState && typeof window.DashboardDeliveredDateState.get === 'function') {
      var mode = window.DashboardDeliveredDateState.get();
      return mode === 'expected' || mode === 'lastUpdated' ? mode : 'actual';
    }
    return 'actual';
  }

  function isDeliveredRowInPeriod(row, period, mode) {
    if (exactStatusBucket(row) !== 'delivered') return false;
    return isDateKeyInPeriod(deliveredDashboardDate(row, mode), period);
  }

  function deliveredDashboardDate(row, mode) {
    if (mode === 'lastUpdated') return lastUpdatedDashboardDate(row) || createdDashboardDate(row);
    return createdDashboardDate(row);
  }

  function rowDashboardDate(row, mode) {
    if (!row) return '';
    if (mode === 'lastUpdated' && exactStatusBucket(row) === 'delivered') return deliveredDashboardDate(row, mode);
    return createdDashboardDate(row);
  }
  ...
  function rowIdentity(row, fallbackIndex) {
    var orderKey = String(row && (row.khodOrderNumber || row.taagerOrderNumber || row.orderNumber || row.id || row.orderId || row.reference || '') || '').trim();
  ...
  function filterOutcomeOrders(rows, period, mode) {
    if (!period || !period.dateFrom || !period.dateTo) return rows.slice();
    return rows.filter(function (row) {
      return exactStatusBucket(row) === 'delivered'
  ...
  function accountLabel(acc, fallback) {
    ...
    return acc.memberName || acc.khodEmail || acc.email || acc.easyEmail || acc.easyStore || acc.storeName || acc.label || acc.name || fallback || dt('shell.account');
  }
  ...
  function dashboardMoneyValue(value) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.moneyValue === 'function') {
      var khodShared = window.KhodDashboardLogic.moneyValue(value);
      return khodShared == null ? 0 : khodShared;
    }
  ...
  function isEasyOrdersSourceStatus(value) {
    var s = String(value == null ? '' : value).trim().toLowerCase();
    return s === 'real' || s === 'missed';
  }

  function rawLifecycleStatus(row) {
    if (!row) return '';
    if (row.orderStatus != null && String(row.orderStatus).trim() !== '') return row.orderStatus;
    if (row.orderStatusBucket != null && String(row.orderStatusBucket).trim() !== '') return row.orderStatusBucket;
    if (row.status != null && !isEasyOrdersSourceStatus(row.status)) return row.status;
    return '';
  }

  function isLifecycleStatusBucket(bucket) {
    if (!bucket || bucket === 'other' || bucket === 'canceled_by_you') return false;
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.statusInfo === 'function') {
      return window.KhodDashboardLogic.statusInfo(bucket).bucket !== 'other';
    }
    return !isEasyOrdersSourceStatus(bucket);
  }

  function rowHasLifecycleStatus(row) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.hasKnownLifecycleStatus === 'function') {
      return window.KhodDashboardLogic.hasKnownLifecycleStatus(row);
    }
    if (!row || typeof row !== 'object') return false;
    if (row.khodStatusKnown === true || row.orderStatusKnown === true) {
      return isLifecycleStatusBucket(exactStatusBucket(row));
    }
    if (row.khodStatusKnown === false || row.orderStatusKnown === false) return false;
    var rawStatus = rawLifecycleStatus(row);
    if (!rawStatus) return false;
    var normalized = String(rawStatus).trim().toLowerCase();
    if (normalized === 'under processing' || normalized === 'processing' || normalized === 'pending khod sync') {
      return !!(row.khodOrderNumberKnown || row.khodOrderNumber || row.taagerOrderNumber);
    }
    return isLifecycleStatusBucket(exactStatusBucket(row));
  }

  function rowProductName(row) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.productName === 'function') {
      return window.KhodDashboardLogic.productName(row);
    }
    if (window.TaagerStatus && typeof window.TaagerStatus.productName === 'function') {
      return window.TaagerStatus.productName(row);
    }
  ...
  function rowTotalPrice(row) {
    if (!row) return 0;
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.revenueValue === 'function') {
      var khodRevenue = window.KhodDashboardLogic.revenueValue(row);
      if (khodRevenue > 0) return khodRevenue;
    }
    ...
    var keys = ['totalPrice', 'priceNoShipping', 'subtotal'];
  ...
  function rowAmountDue(row) {
    if (!row) return 0;
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.codValue === 'function') {
      var khodCod = window.KhodDashboardLogic.codValue(row);
      if (khodCod > 0) return khodCod;
    }
    var keys = ['amountDue', 'dashboardAmountDue', 'codValue', 'amountDueRaw'];
  ...
  function rowCommissionValue(row) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.commissionValue === 'function') {
      var khodCommission = window.KhodDashboardLogic.commissionValue(row);
      if (khodCommission > 0) return khodCommission;
  ...
  function orderOnlyKey(row, fallbackIndex) {
    ...
    var direct = String(row && (row.khodOrderNumber || row.taagerOrderNumber || row.orderNumber || row.id || row.orderId || row.reference || '') || '').trim();
  ...
  function orderLevelRows(rows, includeCanceled) {
    ...
    (Array.isArray(rows) ? rows : []).forEach(function (row, idx) {
      ...
      var rowSalesValue = rowTotalPrice(row);
      var rowCodValue = rowAmountDue(row);
      var rowCommission = rowCommissionValue(row);
      map[key].dashboardTotalPrice += rowSalesValue;
      map[key].totalPrice = map[key].dashboardTotalPrice;
      map[key].salesValue = map[key].dashboardTotalPrice;
      map[key].dashboardAmountDue = rowCodValue;
      map[key].codValue = rowCodValue;
      map[key].dashboardCommission = rowCommission;
      map[key].commissionValue = rowCommission;
      map[key].itemCount += 1;
  ...
  function overviewMetricSummary(rows, period, mode) {
    ...
    rows.forEach(function (row) {
      ...
      var hasLifecycleStatus = rowHasLifecycleStatus(row);
      var ndrEligible = hasLifecycleStatus && !isNdrCanceledBucket(exactBucket);
      var rowIsNetOrder = !isCanceledByYouBucket(exactBucket);
      ...
      if (isDeliveredInNdrCohort && ndrEligible) ndrDelivered++;
      ...
      if (inCreatedPeriod) {
        ...
        if (!rowIsNetOrder) summary.canceledByYouCount++;
      }
      ...
      if (inCreatedPeriod && rowIsNetOrder) {
        ...
        if (hasLifecycleStatus && !isConfirmedBaseExcludedBucket(exactBucket)) summary.confirmedCount++;
      }
      ...
      } else if (inCreatedPeriod && rowIsNetOrder && isLostBucket(exactBucket)) {
        summary.lostCommission += commissionVal;
      } else if (inCreatedPeriod && rowIsNetOrder && isIncomingBucket(exactBucket)) {
  ...
  function orderSourceRawValue(row) {
    var value = row && (
      ...
      (row.received_by != null ? row.received_by :
      (row['الطلب المستلم بواسطة'] != null ? row['الطلب المستلم بواسطة'] :
      (row['المصدر'] != null ? row['المصدر'] : ''))))))
    );
  ...
  function buildOrderSourceBreakdown(rows, meta, minSample) {
    ...
    rows.forEach(function (row, rowIndex) {
      ...
      var ndrEligible = !isNdrCanceledBucket(exactBucket);
      ...
      if (inCreatedPeriod) {
        ...
        if (rowIsNetOrder) {
          ...
          if (cityName) {
            ...
            if (!bucket._cities[cityName]._sets.orders) bucket._cities[cityName]._sets.orders = {};
            if (addOnce(bucket._cities[cityName]._sets.orders, sourceOrderKey)) bucket._cities[cityName].orders++;
          }
          var productName = String(row.products || row.productName || row.product || row.sku || '').trim();
          if (productName) {
            if (!bucket._products[productName]) bucket._products[productName] = { name: productName, orders: 0, delivered: 0, _sets: {} };
            if (!bucket._products[productName]._sets.orders) bucket._products[productName]._sets.orders = {};
            if (addOnce(bucket._products[productName]._sets.orders, sourceOrderKey)) bucket._products[productName].orders++;
          }
        }
      }

      if (ndrEligible && inNdrCohortPeriod && rowIsNetOrder && exactBucket === 'delivered') {
      ...
      if (isDeliveredRowInPeriod(row, period, mode) && rowIsNetOrder) {
        ...
        if (deliveredCityName && bucket._cities[deliveredCityName]) {
          if (!bucket._cities[deliveredCityName]._sets.delivered) bucket._cities[deliveredCityName]._sets.delivered = {};
          if (addOnce(bucket._cities[deliveredCityName]._sets.delivered, sourceOrderKey)) bucket._cities[deliveredCityName].delivered++;
        }
        var deliveredProductName = String(row.products || row.productName || row.product || row.sku || '').trim();
        if (deliveredProductName && bucket._products[deliveredProductName]) {
          if (!bucket._products[deliveredProductName]._sets.delivered) bucket._products[deliveredProductName]._sets.delivered = {};
          if (addOnce(bucket._products[deliveredProductName]._sets.delivered, sourceOrderKey)) bucket._products[deliveredProductName].delivered++;
        }
      }
    });
  ...
  function dashboardFinancials(input) {
    var core = window.KhodDashboardFinancialCore || window.TaagerDashboardFinancialCore;
    ...
    var actualCommission = Number(input.actualEarnedCommission != null
      ? input.actualEarnedCommission
      : input.actualEarnedProfitAfterTax) || 0;
    ...
    var averageCommission = actualDelivered > 0 ? actualCommission / actualDelivered : 0;
    var exact = netOrders * ndr;
    var commission = exact * averageCommission;
    ...
    return {
      ...
      breakEvenCpa: averageCommission * ndr,
      expectedDeliveriesExact: exact,
      expectedDeliveriesDisplay: Math.round(exact),
      expectedCommission: commission,
      expectedCommissionBeforeAdSpend: commission,
      expectedTotalProfitBeforeAdSpend: commission,
      expectedNetProfit: commission - spend,
      expectedDeliveredSales: sales,
      expectedDeliveredCpa: exact > 0 ? spend / exact : 0,
      expectedRoi: spend > 0 ? ((commission - spend) / spend) * 100 : 0,
      expectedCommissionRoas: spend > 0 ? commission / spend : 0,
      expectedProfitRoas: spend > 0 ? commission / spend : 0,
  ...
  function exactStatusInfo(bucket) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.statusInfo === 'function') {
      return window.KhodDashboardLogic.statusInfo(bucket);
    }
  ...
  function orderedStatusFlow() {
    if (window.KhodDashboardLogic && Array.isArray(window.KhodDashboardLogic.ordered)) {
      return window.KhodDashboardLogic.ordered.filter(function (meta) {
        return meta && isLifecycleStatusBucket(meta.bucket);
      });
    }
    return [
      { bucket: 'pending', order: 10, businessGroup: 'incoming', color: '#f59e0b' },
      { bucket: 'confirmed', order: 20, businessGroup: 'incoming', color: '#22c55e' },
      { bucket: 'processing', order: 30, businessGroup: 'incoming', color: '#3b82f6' },
      { bucket: 'waiting', order: 40, businessGroup: 'incoming', color: '#06b6d4' },
      { bucket: 'shipping', order: 50, businessGroup: 'incoming', color: '#f97316' },
      { bucket: 'delivered', order: 60, businessGroup: 'delivered', color: '#10b981' },
      { bucket: 'failed', order: 70, businessGroup: 'lost', color: '#ef4444' },
      { bucket: 'canceled', order: 80, businessGroup: 'canceled', color: '#94a3b8' }
    ];
  }

  function exactStatusBucket(row) {
    var status = rawLifecycleStatus(row);
    if (String(status || '').trim().toLowerCase() === 'canceled_by_you') return 'canceled_by_you';
    if (window.KhodDashboardLogic) return window.KhodDashboardLogic.normalize(status).bucket;
  ...
  function isFailedOutcomeBucket(bucket) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.isFailed === 'function') {
      return window.KhodDashboardLogic.isFailed(bucket);
    }
  ...
  function isCanceledLifecycleBucket(bucket) {
    if (bucket === 'canceled_by_you') return false;
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.isCanceled === 'function') {
      return window.KhodDashboardLogic.isCanceled(bucket);
    }
    return bucket === 'canceled' || bucket === 'cancelled';
  }
  ...
  function isEligibleForNdrStatus(status, bucket) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.isEligibleForNdr === 'function') {
      return window.KhodDashboardLogic.isEligibleForNdr(status || bucket);
    }
    if (window.TaagerStatus && typeof window.TaagerStatus.isEligibleForNdr === 'function') {
      return window.TaagerStatus.isEligibleForNdr(status);
    }
    return !isNdrCanceledBucket(bucket);
  }

  function isIncomingBucket(bucket) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.isIncoming === 'function') {
      return window.KhodDashboardLogic.isIncoming(bucket);
    }
    return bucket === 'pending' || bucket === 'confirmed' || bucket === 'processing' ||
      bucket === 'waiting' || bucket === 'shipping';
  }

  function isLostBucket(bucket) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.statusInfo === 'function') {
      return window.KhodDashboardLogic.statusInfo(bucket).businessGroup === 'lost';
    }
    return bucket === 'failed';
  }

  function isConfirmedBaseExcludedBucket(bucket) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.isConfirmed === 'function') {
      return !window.KhodDashboardLogic.isConfirmed(bucket);
    }
  ...
  function productStatusGroup(bucket) {
    if (window.KhodDashboardLogic && typeof window.KhodDashboardLogic.statusGroup === 'function') {
      return window.KhodDashboardLogic.statusGroup(bucket);
    }
  ...
  function effectiveCountry(country, meta, fallback) {
    // KHOD dashboard data is Saudi-only. Country inputs survive elsewhere only as
    // legacy storage/mapping aliases and must not split dashboard business data.
    return 'sa';
  }
  // ─────────────────────────────────────────────────────────────────────────────
  ...
  function orderRef(row) {
    return row.khodOrderNumber || row.taagerOrderNumber || row.orderNumber || row.id || row.orderId || '';
  }
  ...
  function statusBreakdownFromCounts(counts, total) {
    var rows = [];
    if (window.KhodDashboardLogic && Array.isArray(window.KhodDashboardLogic.all)) {
      window.KhodDashboardLogic.all.forEach(function (meta) {
        if (!meta || !isLifecycleStatusBucket(meta.bucket)) return;
        var label = typeof window.KhodDashboardLogic.display === 'function'
          ? window.KhodDashboardLogic.display(meta.bucket)
          : (meta.label || meta.bucket);
        rows.push({
          bucket: meta.bucket,
          label: label,
          ...
          color: typeof window.KhodDashboardLogic.color === 'function' ? window.KhodDashboardLogic.color(meta.bucket) : meta.color,
          eligible: typeof window.KhodDashboardLogic.isEligibleForNdr === 'function' ? window.KhodDashboardLogic.isEligibleForNdr(meta.bucket) : true,
          delivered: typeof window.KhodDashboardLogic.isDelivered === 'function' ? window.KhodDashboardLogic.isDelivered(meta.bucket) : meta.bucket === 'delivered',
          dashboardBucket: typeof window.KhodDashboardLogic.dashboardBucket === 'function' ? window.KhodDashboardLogic.dashboardBucket(meta.bucket) : meta.bucket
        });
      });
      return rows;
    }
  ...
  function buildAccountOptions(accounts, credList, period) {
    ...
    return ids.map(function (id) {
      ...
      var email = sourceAccount.khodEmail || sourceAccount.email || sourceAccount.easyEmail || '';
      var snapshot = Array.isArray(snap.snapshot) ? snap.snapshot : [];
      var netSnapshotRows = orderLevelRows(snapshot, false);
      var netPeriodRows = period && period.dateFrom && period.dateTo
        ? netSnapshotRows.filter(function (row) { return isRowCreatedInPeriod(row, period); })
        : netSnapshotRows;
      return Object.assign({}, sourceAccount, {
        ...
        taagerCountry: sourceAccount.khodCountry || sourceAccount.taagerCountry || identity.khodCountry || 'sa',
        email: email,
  ...
  function hashRows(rows) {
    ...
    var first = rows[0] ? (rows[0].khodOrderNumber || rows[0].taagerOrderNumber || rows[0].orderNumber || rows[0].id || '') : '';
    var last  = rows[rows.length - 1] ? (rows[rows.length - 1].khodOrderNumber || rows[rows.length - 1].taagerOrderNumber || rows[rows.length - 1].orderNumber || rows[rows.length - 1].id || '') : '';
  ...
  window.invalidateDashboardCache = function (reason) {
    ...
    if (window.KhodPageLifecycle && typeof window.KhodPageLifecycle.invalidate === 'function') {
      window.KhodPageLifecycle.invalidate('page-dashboard', reason || 'dashboard-data');
    }
  };
  ...
  window.runDashboardAggregator = function (callback) {
    ...
    function fetchDashboardSnapshot() {
      if (window.KhodPreloader && typeof window.KhodPreloader.dashboardStage === 'function') {
        window.KhodPreloader.dashboardStage('snapshot', { activity: 'Loading saved snapshot payload' });
      }
      if (window.api.getDashboardSnapshotGzip && typeof window.DecompressionStream === 'function') {
        ...
        var request = window.api.getDashboardSnapshotGzip(ALL_ACCOUNTS, _snapshotRevision).then(function (payload) {
          ...
          snapshotTransportMeta.fallback = window.api.getDashboardSnapshotJson ? 'json' : 'object';
          var fallbackPhase = startAggregationPhase('snapshot-transfer', { transport: snapshotTransportMeta.fallback + '-fallback' });
          ...
          return fallback.then(function (value) {
            endAggregationPhase(fallbackPhase, { ok: true });
            return parseSnapshotJson(value, snapshotTransportMeta.fallback + '-fallback');
          }, function (err) {
            endAggregationPhase(fallbackPhase, { ok: false, error: err && err.message ? err.message : String(err || '') });
            throw err;
          });
        });
        _snapshotRequestsInFlight[snapshotKey] = { promise: request, meta: snapshotTransportMeta };
    ...
    Promise.all([
      ...
      var rowScopePhase = startAggregationPhase('snapshot-row-scope', { accounts: Object.keys(accounts).length });
      if (window.KhodPreloader && typeof window.KhodPreloader.dashboardStage === 'function') {
        var accountCountForLoader = Object.keys(accounts).length;
        window.KhodPreloader.dashboardStage('accounts', {
      ...
      endAggregationPhase(rowScopePhase, { ok: true, rows: rows.length, activeAccountId: activeId });
      var meta = {
      ...
      if (window.KhodPreloader && typeof window.KhodPreloader.dashboardStage === 'function') {
        window.KhodPreloader.dashboardStage('metrics', {
      ...
      if (result && result.meta) result.meta.aggregationPhaseTimings = aggregationPhaseTimings.slice();
      if (window.KhodPreloader && typeof window.KhodPreloader.dashboardStage === 'function') {
        window.KhodPreloader.dashboardStage('modules', { activity: 'Dashboard views prepared' });
      }
  ...
  function processSnapshotRows(rows, accountId, meta) {
    ...
    var deliveredCount = 0, calculatorDeliveredCount = 0, shippingCount = 0, failedCount = 0, canceledCount = 0, canceledByYouCount = 0;
    ...
    var totalPriceLookup = buildTotalPriceLookup(rows);
    var totalSales = 0, totalDeliveredSales = 0, failedSales = 0, canceledSales = 0;
    ...
    function statusFinancial(bucket) {
      ...
      if (!statusFinancials[bucket]) {
        ...
        var label = window.KhodDashboardLogic && typeof window.KhodDashboardLogic.display === 'function'
          ? window.KhodDashboardLogic.display(bucket)
          : (window.TaagerStatus && typeof window.TaagerStatus.display === 'function'
    ...
    rows.forEach(function (row, rowIndex) {
      var bucket = getStatusBucket(rawLifecycleStatus(row));
      var exactBucket = exactStatusBucket(row);
      var hasLifecycleStatus = rowHasLifecycleStatus(row);
      var ndrEligible = hasLifecycleStatus && isEligibleForNdrStatus(rawLifecycleStatus(row), exactBucket);
      // KHOD commission is normalized by rowCommissionValue. Older profit field names
      // remain downstream compatibility aliases only.
      var commissionVal = rowCommissionValue(row);
      ...
      var rowInConfirmedBase = hasLifecycleStatus && inNdrCohortPeriod && rowIsNetOrder && !isConfirmedBaseExcludedBucket(exactBucket);
      var rowInBusinessConfirmedBase = hasLifecycleStatus && inCreatedPeriod && rowIsNetOrder && !isConfirmedBaseExcludedBucket(exactBucket);
      ...
      displayRows.push(Object.assign({}, row, {
        ...
        codValue: dueVal,
        dashboardCommission: commissionVal,
      ...
      if (isLostBucket(exactBucket)) {
        if (inCreatedPeriod && rowIsNetOrder) {
          if (addOnce(lostOrderSet, orderKey)) {
            failedCount++;
            failedSales += priceVal;
            lostCommission += commissionVal;
          }
          if (!rowIsPrepaid && addOnce(codOrderSet, 'lost:' + orderKey)) lostCodSar += dueVal;
        }
      } else if (isCanceledLifecycleBucket(exactBucket)) {
        if (inCreatedPeriod && rowIsNetOrder && addOnce(lostOrderSet, 'canceled:' + orderKey)) {
          canceledCount++;
          canceledSales += priceVal;
        }
      ...
      if (cityName && hasLifecycleStatus) {
        if (!cityStats[cityKeyName]) {
          cityStats[cityKeyName] = {
            ...
            prepaidFailedCount: 0, codFailedCount: 0,
      ...
      var cityOrderKey = cityKeyName + ':' + accountId + ':' + orderKey;
        ...
        if (hasLifecycleStatus && inCreatedPeriod && !rowIsCanceledByYou && addOnce(cityOrderSet, 'status:' + cityOrderKey)) {
          cs.statusTotalCount++;
        ...
        if (isDeliveredInPeriod && rowIsNetOrder) {
          ...
          if (addOnce(cityOrderSet, 'lost:' + cityOrderKey)) {
            ...
            if (rowIsPrepaid) {
              cs.prepaidFailedCount++;
            } else {
              cs.codFailedCount++;
            }
          }
        } else if (isCanceledLifecycleBucket(exactBucket) && inCreatedPeriod && rowIsNetOrder) {
          if (addOnce(cityOrderSet, 'canceled:' + cityOrderKey)) {
            cs.canceledCount++;
            if (rowIsPrepaid) {
        ...
        var cityProductName = rowProductName(row);
        ...
        if (cityProductKey) {
          ...
          // Track statusTotalCount only for net orders (excluding canceled_by_you).
          if (hasLifecycleStatus && inCreatedPeriod && !rowIsCanceledByYou && addOnce(productOrderSet, 'cityProductStatus:' + cpOrderKey)) {
            cp.statusTotalCount++;
          ...
          } else if (isCanceledLifecycleBucket(exactBucket) && inCreatedPeriod && rowIsNetOrder && addOnce(productOrderSet, 'cityProductCanceled:' + cpOrderKey)) {
            cp.canceled++;
      ...
      var productName = rowProductName(row);
      var productKey = (row.sku || productName || '').toLowerCase();
      if (productKey && hasLifecycleStatus) {
        ...
        if (inCreatedPeriod && rowIsNetOrder && addOnce(productOrderSet, 'placed:' + productOrderKey)) {
          ...
          var firstOrderDateKey = createdDashboardDate(row);
          if (firstOrderDateKey && (!productStats[productKey].firstOrderCreatedAt || firstOrderDateKey < productStats[productKey].firstOrderCreatedAt)) {
            productStats[productKey].firstOrderCreatedAt = firstOrderDateKey;
          }
        ...
        if (cityKey) {
          ...
          if (hasLifecycleStatus && inCreatedPeriod && rowIsNetOrder && addOnce(productOrderSet, 'productCityStatus:' + pcmOrderKey)) {
          ...
          } else if (isCanceledLifecycleBucket(exactBucket) && inCreatedPeriod && rowIsNetOrder && addOnce(productOrderSet, 'productCityCanceled:' + pcmOrderKey)) {
            pcm.canceled++;
          }
          ...
          if (rowIsPrepaid) {
            ...
            } else if (isCanceledLifecycleBucket(exactBucket) && inCreatedPeriod && rowIsNetOrder && addOnce(productOrderSet, 'productCityPrepaidCanceled:' + pcmOrderKey)) {
              pcm.prepaidCanceled++;
            }
          } else {
            if (isDeliveredInPeriod && rowIsNetOrder && addOnce(productOrderSet, 'productCityCodDelivered:' + pcmOrderKey)) {
              pcm.codDelivered++;
            } else if (isCanceledLifecycleBucket(exactBucket) && inCreatedPeriod && rowIsNetOrder && addOnce(productOrderSet, 'productCityCodCanceled:' + pcmOrderKey)) {
              pcm.codCanceled++;
            }
          }
        }
        ...
        if (inCreatedPeriod || inSelectedNdrBase || isDeliveredInNdrCohort) {
          ...
          if (hasLifecycleStatus && inCreatedPeriod && !rowIsCanceledByYou && addOnce(productOrderSet, 'pieceStatus:' + productKey + ':' + piecesKey + ':' + orderKey)) {
          ...
          if (cityKey) {
            ...
            if (hasLifecycleStatus && inCreatedPeriod && !rowIsCanceledByYou && addOnce(productOrderSet, 'quantityCityStatus:' + productKey + ':' + piecesKey + ':' + cityKey + ':' + orderKey)) {
    ...
    function resetExpectedRateCountersFromSource() {
      ...
      sourceRows.forEach(function (row, rowIndex) {
        ...
        var rowIsCanceledByYou = isCanceledByYouBucket(exactBucket);
        var rowIsNetOrder = !rowIsCanceledByYou;
        var rowInConfirmedBase = hasLifecycleStatus && rowIsNetOrder && !isConfirmedBaseExcludedBucket(exactBucket);
        var rowIsPrepaid = isRowPrepaid(row);
        var orderKey = orderOnlyKey(row, rowIndex);
    ...
    var sortedCities = Object.keys(cityStats).map(function (keyName) {
      ...
      return {
        ...
        // T-09: Financial
        totalRevenue: stat.totalRevenue, salesValue: stat.totalRevenue, codValue: stat.due, avgOrderValue: avgOrderValue,
        ...
        commissionValue: stat.earnedProfitAfterTax != null ? stat.earnedProfitAfterTax : stat.earnedCommission,
        ...
        prepaidFailedCount: stat.prepaidFailedCount, codFailedCount: stat.codFailedCount,
    ...
    var statusKnownTotal = Object.keys(statusCounts).reduce(function (sum, key) {
    ...
    var statusBreakdown = statusBreakdownFromCounts(statusCounts, statusKnownTotal);
    var exactStatusSummary = orderedStatusFlow().map(function (info) {
      var row = statusFinancial(info.bucket);
      return Object.assign({}, row, {
        label: window.KhodDashboardLogic && typeof window.KhodDashboardLogic.display === 'function'
          ? window.KhodDashboardLogic.display(info.bucket)
          : (window.TaagerStatus && typeof window.TaagerStatus.display === 'function'
    ...
    var pipelineStages = exactStatusSummary.map(function (row) {
      ...
      st.commissionValue = roundMoney(row.commissionValue != null ? row.commissionValue : row.profitAfterTax);
      st.codSar = roundMoney(row.cod);
      st.codValue = roundMoney(row.codValue != null ? row.codValue : row.cod);
      st.salesSar = roundMoney(row.sales);
      st.salesValue = roundMoney(row.salesValue != null ? row.salesValue : row.sales);
    ...
    // Dashboard rows are created-date based; delivered outcomes follow the selected mode.
    var createdOrders = null;
    var outcomeOrders = null;
    function getCreatedOrders() {
      if (createdOrders === null) createdOrders = orderLevelRows(filterCreatedOrders(displayRows, meta.period), true);
      return createdOrders;
    }
    function getOutcomeOrders() {
      if (outcomeOrders === null) {
        outcomeOrders = orderLevelRows(filterOutcomeOrders(displayRows, meta.period, meta.deliveredDateMode || 'actual'), true);
      }
    ...
    return {
      meta: meta,
      financials: {
        salesValue: roundMoney(totalSales),
        codValue: roundMoney(totalDue),
        commissionValue: roundMoney(earnedCommission),
        deliveredSales: roundMoney(totalDeliveredSales),
        failedSales: roundMoney(failedSales),
        canceledSales: roundMoney(canceledSales)
      },
      overview: {
        ...
        totalSales:          { value: totalSales, salesValue: totalSales, delta: calcDelta(totalSales, previousOverview.totalSales),                   unit: meta.activeCurrency || 'SAR', color: 'green'  },
        ...
        totalDeliveredSales: { value: totalDeliveredSales, deliveredSales: totalDeliveredSales, delta: calcDelta(totalDeliveredSales, previousOverview.totalDeliveredSales), unit: meta.activeCurrency || 'SAR', color: 'green'  },
        failedSales:         { value: failedSales, failedSales: failedSales, unit: meta.activeCurrency || 'SAR', color: 'red' },
        canceledSales:       { value: canceledSales, canceledSales: canceledSales, unit: meta.activeCurrency || 'SAR', color: 'gray' },
      ...
      pipeline: {
        metrics: {
          ...
          statusTotalCount: statusKnownTotal,
          lifecycleStatusCount: statusKnownTotal,
        ...
        // Compatibility field retained for older section callers; its contents are KHOD stages.
        legacyStages: pipelineStages
      },
      statusBreakdown: statusBreakdown,
      statusSummary: exactStatusSummary,
      lostBreakdown: lostBreakdown,
      ...
      cod: {
        ...
        codValue: totalDue,
        amountDue: totalDue,
        dashboardAmountDue: totalDue,
        salesValue: totalSales,
        commissionValue: earnedCommission,
      ...
      products: {
        summary: {
          ...
          commissionValue: earnedCommission,
          salesValue: totalSales,
          deliveredSales: totalDeliveredSales,
      ...
      roi: {
        ...
        averageCommission: avgCommission, avgCommission: avgCommission,
        averageProfit: avgCommission,
        taagerProfitAfterTax: taagerProfitAfterTax,
        deliveredCount: deliveredCount, totalDeliveredSales: totalDeliveredSales,
        actualDeliveredCount: actualDeliveredCount,
        actualEarnedCommission: actualEarnedCommission,
        commissionValue: actualEarnedCommission,
        actualEarnedProfitAfterTax: actualEarnedCommission,
        actualDeliveredSales: actualTotalDeliveredSales,
        ...
        expectedCommissionBeforeAdSpend: accountFinancials.expectedCommissionBeforeAdSpend,
