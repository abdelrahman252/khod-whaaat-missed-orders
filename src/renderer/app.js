...
window.TaagerCurrency = (() => {
  ...
  const SUPPORTED = ["SAR", "USD"];
  const DEFAULT_RATES = { SAR: 3.75, USD: 1 };
  ...
  function cleanCurrency(value, fallback) {
    ...
    if (SUPPORTED.indexOf(cur) !== -1) return cur;
    const fallbackCurrency = String(fallback || "SAR").trim().toUpperCase();
    return SUPPORTED.indexOf(fallbackCurrency) !== -1 ? fallbackCurrency : "SAR";
  }

  function readStoredRates() {
    try {
      const parsed = JSON.parse(localStorage.getItem(STORAGE_KEY) || "null");
      if (parsed && parsed.rates && typeof parsed.rates === "object") return sanitizeRates(parsed.rates);
    } catch (e) {
  ...
  function readStoredMeta() {
    try {
      ...
      if (parsed && typeof parsed === "object") {
        return { source: parsed.source || "defaults", updatedAt: parsed.updatedAt || "", rates: sanitizeRates(parsed.rates || {}) };
      }
    } catch (e) {
      console.warn("[Currency] Unable to read cached rate metadata:", e);
    }
    return { source: "defaults", updatedAt: "", rates: Object.assign({}, DEFAULT_RATES) };
  }
  ...
  function persistRates(rates, source) {
    ...
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ source: source || "manual", updatedAt: new Date().toISOString(), rates: cached }));
    } catch (e) {
  ...
  function snapshot() {
    ...
    return { rates: Object.assign({}, cached), source: stored.source || "defaults", updatedAt: stored.updatedAt || "", base: "USD", supported: SUPPORTED.slice() };
  }
  ...
  function format(value, currency, opts) {
    ...
    const locale = opts.locale || (window.dashboardI18n && window.dashboardI18n.locale && window.dashboardI18n.locale()) || "en-US";
    const decimals = opts.decimals == null ? 0 : Number(opts.decimals || 0);
    try {
      return new Intl.NumberFormat(locale, { style: opts.style === "code" ? "decimal" : "currency", currency: cur, minimumFractionDigits: decimals, maximumFractionDigits: decimals }).format(Number(value || 0)) + (opts.style === "code" ? " " + cur : "");
    } catch (e) {
      return Number(value || 0).toLocaleString("en-US", { minimumFractionDigits: decimals, maximumFractionDigits: decimals }) + " " + cur;
    }
  }
  ...
  async function ensureLiveRates(opts) {
    opts = opts || {};
    var meta = readStoredMeta();
    if (meta.source === "manual" && !opts.force) return { ok: true, refreshed: false, snapshot: snapshot() };
    if (!opts.force && !isStale(opts.maxAgeMs || STALE_AFTER_MS)) return { ok: true, refreshed: false, snapshot: snapshot() };
  ...
  return { supported: SUPPORTED.slice(), defaults: Object.assign({}, DEFAULT_RATES), base: "USD", rates, snapshot, setManualRates, resetRates, isStale, refreshLiveRates, ensureLiveRates, convert, format, cleanCurrency };
})();
...
const _STRINGS = {
  ...
  ar: {
    ...
    "welcome.launch_min":     "🔕 تصغير Chrome",
    "welcome.launch_min_desc":"عند التفعيل، يبقى Chrome مصغراً أثناء تشغيل البوت. نافذة التطبيق تبقى على حالتها.",
...
const FEATURE_SCRIPT_GROUPS = {
  ...
  operations: {
    styles: ["styles/operations.css", "styles/analytics.css"],
    ordered: [
      [
        "locales/ar/analytics.js",
        "locales/en/analytics.js",
  ...
  dashboard: {
    ...
    ordered: [
      ...
      [
        ...
        "pages/dashboard/dashboard-orders-query-core.js",
      ...
      ["pages/dashboard/dashboard-shell.js", "pages/dashboard/dashboard.js"],
    ],
  },
};
...
function loadStyleOnce(href) {
  if (!href || _loadedFeatureStyles.has(href)) return Promise.resolve();
  ...
  if (existing && existing.getAttribute("rel") === "stylesheet") {
...
function normalizeFeatureGroup(feature) {
  const group = FEATURE_SCRIPT_GROUPS[feature];
  if (!group) return { styles: [], ordered: [] };
  if (Array.isArray(group)) return { styles: [], ordered: group.map(src => [src]) };
  return {
    styles: group.styles || [],
    ordered: group.ordered || (group.scripts ? group.scripts.map(src => [src]) : []),
  };
}

async function ensureFeatureScripts(feature, options) {
  options = options || {};
  const start = "feature:" + feature + ":start";
  const end = "feature:" + feature + ":end";
  markPerf(start);
  const group = normalizeFeatureGroup(feature);
  await Promise.all(group.styles.map(loadStyleOnce));
  for (const batch of group.ordered) {
    const scripts = Array.isArray(batch) ? batch : [batch];
    if (options.parallel === false || scripts.length === 1) {
  ...
  markPerf(end);
  measurePerf("feature:" + feature, start, end);
}

window.ensureFeatureScripts = ensureFeatureScripts;
window.prefetchFeatureScripts = function prefetchFeatureScripts(feature, options) {
  options = options || {};
  const idleTimeout = Number.isFinite(options.timeout) ? options.timeout : 2500;
  const delay = Number.isFinite(options.delay) ? options.delay : 1200;
  const scheduleIdle = window.requestIdleCallback || function (cb) { return setTimeout(cb, idleTimeout); };
  return setTimeout(function () {
    scheduleIdle(function () {
      ensureFeatureScripts(feature).catch(function (err) {
        console.warn("[Perf] Feature prefetch failed:", feature, err && err.message ? err.message : err);
      });
    }, { timeout: idleTimeout });
  }, delay);
};
window.invalidateAnalyticsRunsCache = function invalidateAnalyticsRunsCache() {
  window.__analyticsRunsCache = null;
  window.__analyticsRunsCacheAt = 0;
  window.__analyticsFlattenCache = new WeakMap();
};
window.invalidateDashboardCache = function invalidateDashboardCache() {
  window.__dashboardAggregatorCache = new Map();
};
...
async function init() {
  ...
  if (window.prefetchFeatureScripts) {
    window.prefetchFeatureScripts("dashboard", { delay: 1200 });
    window.prefetchFeatureScripts("analytics", { delay: 2200 });
    window.prefetchFeatureScripts("operations", { delay: 3000 });
  }
}
...
async function _saveAnalyticsFromResult(data, selectedAccountIds) {
  ...
  if (data._multiAccount && Array.isArray(data._accountResults)) {
    ...
    if (window.invalidateAnalyticsRunsCache) window.invalidateAnalyticsRunsCache();
...
function _renderFeatureLoader(pageId, label) {
  const el = document.getElementById(pageId);
  if (!el || el.childElementCount) return;
  const text = window._t ? window._t("ui.loading") : "Loading...";
  el.innerHTML = `
    <div class="sv3-shell" style="height:100%;">
      ${renderSharedSidebar(label === "Dashboard" ? "dashboard" : label === "Operations" ? "operations" : label === "Analytics" ? "analytics" : "run")}
      <div style="flex:1;min-height:0;display:flex;align-items:center;justify-content:center;color:var(--text2);">
        <div class="khod-state khod-state-loading" role="status" aria-live="polite">
          <div class="khod-spinner" aria-hidden="true"></div>
          <div class="khod-state-title">${text}</div>
        </div>
      </div>
    </div>`;
...
async function goToAnalytics() {
  ...
  markPerf("nav:analytics:start");
  _renderFeatureLoader("page-analytics", "Analytics");
  ...
  try {
    ...
    markPerf("nav:analytics:end");
    measurePerf("nav:analytics", "nav:analytics:start", "nav:analytics:end");
  } catch (err) {
    if (window.KhodMonitoring) window.KhodMonitoring.captureException(err, { operation: "analytics.render" });
    if (isLatestFeatureRoute("analytics", token) && isActivePage("page-analytics")) throw err;
  }
}

async function goToOperations() {
  const token = nextFeatureRouteToken("operations");
  markPerf("nav:operations:start");
  _renderFeatureLoader("page-operations", "Operations");
  ...
  try {
    ...
    markPerf("nav:operations:end");
    measurePerf("nav:operations", "nav:operations:start", "nav:operations:end");
  } catch (err) {
    if (window.KhodMonitoring) window.KhodMonitoring.captureException(err, { operation: "operations.render" });
    if (isLatestFeatureRoute("operations", token) && isActivePage("page-operations")) throw err;
  }
}

async function goToDashboard() {
  const token = nextFeatureRouteToken("dashboard");
  markPerf("nav:dashboard:start");
  _renderFeatureLoader("page-dashboard", "Dashboard");
  ...
  try {
    ...
    markPerf("nav:dashboard:end");
    measurePerf("nav:dashboard", "nav:dashboard:start", "nav:dashboard:end");
  } catch (err) {
    if (window.KhodMonitoring) window.KhodMonitoring.captureException(err, { operation: "dashboard.render" });
    if (isLatestFeatureRoute("dashboard", token) && isActivePage("page-dashboard")) throw err;
  }
}
...
async function goToAiIntelligence() {
  ...
  _renderFeatureLoader("page-ai-intelligence", "KHOD AI");
  showPage("page-ai-intelligence");
  try {
    ...
    if (isLatestFeatureRoute("ai-intelligence", token) && isActivePage("page-ai-intelligence")) throw err;
  }
}
...
function _setDashboardFetchUi(state) {
  ...
  if (btn) {
    ...
    const draftPending = btn.dataset.draftPending === "true";
    btn.disabled = !!state.active || draftPending;
