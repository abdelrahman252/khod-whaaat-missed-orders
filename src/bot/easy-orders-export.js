...
function createEasyOrdersExportFlow(options = {}) {
  ...
  async function findExportLink(page, keyword, ignoredHrefs = []) {
    return page.evaluate(({ keyword, ignoredHrefs }) => {
      ...
      // EasyOrders renders notifications as table rows. Reading one row per
      // notification avoids duplicate ancestors and stale card matches.
      const candidates = Array.from(document.querySelectorAll("table tbody tr, table tr, [role='row']"))
  ...
  async function triggerExport(page, exportFromDate, keyword) {
    ...
    // Ignore workbook links that existed before this export request. This
    // prevents returning an older same-day report while the new one is late.
    try {
    ...
    for (let attempt = 1; attempt <= exportAttempts; attempt++) {
      ...
      const errorCode = rateLimited ? "EASY_ORDERS_EXPORT_RATE_LIMITED" : "EASY_ORDERS_NOTIFICATION_TIMEOUT";
