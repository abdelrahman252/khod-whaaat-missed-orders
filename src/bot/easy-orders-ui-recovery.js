...
function createEasyOrdersUiRecovery(options = {}) {
    ...
    for (const response of responses || []) {
      try {
        ...
        if (status >= 400 || method !== "GET") {
          body = String(await response.text().catch(() => "") || "").replace(/\s+/g, " ").trim().slice(0, 1000);
        }
  ...
  async function resendRealOrder(page, candidate, options = {}) {
    ...
    const onResponse = (response) => {
      ...
      const method = String(request.method() || "GET").toUpperCase();
      const status = Number(response.status() || 0);
      if (/easy-orders\.net/i.test(response.url()) && (method !== "GET" || status >= 400)) actionResponses.push(response);
    };
    page.on("response", onResponse);
    let toast = "";
    try {
      await button.click({ timeout: 10000 });
      toast = await Promise.race([waitForToast(page, before, 8000), page.waitForTimeout(8000).then(() => "")]);
    } finally {
    ...
    const successfulResponse = responseSummaries.some((response) => response.status >= 200 && response.status < 300 && response.method !== "GET");
    if (responseSummaries.length) log(`EasyOrders recovery real resend network: ${candidate.easyShortId || candidate.easyOrderUuid || candidate.normPhone || ""} -> ${responseSummaries.map((response) => `${response.method} ${response.status}${response.body ? ` ${response.body}` : ""}`).join(" | ")}`);
    if (responseFailure) log(`EasyOrders recovery real resend failed: ${candidate.easyShortId || candidate.easyOrderUuid || candidate.normPhone || ""} -> ${responseFailure}`);
    else log(`EasyOrders recovery real resend result: ${candidate.easyShortId || candidate.easyOrderUuid || candidate.normPhone || ""} -> ${toast || (successfulResponse ? "Resend request accepted" : "Resend clicked, no toast captured")}`);
    return {
      ...candidate,
      detailUrl: page.url(),
