...
const ALLOWED_AR_LATIN_TERMS = /\b(?:SKU|COD|NDR|CPA|ROI|ROAS|SAR|USD|EGP|KPI|AI|KHOD WHAAT|KHOD|TikTok|Snapchat|Facebook|Windsor|Easy-?Orders|Whaat|GMV|Saudi|iPick|API)\b/gi;
...
["analytics", "operations"].forEach((namespace) => {
  const enKeys = walkKeys(win.KHOD_LOCALES.en[namespace]);
  const arKeys = walkKeys(win.KHOD_LOCALES.ar[namespace]);
...
const arabicLocaleLeaks = [
  ...
  ...visibleLatinLeaks(win.KHOD_LOCALES.ar.analytics, "analytics"),
  ...visibleLatinLeaks(win.KHOD_LOCALES.ar.operations, "operations"),
];
ok("Arabic locales have no visible English fragments", arabicLocaleLeaks.length === 0, arabicLocaleLeaks.slice(0, 30).join("\n    "));

const khodAi = read("src/renderer/pages/dashboard/sections/section-khod-ai.js");
ok("KHOD WHAAT AI metric labels are localized", ["aii.metric.health", "aii.metric.alerts", "aii.metric.opportunities"].every((key) => khodAi.includes(key)));
ok("KHOD WHAAT AI disclaimer is localized", khodAi.includes("aii.disclaimer.metrics") && khodAi.includes("aii.disclaimer.scope"));
...
ok("AI Intelligence CSS has light-theme overrides", aiCss.includes('[data-theme="light"] #page-ai-intelligence .khod-ai-section'));
