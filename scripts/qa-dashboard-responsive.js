...
const RUN_LIVE_AI_INTERACTIONS = process.env.TAAGER_QA_LIVE_AI === "1";
const RUN_REACTIVE_INTERACTIONS = process.env.TAAGER_QA_REACTIVE_INTERACTIONS === "1";
const EXHAUSTIVE_WIDTHS = process.env.TAAGER_QA_EXHAUSTIVE_WIDTHS === "1";
const REQUESTED_DASHBOARD_SECTIONS = String(process.env.TAAGER_QA_SECTIONS || "")
  .split(",")
...
const DASHBOARD_SCREENSHOT_TIMEOUT_MS = 90000;
...
async function ensureViewportSize(page, size) {
...
function selectedDashboardSections(sectionIds) {
  if (!REQUESTED_DASHBOARD_SECTIONS.length) return sectionIds;
  return sectionIds.filter((sectionId) => REQUESTED_DASHBOARD_SECTIONS.includes(sectionId));
}
...
async function launchElectronForQa() {
  let playwrightApp = null;
  const useCdpFirst = process.env.TAAGER_QA_FORCE_CDP === "1" || process.platform === "win32";
  try {
    if (useCdpFirst) throw new Error("CDP selected as the reliable Windows QA launch path");
    playwrightApp = await withTimeout(electron.launch({
      cwd: ROOT,
      args: [ROOT],
      env: {
        ...process.env,
        ELECTRON_DISABLE_SECURITY_WARNINGS: "true",
        KHOD_QA_USER_DATA_DIR: QA_USER_DATA_DIR,
      },
    }), 20000, "Playwright Electron launch");
    const firstPage = await withTimeout(playwrightApp.firstWindow(), 15000, "Playwright Electron first window");
    await firstPage.waitForLoadState("domcontentloaded", { timeout: 10000 });
    return {
      mode: "electron",
      firstWindow: async () => firstPage,
      close: async () => playwrightApp.evaluate(({ app }) => app.exit(0)).catch(() => playwrightApp.close().catch(() => {})),
    };
  } catch (err) {
    if (useCdpFirst) console.log("[qa] using CDP-first Electron launch on Windows");
    else console.warn("[qa] Playwright Electron launch failed, falling back to CDP:", err.message);
    if (playwrightApp) {
      await withTimeout(
        playwrightApp.evaluate(({ app }) => app.exit(0)),
        5000,
        "Failed Playwright Electron exit"
      ).catch(() => withTimeout(playwrightApp.close(), 5000, "Failed Playwright Electron cleanup").catch(() => {}));
      playwrightApp = null;
    }
  }

  const executablePath = require("electron");
  const port = Number(process.env.TAAGER_QA_CDP_PORT || (9300 + (process.pid % 500)));
  ...
  try {
    await waitForCdp(port, DASHBOARD_SCREENSHOT_MODE ? 3000 : 30000);
...
function makeOrder(i) {
  ...
  const statuses = ["Delivered", "In shipping", "Confirmed", "Waiting", "Failed", "Canceled", "Pending", "Under processing"];
  ...
  return {
    ...
    notes: i % 7 === 0 ? "prepaid visa" : "",
    source: i % 4 === 0 ? "missed" : "real",
    accountId: "sa-main",
    dashboardAccountId: "sa-main",
...
async function mountDashboard(page) {
  ...
  await page.evaluate(async (payload) => {
    ...
    function buildDashboardResult(srcOrders) {
      ...
      const rankedList = Object.values(productMap).map((p) => {
        ...
        p.netOrderCount = p.placedCount;
        p.totalOrderCount = p.placedCount;
        p.ndrBaseOrders = p.placedCount;
        p.ndrDeliveredOrders = p.deliveredCount;
        p.drBaseOrders = p.confirmedCount;
        p.drDeliveredOrders = p.deliveredCount;
        p.rateMode = "actual";
        p.rateSource = "product";
        ...
        return p;
      }).sort((a, b) => (b.deliveredCount - a.deliveredCount) || (b.commission - a.commission) || String(a.key || "").localeCompare(String(b.key || ""))).map((p, idx) => Object.assign(p, {
        rank: idx + 1,
        emoji: ["🛡️", "🚚", "💳", "📈"][idx % 4],
      ...
      const incoming = Math.round((total - delivered - failed - canceled) * avgCommission);
      const lost = Math.round((failed + canceled) * avgCommission);
      ...
      return {
        ...
        pipeline: {
          metrics: { totalOrders: total, deliveredOrders: delivered, failedOrders: failed, canceledOrders: canceled, activeOrders: total - delivered - failed - canceled },
          stages: [
            { id: "pending", label: "Pending", count: Math.round(total * 0.12), color: "#94a3b8" },
            { id: "confirmed", label: "Confirmed", count: Math.round(total * 0.22), color: "#38bdf8" },
            { id: "processing", label: "Under processing", count: Math.round(total * 0.12), color: "#a78bfa" },
            { id: "waiting", label: "Waiting", count: Math.round(total * 0.12), color: "#fbbf24" },
            { id: "shipping", label: "In shipping", count: Math.round(total * 0.18), color: "#f59e0b" },
            { id: "delivered", label: "Delivered", count: delivered, color: "#22c55e" },
            { id: "failed", label: "Failed", count: failed, color: "#ef4444" },
            { id: "canceled", label: "Canceled", count: canceled, color: "#f43f5e" },
          ],
        },
    ...
    if (window.api && typeof window.api.saveDashboardSnapshot === "function") {
      await window.api.saveDashboardSnapshot("sa-main", {
        snapshot: orders,
        dateFrom: "2026-05-01",
        dateTo: "2026-05-31",
    ...
    function forceQaDashboardPeriod() {
      const qaDashboardPeriod = { preset: "custom", dateFrom: "2026-05-01", dateTo: "2026-05-31" };
      if (window.DashboardPeriodState && typeof window.DashboardPeriodState.setCustomRange === "function") {
        window.DashboardPeriodState.setCustomRange(qaDashboardPeriod.dateFrom, qaDashboardPeriod.dateTo);
        window.DashboardPeriodState.get = () => Object.assign({}, qaDashboardPeriod);
      }
    }
    forceQaDashboardPeriod();
    window.dashboardGeoData = window.__qaDashboardResult;
    ...
    forceQaDashboardPeriod();
    const fixtureData = Object.assign({}, window.__qaDashboardResult);
    const liveData = window.dashboardGeoData || {};
    Object.keys(liveData).forEach((key) => delete liveData[key]);
    Object.assign(liveData, fixtureData, { _loaded: true, _loading: false });
    window.dashboardGeoData = liveData;
    if (typeof window.refreshDashboardShell === "function") {
      window.refreshDashboardShell(document.getElementById("db-shell-mount"), liveData);
    }
  ...
  console.log("[qa] fixture state", await page.evaluate(() => JSON.stringify({
    fixtureOrders: window.__qaDashboardResult?.overview?.totalOrders?.value,
    mountedOrders: window.dashboardGeoData?.overview?.totalOrders?.value,
    activeSection: document.querySelector(".dash-nav-btn.is-active")?.getAttribute("data-section"),
  })));
  await page.waitForFunction(() => Number(window.dashboardGeoData?.overview?.totalOrders?.value || 0) > 0, null, { timeout: 15000 });
  console.log("[qa] KHOD WHAAT fixture mounted with nonzero dashboard data");
}
...
async function mountAiIntelligence(page) {
  ...
  await page.waitForSelector("#page-ai-intelligence .khod-ai-section", { timeout: 15000 });
}
...
async function waitForDashboardSectionReady(page, sectionId) {
  await page.waitForFunction((id) => {
    const mount = document.getElementById("db-shell-mount");
    const pane = document.getElementById("dash-section-pane");
    const activePane = pane && Array.from(pane.children).find((child) =>
      child.dataset &&
      child.dataset.sectionId === id &&
      !child.hidden &&
      child.getAttribute("aria-hidden") !== "true"
    );
    return mount?._dashboardActiveSection === id &&
      !!activePane && (activePane.innerText || "").trim().length > 0;
  }, sectionId, { timeout: 15000 });
  await page.evaluate(() => new Promise((resolve) => {
    requestAnimationFrame(() => requestAnimationFrame(resolve));
  }));
}

async function verifyDashboardDataStates(page) {
  const singleState = await page.evaluate(() => {
    const singleOrders = (window.__qaOrders || []).filter((_, index) => index % 2 === 0);
    const result = window.__qaBuildDashboardResult(singleOrders);
    result.meta.activeAccountId = "sa-main";
    result.meta.activeAccountLabel = "Saudi Main Store";
    window.__qaActiveAccountId = "sa-main";
    window.currentActiveAccountLabel = "Saudi Main Store";
    window.__qaDashboardResult = result;
    window.dashboardGeoData = result;
    window.runDashboardAggregator = (done) => done(result);
    window.showPage("page-dashboard");
    const stateData = Object.assign({}, result);
    const liveData = window.dashboardGeoData || {};
    Object.keys(liveData).forEach((key) => delete liveData[key]);
    Object.assign(liveData, stateData, { _loaded: true, _loading: false });
    window.dashboardGeoData = liveData;
    window.refreshDashboardShell(document.getElementById("db-shell-mount"), liveData);
    return { accountId: result.meta.activeAccountId, orders: result.orders.length };
  });
  await page.waitForSelector("#db-shell-mount.dash-shell", { timeout: 15000 });
  await inspect(page, "dashboard-single-account", { width: 900, height: 640 });
  if (singleState.accountId !== "sa-main" || !singleState.orders) {
    throw new Error("Single-account dashboard state did not mount: " + JSON.stringify(singleState));
  }

  const emptyState = await page.evaluate(() => {
    const result = window.__qaBuildDashboardResult([]);
    result.meta.hasData = false;
    result.meta.activeAccountId = "sa-main";
    result.meta.activeAccountLabel = "Saudi Main Store";
    window.__qaDashboardResult = result;
    window.dashboardGeoData = result;
    ...
    window.dashboardGeoData = liveData;
    window.refreshDashboardShell(document.getElementById("db-shell-mount"), liveData);
    return { hasData: result.meta.hasData, orders: result.orders.length };
  });
  await page.waitForSelector("#db-shell-mount.dash-shell", { timeout: 15000 });
  await inspect(page, "dashboard-empty-state", { width: 900, height: 640 });
  const paneHasContent = await page.locator("#dash-section-pane").evaluate((pane) => (pane.innerText || "").trim().length > 2);
  if (emptyState.hasData !== false || emptyState.orders !== 0 || !paneHasContent) {
    throw new Error("Empty dashboard state did not render: " + JSON.stringify({ emptyState, paneHasContent }));
  }

  await page.evaluate(() => {
    const result = window.__qaBuildDashboardResult(window.__qaOrders || []);
    window.__qaActiveAccountId = "__all__";
    window.currentActiveAccountLabel = "All Accounts";
    window.__qaDashboardResult = result;
    window.dashboardGeoData = result;
    window.runDashboardAggregator = (done) => done(result);
    window.showPage("page-dashboard");
  ...
  await page.waitForSelector("#db-shell-mount.dash-shell", { timeout: 15000 });
  console.log("[qa] All Accounts, single-account, and empty dashboard states verified");
}

async function verifyLiveKhodAiSmoke(page) {
  ...
  await page.locator('.dash-nav-btn[data-section="khodAi"]').evaluate((button) => button.click());
  ...
  console.log("[qa] live KHOD WHAAT AI mirror debug", JSON.stringify(mirrorDebug));
  ...
  if (!/\b(?:NDR|CPA)\b|الربح|الخسار/.test(arabicLoss.text) ||
      ...
      /I am KHOD WHAAT AI|How can I assist|أنا KHOD WHAAT AI|أستطيع مساعدتك/i.test(arabicLoss.text)) {
  ...
  if (!/\b(?:NDR|CPA)\b|الربح|الخسار/i.test(englishProfit.text) ||
      ...
      /I am KHOD WHAAT AI|How can I assist|أنا KHOD WHAAT AI|أستطيع مساعدتك/i.test(englishProfit.text)) {
  ...
  let plan = await askLive("ابنِ خطة توسع");
  if (/مبلغ الإنفاق|العملة|500 SAR|120 USD|8000 EGP|250 AED/i.test(plan.text)) {
    plan = await askLive("500 SAR");
  }
  ...
  console.log("[qa] live KHOD WHAAT AI Arabic smoke prompts verified");
}

async function verifyLiveKhodAiPerformance(page) {
  await showDashboard(page);
  await page.evaluate(() => {
  ...
  try {
    ...
    const snapshot = await page.evaluate(() => {
      ...
      return {
        ...
        renderFunctionReady: typeof window.renderSectionKhodAi === "function",
  ...
  console.log("[qa] live KHOD WHAAT AI performance", JSON.stringify({ ranking, plan }));
}
...
async function verifyDashboardPerformanceAcceptance(page) {
  ...
  await page.evaluate(() => {
    if (window.KhodPerf && window.KhodPerf.clear) window.KhodPerf.clear();
    if (typeof window.applyLang === "function") window.applyLang("en");
  });
  ...
  const cachedRestoreMs = await page.evaluate(async () => {
    ...
    if (window.KhodPerf && window.KhodPerf.clear) window.KhodPerf.clear();
    button.click();
    const entry = window.KhodPerf.entries().filter((item) =>
      item.type === "measure" &&
  ...
  await showDashboard(page);
  const stability = await page.evaluate(() => window.KhodPerf.runDashboardSectionStabilityCheck(
  ...
  if (after.dashboardPaneChildren > 17) throw new Error("Dashboard pane cache is unbounded: " + JSON.stringify({ before, after }));
...
async function inspect(page, label, size) {
  ...
  await ensureViewportSize(page, size);
  await page.waitForTimeout(DASHBOARD_SCREENSHOT_MODE ? 100 : 550);
  ...
  const metrics = await page.evaluate(() => {
    ...
    const activeRoot = document.querySelector(".page.active") || document.body;
    const legacyVisibleText = (activeRoot.innerText || "").match(/\bTaager\b|profit[ -]after[ -]tax/ig) || [];
    const legacyVisibleNodes = legacyVisibleText.length
      ? Array.from(activeRoot.querySelectorAll("*"))
        .filter((node) => node.offsetParent && /\bTaager\b|profit[ -]after[ -]tax/i.test(node.innerText || ""))
        .filter((node) => !Array.from(node.children).some((child) => /\bTaager\b|profit[ -]after[ -]tax/i.test(child.innerText || "")))
        .slice(0, 5)
        .map((node) => ({
          selector: node.id ? "#" + node.id : String(node.className || node.tagName).trim(),
          text: (node.innerText || "").trim().replace(/\s+/g, " ").slice(0, 180),
        }))
      : [];
    return {
      ...
      activeSection: document.querySelector(".dash-nav-btn.is-active")?.getAttribute("data-section"),
      activeSectionHasContent: !!Array.from(document.querySelectorAll("#dash-section-pane > *"))
        .find((el) => !el.hidden && el.getAttribute("aria-hidden") !== "true" &&
          el.getClientRects().length > 0 && (el.innerText || "").trim().length > 0),
      ...
      legacyVisibleText: legacyVisibleText.slice(0, 5),
      legacyVisibleNodes,
      ai: {
        ...
        intelligencePage: !!document.querySelector(".khod-ai-section"),
  ...
  console.log("[qa:timing] inspect " + label + " " + (Date.now() - inspectStartedAt) +
    "ms (evaluate " + (Date.now() - evaluateStartedAt) + "ms, DOM " + metrics.domNodeCount + ")");
  ...
  if (metrics.legacyVisibleText.length) {
    throw new Error(label + " exposes legacy Taager/profit-after-tax wording: " + JSON.stringify(metrics.legacyVisibleNodes));
  }
  if (metrics.activePage === "page-dashboard" && !metrics.activeSectionHasContent) {
    throw new Error(label + " renders a blank dashboard section");
  }
...
async function captureDashboardSection(page, sectionId, size) {
  ...
  if (!EXHAUSTIVE_WIDTHS) {
    await page.waitForTimeout(100);
    await page.screenshot({
      path: path.join(DASHBOARD_SCREENSHOT_DIR, `${sectionId}-laptop.jpg`),
      type: "jpeg",
      quality: 82,
      animations: "disabled",
      fullPage: false,
      timeout: DASHBOARD_SCREENSHOT_TIMEOUT_MS,
    });
    console.log("[qa:timing] capture " + sectionId + " " + (Date.now() - captureStartedAt) + "ms");
    return;
  }
...
(async () => {
  ...
  try {
    ...
    page.on("console", async (msg) => {
      ...
      if (msg.text().includes("[DashboardQuery][shadow]")) {
        const args = msg.args();
        if (args[1]) {
          try {
            console.log("[renderer:shadow-json]", JSON.stringify(await args[1].jsonValue()));
          } catch (_) {}
        }
      }
    });
    ...
    if (process.env.TAAGER_QA_AI_SMOKE_ONLY === "1") {
      await verifyLiveKhodAiSmoke(page);
      console.log("[qa] live KHOD WHAAT AI smoke verification complete");
      return;
    }
    if (process.env.TAAGER_QA_AI_PERF_ONLY === "1") {
      await verifyLiveKhodAiPerformance(page);
      console.log("[qa] live KHOD WHAAT AI performance verification complete");
      return;
    }
    ...
    const sizes = SMALL_LAPTOP_MODE ? (EXHAUSTIVE_WIDTHS ? [
      ...
      ["laptop-1100x720", { width: 1100, height: 720 }],
    ]) : DASHBOARD_SCREENSHOT_MODE ? [
      ["dashboard-laptop-ar", { width: 1100, height: 720 }],
    ] : [
    ...
    if (SMALL_LAPTOP_MODE) {
      const laptopSectionIds = selectedDashboardSections([
        "master",
        "overview",
        "pipeline",
        "orders",
      ...
      for (const sectionId of laptopSectionIds) {
        await page.locator(`.dash-nav-btn[data-section="${sectionId}"]`).evaluate((button) => button.click());
        await waitForDashboardSectionReady(page, sectionId);
    ...
    const sectionIds = selectedDashboardSections([
      "master",
      "overview",
      "pipeline",
      "orders",
    ...
    for (const sectionId of sectionIds) {
      await page.evaluate((id) => {
        ...
        if (!button) throw new Error("Dashboard section button not found: " + id);
        button.click();
      }, sectionId);
      await waitForDashboardSectionReady(page, sectionId);
      if (DASHBOARD_SCREENSHOT_MODE && !EXHAUSTIVE_WIDTHS) {
        await captureDashboardSection(page, sectionId, { width: 1100, height: 720 });
        await inspect(page, "dashboard-section-" + sectionId + "-laptop", { width: 1100, height: 720 });
      } else {
    ...
    await verifyDashboardDataStates(page);
    if (RUN_LIVE_AI_INTERACTIONS) await verifyLiveKhodAiSmoke(page);
    else console.log("[qa] skipping live AI prompts during deterministic visual QA");
    if (RUN_REACTIVE_INTERACTIONS) {
      await verifyProductReactiveInteractions(page);
      await verifyOrdersDetailInteractions(page);
    } else {
      console.log("[qa] skipping optional reactive interaction suite during visual QA");
    }
    ...
    if (pageErrors.length) {
      throw new Error("Dashboard renderer emitted uncaught JavaScript errors: " + pageErrors.slice(0, 5).join(" | "));
    }
  }
})().then(() => {
  process.exit(0);
