...
(async () => {
  ...
  try {
    ...
    assert.equal(await flow.ensureEnglish(page, { force: true }), true, "forced fallback keeps an already-English page stable");
    assert.equal(await page.evaluate(() => window.englishClicks), 1, "English menu item is not reselected after verification");
