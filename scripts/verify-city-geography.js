...
const geoStart = appSource.indexOf("window.KhodGeo = (() => {");
...
Object.entries(EXPECTED_COUNTS).forEach(([country, expectedCount]) => {
  ...
  const polygons = outlinePolygons(geoContext.window.KhodGeo.outline(country));
  cities.forEach((city) => {
...
assert.ok(citiesSource.includes("KhodGeo.officialCities"), "Cities map renders official baseline cities");
assert.ok(codSource.includes("KhodGeo.officialCities"), "COD map renders official baseline cities");
