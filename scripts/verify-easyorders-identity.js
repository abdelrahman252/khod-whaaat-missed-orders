...
const nestedStatusMenuPopover = `
  <div role="presentation">
    <div class="MuiPaper-root MuiMenu-paper">
      <div>
        <p>1dopamine <span aria-label="active store">?</span></p>
        <span>am4243696@gmail.com</span>
      </div>
      <h6>Stores:</h6>
      <button><span>other store</span></button>
      <button><span>1dopamine</span></button>
      <button>Sign Out</button>
    </div>
  </div>
`;
...
function simulatedEasyOrdersApp(scenario) {
  const data = JSON.stringify({
    ...
    headerStaysUnreadableAfterSelection: scenario.headerStaysUnreadableAfterSelection === true,
    exposeEmailInStorage: scenario.exposeEmailInStorage === true,
    selectionDoesNotChange: scenario.selectionDoesNotChange === true,
    selectionFailuresRemaining: Number(scenario.selectionFailuresRemaining || 0),
      ...
      <body>
        <script>
          ...
          function dashboardMarkup() {
            ...
            const readable = scenario.headerReadable || (!!selectedStore && !scenario.headerStaysUnreadableAfterSelection);
            if (scenario.exposeEmailInStorage) localStorage.setItem("authUserEmail", scenario.email);
            return \`
          ...
          function activateStore(store) {
            const selectionAttempt = Number(localStorage.getItem("selectionAttempt") || 0) + 1;
            localStorage.setItem("selectionAttempt", String(selectionAttempt));
            if (selectionAttempt <= scenario.selectionFailuresRemaining) return;
...
async function runFlowScenario(browser, scenario) {
  ...
  const flow = createEasyOrdersExportFlow({
    ...
    storeSelectionNavigationTimeoutMs: 1200,
  });
...
(async () => {
  ...
  try {
    ...
    assert.deepStrictEqual(
      await parseFixture(page, nestedStatusMenuPopover),
      {
        email: "am4243696@gmail.com",
        store: "1dopamine",
        source: "account-popover-header",
      },
      "header dropdown identity reads visible store text even when the store label contains a child status icon"
    );
...

    {
      const scenario = await runFlowScenario(browser, {
        ...
        initialStore: "Unknown Header",
        headerReadable: false,
        headerStaysUnreadableAfterSelection: true,
        exposeEmailInStorage: true,
      });
      try {
        await scenario.flow.login(scenario.page);
        assert.equal(
          await scenario.page.evaluate(() => localStorage.getItem("activeStore")),
          "North Star Outlet",
          "explicit store selection is accepted when the header stays unreadable but authenticated email evidence remains visible"
        );
        assert(
          scenario.events.some((event) => event.event === "identity-verified" && event.store === "North Star Outlet"),
          "fallback verification still emits identity-verified for the configured store"
        );
      } finally {
        await scenario.context.close();
      }
...

    {
      ...
      try {
        ...
        assert.equal(
          await scenario.page.evaluate(() => localStorage.getItem("activeStore")),
          "North Star Outlet",
          "a transient store-selection failure is recovered by reload and retry"
        );
        assert.equal(
          Number(await scenario.page.evaluate(() => localStorage.getItem("selectionAttempt"))),
          2,
          "store recovery retries the configured store after the first click stalls"
        );
      } finally {
        await scenario.context.close();
      }
    ...
    const khodRunnerSource = fs.readFileSync(require.resolve("../src/bot/khod-flow-runner"), "utf8");
    ...
    assert(!khodRunnerSource.includes("texts.slice(storeHeadingIndex + 1)"), "legacy KHOD runner available-store list fallback was removed");
    assert(
    ...
    assert(
      runnerSource.includes('require("./khod-flow-runner")') &&
      khodRunnerSource.includes("await easyOrdersFlow.login(page);") &&
      khodRunnerSource.includes("easyOrdersFlow.exportReport(page, exportFromDate, \"orders\")") &&
      khodRunnerSource.includes("easyOrdersFlow.exportReport(page, exportFromDate, \"missed-orders\")") &&
      khodRunnerSource.includes("await easyOrdersFlow.assertSession(page);"),
      "normal runner login, exports, and session guards use the shared EasyOrders flow"
    );
    assert(
      !dashboardSource.includes("easyOrdersFlow") &&
      dashboardSource.includes("KHOD WHAAT credentials missing for this account"),
      "dashboard-only refresh remains KHOD-only and does not invoke the EasyOrders flow"
    );
