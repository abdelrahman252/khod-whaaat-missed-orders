...
var dash = read('dashboard.js');
var deliveredModeHandler = (dash.match(/function handleDeliveredDateModeChange\(\) \{[\s\S]*?\n    \}/) || [''])[0];
...
ok('cod includes totalCitiesCount',
  agg.includes("totalCitiesCount: Object.keys(cityStats).length") ||
  agg.includes("get totalCitiesCount()"));
...
ok('created mode outcome scope can use delivered predicate', agg.includes('filterOutcomeOrders(displayRows, meta.period') && agg.includes("meta.deliveredDateMode || 'actual'"));
ok('mode changes do not trigger marketing sync', deliveredModeHandler.includes('runAggregator(true);') && !deliveredModeHandler.includes('triggerMarketingSync'));

// ── 3. section6: no hardcoded مايو in FALLBACK ──────────────────────────────
console.log('\n[3] Section6 — dynamic month labels');
var s6 = read('sections/section6-commission.js') + '\n' + read('sections/section6-commission-hydrated.js');
...
var s4 = read('sections/section4-cod.js') + '\n' + read('sections/section4-cod-hydrated.js');
ok('No hardcoded "94 طلب"',             !s4.includes('94 طلب'));
ok('deliveredCount used in payment methods', s4.includes('D.deliveredCount'));
ok('COD section uses lightweight hydration wrapper', s4.includes('renderSection4HydratedEntry') && s4.includes("ensureFeatureScripts('dashboardCodHydrated')"));
...
var s7 = read('sections/section7-calculator.js') + '\n' + read('sections/section7-calculator-hydrated.js');
...
ok('calculator directs marketing filtering through dashboard period', s7.includes('s7-sync-period') && s7.includes('Selecting a period from the top dashboard bar'));
ok('calculator persists ROI changes', s7.includes('persistCalculatorSettings') && s7.includes('DashboardRoiState.set'));
ok('calculator section uses lightweight hydration wrapper', s7.includes('renderSection7HydratedEntry') && s7.includes("ensureFeatureScripts('dashboardCalculatorHydrated')"));
...
var shared = read('dashboard-shared.js');
var s5 = read('sections/section5-products.js') + '\n' + read('sections/section5-products-hydrated.js');
var s9 = read('sections/section9-product-forecast.js') + '\n' + read('sections/section9-product-forecast-hydrated.js');
var cities = read('sections/section-cities.js') + '\n' + read('sections/section-cities-hydrated.js');
...
ok('delivered attribution state validates and persists mode', bus.includes('DashboardDeliveredDateState') && bus.includes('khod_dashboard_delivered_date_mode') && bus.includes('normalizeDeliveredDateMode') && bus.includes("LAST_UPDATED: 'lastUpdated'") && bus.includes("value === 'updatedAt') return DELIVERED_DATE_MODES.LAST_UPDATED"));
ok('topbar includes delivered attribution selector', shell.includes('dashboard-delivered-date-select-wrap') && shell.includes('deliveredDateOptions') && shell.includes('onDeliveredDateModeChange') && shell.includes("value: 'lastUpdated'"));
ok('KPI cards render truthful values before optional animation',
  !shared.includes("? displayValue : '0'") &&
  shared.split('+ displayValue +').length - 1 >= 2);
ok('Rates and update controls have a registered refresh icon',
  shared.includes('refreshCw:') && shell.includes("icon('refreshCw', 'currentColor')"));
ok('dashboard section mapping preserves authoritative financials',
  dashboard.includes('financials: result.financials || null'));
...
ok('product section uses lightweight hydration wrapper', s5.includes('renderSection5HydratedEntry') && s5.includes("ensureFeatureScripts('dashboardProductsHydrated')"));
...
ok('product forecast uses lightweight hydration wrapper', s9.includes('renderSectionProductForecastHydratedEntry') && s9.includes("ensureFeatureScripts('dashboardForecastHydrated')"));
...
ok('cities section uses lightweight hydration wrapper', cities.includes('renderSectionCitiesHydratedEntry') && cities.includes("ensureFeatureScripts('dashboardCitiesHydrated')"));
ok('new labels and help text are centralized', localeEn.includes("'products.pnlHelp'") && localeAr.includes("'products.pnlHelp'"));
ok('delivered attribution labels are centralized', localeEn.includes("'deliveredDate.updatedAt'") && localeAr.includes("'deliveredDate.createdAt'") && localeEn.includes("'deliveredDate.lastUpdated'") && localeAr.includes("'deliveredDate.lastUpdated'"));
...
ok('marketing and campaign sections use lightweight hydration wrappers',
  marketing.includes('renderSectionMarketingConnectionsHydratedEntry') &&
  marketing.includes("ensureFeatureScripts('dashboardMarketingHydrated')") &&
  campaigns.includes('renderSectionCampaignsHydratedEntry') &&
  campaigns.includes("ensureFeatureScripts('dashboardCampaignsHydrated')"));
...
ok('dashboard loader resets completed stage percentages before reuse', fs.readFileSync(path.join(__dirname, '..', 'src', 'renderer', 'app.js'), 'utf8').includes('options.reset || state.complete'));
ok('dashboard open and account/period/currency changes auto-sync marketing spend',
  dashboard.includes("syncMarketingAndRunAggregator(false, 'dashboard-open')") &&
  dashboard.includes("syncMarketingAndRunAggregator(true, 'account-change')") &&
  dashboard.includes("syncMarketingAndRunAggregator(true, 'period-change')") &&
  dashboard.includes("syncMarketingAndRunAggregator(true, 'currency-change')") &&
  dashboard.includes('activeMarketingSyncPromise'));
...
ok('single-account marketing view only keeps claimable workspace accounts as diagnostics', marketing.includes('claimableAccountCount') && marketing.includes('assignmentMemberBody') && marketing.includes('assignedAccountsMarkup') && !marketing.includes('data-claimable-account'));
ok('marketing backend can read same KHOD WHAAT assignment across licenses', marketingBackend.includes('sharedAccountId') && marketingBackend.includes('dashboard_account_id: `eq.${sharedId}`') && marketingBackend.includes('claimableAccounts: []'));
...
ok('team marketing view can sync every mapped KHOD WHAAT account', marketing.includes('syncAllMarketingData') && marketing.includes('marketing-sync-all'));
...
ok('marketing sync returns campaign breakdown for product spend',
  ...
  marketingBackend.includes('platformPurchaseFields') &&
...
ok('product forecast uses the shared product-attribution engine', s9.includes('KhodProductAttribution') && s9.includes('createProductIndex') && s9.includes('matchCampaign') && s9.includes('ambiguousRows') && s9.includes('snapchat') && s9.includes('facebook'));
...
var scenarioSpendInput = (s9.match(/<input type="number" class="s9-sim-spend-input[\s\S]*?>/) || [''])[0];
var scenarioSpendHandler = (s9.match(/var simSpendInp =[\s\S]*?var totalOrdersInp =/) || [''])[0];
ok('product forecast keeps marketing real spend while scenarios remain editable/resettable',
  !s9.includes('s9-sync-now') &&
  s9.includes('syncedAdSpend') &&
  scenarioSpendInput &&
  !scenarioSpendInput.includes('disabled') &&
  !scenarioSpendInput.includes('readonly') &&
  !scenarioSpendHandler.includes('syncedAdSpend') &&
  s9.includes('resetSimulationToReal'));
ok('campaign decisions use one structured evaluator', campaignIntel.includes('window.KhodCampaignDecision') && campaignIntel.includes('decisionEngine.evaluate') && campaignDecision.includes('passedChecks') && campaignDecision.includes('failedChecks') && campaignDecision.includes('confidence'));
...
ok('AI scorecards consume the shared campaign evaluator', fs.readFileSync(path.join(__dirname, '..', 'src', 'renderer', 'pages', 'ai-intelligence', 'engine', 'business-orchestrator.js'), 'utf8').includes('window.KhodCampaignDecision'));
...
ok('campaign net profit and ROI use semantic financial colors', campaigns.includes('campaign-financial-') && campaigns.includes('financialState(group.netProfit') && campaigns.includes('financialState(group.roiPct'));
