...
function compareRuntimeSource() {
  ...
  assert.ok(main.includes('dashboardQueryRuntimeFlag("KHOD_DASHBOARD_QUERY_SHADOW", "TAAGER_DASHBOARD_QUERY_SHADOW")'), "Shadow mode defaults off and reads KHOD WHAAT flag before legacy fallback");
  assert.ok(main.includes('dashboardQueryRuntimeFlag("KHOD_DASHBOARD_QUERY_ORDERS", "TAAGER_DASHBOARD_QUERY_ORDERS")'), "Orders rollout flag defaults off and reads KHOD WHAAT flag before legacy fallback");
  assert.ok(main.includes('dashboardQueryRuntimeFlag("KHOD_DASHBOARD_QUERY_PRODUCTS", "TAAGER_DASHBOARD_QUERY_PRODUCTS")'), "Products rollout flag defaults off and reads KHOD WHAAT flag before legacy fallback");
  assert.ok(main.includes('dashboardQueryRuntimeFlag("KHOD_DASHBOARD_QUERY_CAMPAIGNS", "TAAGER_DASHBOARD_QUERY_CAMPAIGNS")'), "Campaigns rollout flag defaults off and reads KHOD WHAAT flag before legacy fallback");
  assert.ok(main.includes('dashboardQueryRuntimeFlag("KHOD_DASHBOARD_QUERY_CITIES", "TAAGER_DASHBOARD_QUERY_CITIES")'), "Cities query prewarm reads KHOD WHAAT flag before legacy fallback");
