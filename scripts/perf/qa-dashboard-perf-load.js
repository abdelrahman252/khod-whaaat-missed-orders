...
/*
  ...
  through window.api.getDashboardSnapshot â€” WITHOUT overriding
  ...
  bar, ~50x the data â€” which is the actual question being asked.
...
const THRESHOLDS = {
  ...
  // machine â€” treat this threshold as a regression guard (did it get much
  ...
  paneCacheMaxEntries: 16,   // Tiger parity: keep every normal dashboard pane warm
  paneCacheMaxChildren: 17   // sixteen cached panes plus at most one transient active loader
};
...
// â”€â”€â”€ Same launch strategy as scripts/qa-dashboard-responsive.js: real
// Electron via Playwright, falling back to a CDP-attached Electron process,
// falling back to a plain Chromium load of index.html. â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function launchElectronForQa() {
  try {
    ...
    await firstPage.waitForLoadState("domcontentloaded", { timeout: 30000 });
    return {
      mode: "electron",
      ...
      installFixture: async (fixture) => {
        ...
        ipcMain.handle("get-credentials", async () => ({
          accounts: [{
            ...
            khodEmail: "perf-khod@example.com",
...
// â”€â”€â”€ window.api mock â€” same noop surface as qa-dashboard-responsive.js, so
// nothing the app calls incidentally throws. Dashboard-relevant calls are
// overridden below with the real fixture. â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Mount the dashboard with the real fixture flowing through the REAL
// aggregator (window.runDashboardAggregator is left untouched). â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function mountDashboardWithRealAggregator(page, fixture) {
  ...
  const apiProbe = await page.evaluate(async ({ accountId, dateFrom, dateTo }) => {
    ...
    try { localStorage.setItem("khod_dashboard_period", JSON.stringify({ preset: "custom", dateFrom, dateTo })); } catch (_) {}
    try { localStorage.setItem("khod_dashboard_expected_ndr_range", JSON.stringify({ dateFrom, dateTo })); } catch (_) {}
    try { localStorage.setItem("khod_dashboard_delivered_date_mode", "actual"); } catch (_) {}
    try { localStorage.setItem("khod_active_account_id", accountId); } catch (_) {}
...
// â”€â”€â”€ Section helpers â€” adapted from verifyDashboardPerformanceAcceptance()
// in qa-dashboard-responsive.js. Same selectors, same thresholds, scaled
// against the 5,000-row fixture instead of 96 hand-built orders. â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function measureCachedSectionRestore(page, sectionId, awaySectionId) {
  if (awaySectionId && awaySectionId !== sectionId) await openSection(page, awaySectionId, "");
  return page.evaluate(async (id) => {
    ...
    while (performance.now() - started < 3000) {
      ...
      if (shell && shell._dashboardActiveSection === id && pane && !pane.hidden) break;
      await new Promise((resolve) => requestAnimationFrame(resolve));
    }
    const entries = window.KhodPerf ? window.KhodPerf.entries() : [];
    const hit = entries.filter((entry) => entry.type === "measure" && entry.name === "dashboard:section:switch" && entry.detail && entry.detail.sectionId === id && entry.detail.cacheHit).pop();
    return { wallClockMs: performance.now() - started, measuredMs: hit ? hit.durationMs : null, cacheHit: !!hit };
  }, sectionId);
}
...
//                       active (the cheap part â€” just routing/cache lookup)
//   - renderFnMs:       the app's own dashboard:section:render measure â€” the
//                       actual render function's execution time
//   - visibleWallClockMs: wall clock until the section's real content
//                       selector appears in the DOM â€” this is the only one
...
async function measureColdRender(page, sectionId, readySelector) {
  await page.evaluate(() => { if (window.KhodPerf && window.KhodPerf.clear) window.KhodPerf.clear(); });
  const genuineRowSelector = sectionId === "products"
    ? ".s5-product-row[data-product-key]"
    : sectionId === "cities"
      ? ".sc-lb-row[data-city][data-lb-filter-match]"
      : "";
  const requiresReadyMarker = !!genuineRowSelector;
  const pageTiming = await page.evaluate(async ({ id, selector, rowSelector, marker }) => {
    const startedAt = performance.now();
    const beforeNodeCount = document.getElementsByTagName("*").length;
    const beforeResources = window.DashboardSectionResources ? window.DashboardSectionResources.snapshot() : [];
    const button = document.querySelector('.dash-nav-btn[data-section="' + id + '"]');
    if (!button) {
      return {
        missingButton: true,
        shellSwitchMs: null,
        visibleWallClockMs: null,
        readyOk: false
      };
    }
    button.click();
    let shellSwitchMs = null;
    while (performance.now() - startedAt < 20000) {
      const mount = document.getElementById("db-shell-mount");
      if (mount && mount._dashboardActiveSection === id) {
        shellSwitchMs = performance.now() - startedAt;
        break;
      }
      await new Promise((resolve) => requestAnimationFrame(resolve));
    }
    if (!selector && !rowSelector) {
      return {
        shellSwitchMs,
        visibleWallClockMs: performance.now() - startedAt,
        readyOk: shellSwitchMs != null
      };
    }
    let readyOk = false;
    while (performance.now() - startedAt < 15000) {
      const mount = document.getElementById("db-shell-mount");
      const pane = mount && mount._dashboardActivePane;
      if (mount && mount._dashboardActiveSection === id && pane && !pane.hidden &&
        (!marker || pane.dataset.dashboardReady === id) &&
        (!rowSelector || pane.querySelector(rowSelector)) &&
        (!selector || pane.querySelector(selector))) {
        readyOk = true;
    ...
    return {
      ...
      readyOk,
      beforeNodeCount,
      afterNodeCount: document.getElementsByTagName("*").length,
      beforeResources,
      afterResources: window.DashboardSectionResources ? window.DashboardSectionResources.snapshot() : []
    };
  }, { id: sectionId, selector: readySelector || "", rowSelector: genuineRowSelector, marker: requiresReadyMarker });
  const shellSwitchMs = pageTiming.shellSwitchMs == null ? null : Math.round(pageTiming.shellSwitchMs);
  const visibleWallClockMs = pageTiming.visibleWallClockMs == null ? null : Math.round(pageTiming.visibleWallClockMs);
  const readyOk = !!pageTiming.readyOk;
  await page.waitForTimeout(120);
  const entries = await page.evaluate((id) => {
    const list = window.KhodPerf ? window.KhodPerf.entries() : [];
  ...
  const longTasks = await page.evaluate(() => (window.KhodPerf ? window.KhodPerf.entries() : []).filter((entry) => entry.type === "longtask"));
  ...
  const loaderState = await page.evaluate((id) => {
    const shell = document.getElementById("db-shell-mount");
    const pane = shell && shell._dashboardActivePane;
    return {
      activeSection: shell && shell._dashboardActiveSection || "",
      livePreloaders: document.querySelectorAll("#dashboard-live-preloader").length,
      activePaneHydrating: pane ? pane.querySelectorAll("[data-dashboard-hydrating]").length : 0,
      activePaneReady: pane ? pane.dataset.dashboardReady || "" : "",
      matchesRequestedSection: !!(shell && shell._dashboardActiveSection === id && pane && !pane.hidden)
    };
  }, sectionId);
  return {
    sectionId, shellSwitchMs, renderFnMs, lazyLoadMs, phaseTimings, visibleWallClockMs, readyOk,
    domNodesBefore: pageTiming.beforeNodeCount,
    domNodesAfter: pageTiming.afterNodeCount,
    domNodeDelta: pageTiming.afterNodeCount - pageTiming.beforeNodeCount,
    resourcesBefore: pageTiming.beforeResources,
    resourcesAfter: pageTiming.afterResources,
    loaderState, longTaskCount: longTasks.length, maxLongTaskMs
  };
}
...
async function main() {
  ...
  if (!fs.existsSync(FIXTURE_PATH)) {
    console.log("[perf] Fixture not found at " + FIXTURE_PATH + " â€” generating it now (defaults: 5000 orders / 100 products)...");
    try {
      require("./build-perf-fixture.js");
    } catch (err) {
      report.fatalError = "Fixture generation failed: " + (err && err.message ? err.message : String(err || ""));
      failures.push(report.fatalError);
      const reportPath = writePerfReport(report, failures);
      console.error("[perf] " + report.fatalError);
  ...
  try {
    ...
    report.fatalError = "Fixture load failed: " + (err && err.message ? err.message : String(err || ""));
    failures.push(report.fatalError);
    const reportPath = writePerfReport(report, failures);
    console.error("[perf] " + report.fatalError);
    console.log("Full report: " + reportPath);
  ...
  report.fixture = { name: FIXTURE_NAME, orderCount: fixture.orderCount, productCount: fixture.productCount, seed: fixture.seed };

  let app = null;
  let page = null;
  const beforeLaunchElectronPids = listRepoElectronPidsSync();

  try {
    app = await launchElectronForQa();
    report.launchMode = app.mode;
    ...
    await page.evaluate(async () => {
      if (typeof window.prepareDashboardSections === "function") {
        await window.prepareDashboardSections();
      }
    });
    report.sectionPreparationMs = Date.now() - preparationStarted;
    console.log(`[perf] one-time section preparation: ${report.sectionPreparationMs}ms`);
    ...
    // â”€â”€ Real aggregation + shell-mount timing, read from the app's own
    // instrumentation (window.KhodPerf), not invented here. â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    const coreEntries = await page.evaluate(() => (window.KhodPerf ? window.KhodPerf.entries() : []));
    ...
    report.aggregationPhases = await page.evaluate(() => {
      ...
      const perfPhases = window.KhodPerf
        ? window.KhodPerf.entries().filter((entry) => entry.type === "measure" && String(entry.name || "").indexOf("dashboard:aggregation:phase:") === 0)
        : [];
      return {
    ...
    const initialSnapshot = await page.evaluate(() => (window.KhodPerf ? window.KhodPerf.snapshot("after-initial-load") : null));
    ...
    // â”€â”€ Cold render of every requested section, against the full fixture â”€â”€
    const sectionsToTest = [
      { id: "overview", selector: "#s1-root" },
      { id: "pipeline", selector: ".s2-body" },
      { id: "orders", selector: "#s3-search" },
      { id: "cod", selector: ".s4-cod-overview-card" },
      { id: "products", selector: "#s5-search" },
      { id: "cities", selector: "#sc-fb-search" },
      { id: "commission", selector: "#s6-area-chart" },
      { id: "marketing", selector: ".marketing-section" },
      { id: "campaigns", selector: ".campaign-section" },
      { id: "calculator", selector: "#s7-in-budget" },
      { id: "gmvTarget", selector: ".gmv-body" },
      { id: "productForecast", selector: "#s9-product-search" },
      { id: "prepaid", selector: ".dash-topbar" },
      { id: "staticUpdate", selector: ".static-update-section" },
      { id: "taagerAi", selector: ".khod-ai-section", cacheable: false }
    ...
    for (const section of sectionsToTest) {
      ...
      console.log(`[perf] cold render â€” ${section.id}: shellSwitch=${result.shellSwitchMs}ms lazyLoad=${result.lazyLoadMs == null ? "n/a" : result.lazyLoadMs + "ms"} renderFn=${result.renderFnMs == null ? "n/a" : result.renderFnMs + "ms"} visibleWallClock=${result.visibleWallClockMs}ms readyOk=${result.readyOk}`);
      if (!result.readyOk) {
      ...
      if (result.loaderState && (result.loaderState.livePreloaders || result.loaderState.activePaneHydrating)) {
        failures.push(`${section.id} left dashboard preloaders mounted after ready: ${JSON.stringify(result.loaderState)}`);
      }
    }

    // â”€â”€ Cached restore â€” full runs only; targeted runs stay focused/short â”€â”€
    report.cachedRestores = {};
    for (let index = 0; index < sectionsToTest.length; index += 1) {
      const section = sectionsToTest[index];
      if (section.cacheable === false) continue;
      const away = sectionsToTest[(index + 1) % sectionsToTest.length];
      const restore = await measureCachedSectionRestore(page, section.id, away && away.id);
      report.cachedRestores[section.id] = restore;
      console.log(`[perf] cached restore - ${section.id}: measured=${restore.measuredMs == null ? "n/a" : Math.round(restore.measuredMs) + "ms"} wall=${Math.round(restore.wallClockMs || 0)}ms cacheHit=${!!restore.cacheHit}`);
      if (!restore.cacheHit) failures.push(`${section.id}: cached restore did not use the pane cache`);
      if (restore.measuredMs != null && restore.measuredMs > THRESHOLDS.cachedRestoreMs) {
        failures.push(`${section.id} cached restore took ${Math.round(restore.measuredMs)}ms (limit ${THRESHOLDS.cachedRestoreMs}ms)`);
      }
    }
    ...
    }, null, { timeout: 15000 }).catch(() => console.log("[perf] warning: overview pane never appeared in the section cache â€” cache-hit check may be unreliable"));
    ...
    if (cachedRestoreMs == null) {
      failures.push("Cached section restore measurement missing â€” could not confirm cache-hit path");
    ...
    // â”€â”€ Search / input interaction latency at full data volume â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    report.interactions = {};
    ...
    // â”€â”€ Stability / leak check across rapid section switching at full load â”€
    if (!SKIP_STABILITY) {
    await openSection(page, "master", ".s8-body");
    const stability = await withTimeout(
      page.evaluate((count) => window.KhodPerf.runDashboardSectionStabilityCheck(
        ["overview", "pipeline", "orders", "cod", "products", "cities", "commission", "marketing", "campaigns", "calculator", "gmvTarget", "productForecast", "prepaid", "staticUpdate"],
        count
      ), SWITCH_COUNT),
    ...
    const settled = await page.evaluate(() => {
      ...
      return {
        snapshot: window.KhodPerf.snapshot("settled-after-50-section-switches"),
        activeSection: mount && mount._dashboardActiveSection || "",
        resources: window.DashboardSectionResources ? window.DashboardSectionResources.snapshot() : [],
    ...
    const activeSectionId = settled.activeSection || "";
    const busyHiddenResources = (settled.resources || []).filter((resource) =>
      resource.sectionId && resource.sectionId !== activeSectionId &&
      (resource.active || resource.timers || resource.rafIds || resource.idleIds || resource.listeners || resource.observers)
    );
    if (busyHiddenResources.length) failures.push("Inactive panes retained active resources: " + JSON.stringify(busyHiddenResources));
    ...
    // â”€â”€ Long tasks captured throughout the whole run (auto-instrumented) â”€â”€
    const longTasks = await page.evaluate(() => (window.KhodPerf ? window.KhodPerf.entries() : []).filter((e) => e.type === "longtask"));
    ...
    if (!app) stopNewRepoElectronPidsSync(beforeLaunchElectronPids);
    console.error("[perf] a step failed mid-run â€” writing the partial report anyway:", err.message);
    ...
    failures.push("Run aborted mid-way (" + err.message + ") â€” every measurement above this point in the console output is still valid, later steps did not run.");
  } finally {
