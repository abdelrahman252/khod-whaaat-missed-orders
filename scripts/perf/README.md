...
Your existing `qa-dashboard-performance.js` (â†’ `qa-dashboard-responsive.js`'s
...
(`buildDashboardResult()`), so it's fast and deterministic â€” but it never
...
expects â€” cloned from `src/renderer/pages/premium-preview.js`'s `makeOrder()`,
...
- Orders spread across the real 13 Saudi provinces/cities from
  `window.KhodGeo` (`src/renderer/app.js`), weighted toward Riyadh/Eastern/
  Mecca.
- Status funnel reuses the exact strings from `premium-preview.js` (proven to
  normalize correctly via `window.KhodStatus`), weighted per product tier
...
- Dates: ~85% inside the **current calendar month**, ~15% in the prior month.
  This matters â€” the dashboard's default period is `"thisMonth"`
...
Same seed â†’ same fixture, so before/after runs are comparable.
...
2. Reads the app's **own** instrumentation (`window.KhodPerf` â€”
   already shipped in `app.js`) rather than inventing a separate timing
   scheme:
   - `dashboard:data:aggregation` â€” the real Pass-1 aggregation pass.
   - `dashboard:shell:mount`, `dashboard:section:render`,
     `dashboard:section:switch` (with `detail.cacheHit`).
   - `window.KhodPerf.entries().filter(e => e.type === "longtask")` â€”
     real >50ms main-thread blocks, auto-captured via `PerformanceObserver`.
   - `window.KhodPerf.snapshot()` â€” DOM node count + JS heap.
3. Cold-renders every section you asked about (orders, products, cities,
   marketing, calculator, productForecast, khodAi), reporting separate
   numbers per section because they answer different questions:
   - `shellSwitchMs` â€” time until the shell flags the section active
   - `lazyLoadMs` â€” loading time for the section's lazy script group
   - `renderFnMs` â€” the render function's own execution time
     (`dashboard:section:render`)
   - `visibleWallClockMs` â€” true wall clock until real content appears,
     which is the only one of the three that includes a first-ever-visit
     lazy section-script fetch (`ensureDashboardSection`) â€” something the
...
4. Re-runs your existing acceptance bar at this scale: empty-search update
   ...
   (100ms), and the 50-rapid-switch stability/leak check (â‰¤500 node growth,
   â‰¤25MB heap growth, six configured cache entries, at most one transient
...
5. Writes a full JSON report to `.codex-tmp/perf-reports/` and exits 1 if
   anything exceeded threshold â€” **and still writes the full partial report
   even if a later step crashes**, so a problem in section 9 doesn't erase
   what you already learned about sections 3â€“7.
...
- All cold sections became ready in **91-223ms**, including KHOD WHAAT AI at
  **188ms**.
...
- All cold sections became ready in **80-469ms**, including KHOD WHAAT AI at
  **237ms**.
...
fixture before delivering this (details below) â€” **but only on a 1-vCPU /
...
- **Real aggregation pass: ~1.2â€“1.6s.** This is the one finding I'd flag
  hardest, because my original complexity-based estimate ("sub-100ms, single
  O(n) pass") undersold it â€” actual per-row work (date parsing, regex status
  ...
  per-interaction cost â€” but it's not instant either. Worth re-measuring on
  your own machine; a multi-core desktop will likely cut this down
  meaningfully, but I wouldn't assume it disappears entirely.
- **First-ever visit to a section can take 0.5â€“3s+ of wall clock**, on top
  ...
  varied run to run in my testing (not the same section twice) â€” consistent
  ...
  current one.
- **Cached section restores were fast** (single-digit ms) every time â€”
...
- **One run showed an orders-section search taking >3s to filter down to
  ...
  saturating. I didn't want to either bury this or call it a confirmed bug â€”
...
- The script itself crashed mid-run once (browser process died) on that same
  1-vCPU/4GB box during the 50-switch stability check â€” almost certainly
...
None of this overturns the original audit's conclusion â€” the architecture
