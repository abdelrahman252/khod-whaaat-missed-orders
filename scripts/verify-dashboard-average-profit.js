...
async function runAggregator(rows) {
  const storage = {
    getItem: (key) => key === "khod_active_account_id" ? "__all__" : null,
    setItem: () => {},
  };
  const window = {
    _kbotLang: "en",
...
(async function verifyAverageProfitContract() {
  const deliveredRows = [
    ...
    row("D-CREATED-IN", "delivered", { lastUpdatedAt: "2026-07-02", profitAfterTax: 30 }),
    row("D-UPDATED-IN", "delivered", { createdAt: "2026-05-30", lastUpdatedAt: "2026-06-07", profitAfterTax: 90 }),
    row("C-1", "confirmed", { profit: 500, taxProfit: 0 }),
  ];
  const dashboard = await runAggregator(deliveredRows);
  assert.equal(dashboard.pipeline.metrics.businessDeliveredCount, 3);
  assert.equal(dashboard.overview.earnedCommission.value, 150);
  assert.equal(dashboard.roi.actualDeliveredCount, 3);
  assert.equal(dashboard.roi.actualEarnedProfitAfterTax, 150);
  assert.equal(dashboard.roi.averageProfit, 50);
  assert.equal(dashboard.roi.avgCommission, 50);
  assert.equal(dashboard.products.rankedList[0].actualDeliveredCount, 3);
  assert.equal(dashboard.products.rankedList[0].actualCommission, 150);
  assert.equal(
    dashboard.orders.some((order) => order.taagerOrderNumber === "D-UPDATED-IN"),
    false,
    "Created-date cohort excludes delivered orders created outside the selected period even if lastUpdatedAt is inside"
  );
  assert.equal(
    dashboard.orders.some((order) => order.taagerOrderNumber === "D-CREATED-IN"),
    true,
    "Created-date cohort includes delivered orders created inside the selected period even if lastUpdatedAt is outside"
  );
  ...
  assert.equal(products.rows[0].deliveredCount, 3);
  assert.equal(products.rows[0].commission, 150);
  assert.equal(products.rows[0].actualDeliveredCount, 3);
  assert.equal(products.rows[0].actualCommission, 150);
  assert.equal(products.rows[0].averageProfit, 50);
  ...
  assert.equal(expectedProducts.rows[0].actualDeliveredCount, 3);
  assert.equal(expectedProducts.rows[0].actualCommission, 150);
  assert.equal(expectedProducts.rows[0].averageProfit, 50, "Expected mode preserves the created-period delivered-order average profit");
