...
assert.equal(match("Intensive Skin Launch", nameProducts).status, "unmatched");
assert.equal(match("Skin Campaign", nameProducts).status, "unmatched");
assert.equal(
  match(
    "SA050402EAATSLH99 Electric Scissors with One Year Warranty",
    [{ key: "vacuum", sku: "SA050106WA0099", name: "Cordless Vacuum with One Year Warranty" }]
  ).matchDetail,
  "unknown_sku",
  "An unknown SKU must not fall through to shared product-name words"
);
assert.equal(
  match(
    "Electric Scissors with One Year Warranty",
    [{ key: "vacuum", sku: "SA050106WA0099", name: "Cordless Vacuum with One Year Warranty" }]
  ).status,
  "unmatched",
  "Shared name words must not allocate spend without the complete product name"
);
