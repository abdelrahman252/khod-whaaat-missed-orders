...
function createContext() {
  ...
  context.window.KhodDashboardFinancialCore =
    context.window.KhodDashboardFinancialCore || context.KhodDashboardFinancialCore;
  context.window.KhodAiScenarioDatabase = context.window.KhodAiScenarioDatabase;
  context.window.KhodAiSessionMemory = context.window.KhodAiSessionMemory;
  context.window.KhodAiIntentDetector = context.window.KhodAiIntentDetector;
  context.window.KhodAiAnalyticsEngine = context.window.KhodAiAnalyticsEngine;
  context.window.KhodAiBusinessOrchestrator = context.window.KhodAiBusinessOrchestrator;
  return context.window;
}

function assertUiContracts(failures) {
  const assistantSection = fs.readFileSync(path.join(ROOT, "src/renderer/pages/dashboard/sections/section-khod-ai.js"), "utf8");
  ...
  assert(/DashboardAiMirror\.warm/.test(dashboardPage), "dashboard data lifecycle must warm the AI mirror after refresh", failures, { id: "mirror-lifecycle", category: "routing", userMessage: "" });
    assert(/rankingRoute\s*&&\s*\(rankingRoute\.rankingRequest\s*\|\|\s*preferMirrorKpi\)\s*\?\s*rankingRoute\s*:\s*orchestration/.test(assistantSection), "live KHOD WHAAT AI section must prefer mirror rankings and account KPI answers before returning orchestrator-local copy", failures, { id: "live-mirror-first-ranking", category: "routing", userMessage: "" });
  assert(/validScalePlanAnswer/.test(assistantSection) && /route\.selectedSlice === "plan" && !validScalePlanAnswer/.test(assistantSection), "live KHOD WHAAT AI section must keep the local plan when Gemini misses the required plan shape", failures, { id: "live-plan-shape-guard", category: "content", userMessage: "" });
  assert(/validBusinessDiagnosisAnswer/.test(assistantSection) && /genericBusinessAnswer/.test(assistantSection), "live KHOD WHAAT AI section must reject generic or metric-free Gemini diagnosis answers", failures, { id: "live-diagnosis-shape-guard", category: "content", userMessage: "" });
  assert(/normalizeMetricSpacing/.test(assistantSection) && /NDR\|CPA/.test(assistantSection), "live KHOD WHAAT AI section must normalize NDR and CPA spacing", failures, { id: "live-metric-spacing", category: "content", userMessage: "" });
  assert(/KhodCampaignDecision/.test(mirrorSource) && /computeRiskScore/.test(mirrorSource) && /computeScalingScore/.test(mirrorSource), "AI mirror must reuse shared dashboard scoring and campaign decision rules", failures, { id: "mirror-shared-scoring", category: "routing", userMessage: "" });
...
function assertReliabilityOverhaul(win, failures) {
  ...
  const lowest = win.KhodAiBusinessOrchestrator.orchestrate("Which cities have the lowest NDR?", data);
  ...
  [
    ...
    const ranked = win.KhodAiBusinessOrchestrator.orchestrate(question, data);
  ...
  const highest = win.KhodAiBusinessOrchestrator.orchestrate("Which cities have the highest NDR?", data);
  ...
  const highestRisk = win.KhodAiBusinessOrchestrator.orchestrate("Which cities have the highest risk?", data);
  ...
  const bestCity = win.KhodAiBusinessOrchestrator.orchestrate("What is my best city?", data);
  ...
  const parsedArabicPlan = win.KhodAiIntentDetector.parse("\u0627\u0628\u0646\u0650 \u062e\u0637\u0629 \u062a\u0648\u0633\u0639", data, {});
  const parsedEnglishPlan = win.KhodAiIntentDetector.parse("Build a scale plan", data, {});
  ...
  const arabicUi = win.KhodAiBusinessOrchestrator.orchestrate("Which cities have the lowest NDR?", data);
  ...
  const arabicBestCity = win.KhodAiBusinessOrchestrator.orchestrate("What is my best city?", data);
  ...
  const arabicKpi = win.KhodAiBusinessOrchestrator.orchestrate("What is my account CPA?", data);
  ...
  const arabicStrategy = win.KhodAiBusinessOrchestrator.orchestrate("Why is my account losing?", data);
  ...
  const arabicQuestion = win.KhodAiBusinessOrchestrator.orchestrate("ما المدن ذات أقل NDR؟", data);
...
function validateAnswerShape(shape, answer, route, options) {
  ...
  if (shape === "diagnosis") {
    return /\b(?:NDR|CPA)\b|الربح|الخسار/i.test(answer) &&
      ...
      !/\bI am KHOD WHAAT AI\b|How can I assist|أستطيع مساعدتك|أنا KHOD WHAAT AI/i.test(answer);
  }
...
function runQuestionBank(win, scenarios, mode) {
  ...
  scenarios.forEach((scenario) => {
    if (win.KhodAiSessionMemory && win.KhodAiSessionMemory.clear) win.KhodAiSessionMemory.clear();
    ...
    const parsed = win.KhodAiIntentDetector.parse(scenario.userMessage, data, {});
    const result = win.KhodAiBusinessOrchestrator.orchestrate(scenario.userMessage, data);
...
function assertConversationContinuity(win, failures) {
  ...
  let first = win.KhodAiBusinessOrchestrator.orchestrate("Why is Product X losing?", data);
  ...
  let resumed = win.KhodAiBusinessOrchestrator.orchestrate("500 SAR", data);
  ...
  let cityFollowUp = win.KhodAiBusinessOrchestrator.orchestrate("What about Alexandria?", data);
...
function assertCityHealthOrdering(win, failures) {
  ...
  const result = win.KhodAiAnalyticsEngine.processIntent({
    intent: "ACCOUNT_HEALTH_CHECK",
...
function assertLiveMutationSync(win, failures) {
  ...
  const first = win.KhodAiBusinessOrchestrator.orchestrate("Top products by commission", data);
  ...
  const afterProductMutation = win.KhodAiBusinessOrchestrator.orchestrate("Top products by commission", data);
  ...
  const afterCityMutation = win.KhodAiBusinessOrchestrator.orchestrate("What about Riyadh?", data);
...
function assertProductRankingAndCpaContinuity(win, failures) {
  ...
  const accountQuestion = win.KhodAiBusinessOrchestrator.orchestrate("Why is NDR weak and what should I change next week?", data);
  ...
  const worst = win.KhodAiBusinessOrchestrator.orchestrate("what is my worst app?", data);
  ...
  const cpa = win.KhodAiBusinessOrchestrator.orchestrate("what is my cpa for it?", data);
  ...
  const expectedMode = win.KhodAiBusinessOrchestrator.orchestrate("what is my cpa for it?", data);
...
function assertArabicFuzzyProductCpa(win, failures) {
  ...
  const missingSpend = win.KhodAiBusinessOrchestrator.orchestrate("wht is my cpa fr مضخة مياه للغسيل للسيارة مع 2 بطارية الوكيل", data);
  ...
  const resumed = win.KhodAiBusinessOrchestrator.orchestrate("400 SAR", data);
...
function assertProductNameResolutionEdges(win, failures) {
  ...
  const partial = win.KhodAiBusinessOrchestrator.orchestrate("what is cpa for مياه للغسيل للسيارة", data);
  ...
  const partialResume = win.KhodAiBusinessOrchestrator.orchestrate("800 SAR", data);
  ...
  win.KhodAiSessionMemory.clear();
  const ambiguous = win.KhodAiBusinessOrchestrator.orchestrate("what is my cpa for مضخة", data);
  ...
  const ranking = win.KhodAiBusinessOrchestrator.orchestrate("highest CPA products", data);
...
function assertAccountCpaAnswers(win, failures) {
  ...
  const missing = win.KhodAiBusinessOrchestrator.orchestrate("what is my account CPA?", data);
  ...
  const answered = win.KhodAiBusinessOrchestrator.orchestrate("what is my account CPA?", data);
...
function assertAssistantBusinessHealthProof(win, failures) {
  ...
  const loss = win.KhodAiBusinessOrchestrator.orchestrate("why am i losing?", data);
  ...
  const next = win.KhodAiBusinessOrchestrator.orchestrate("what should I do?", data);
  ...
  const steps = win.KhodAiBusinessOrchestrator.orchestrate("help me do it step by step", data);
...
function assertArabicAssistantFollowUps(win, failures) {
  ...
  const first = win.KhodAiBusinessOrchestrator.orchestrate("ليه بخسر؟", data);
  ...
  const explicitWeakCities = win.KhodAiBusinessOrchestrator.orchestrate("ما أضعف المدن؟", data);
  ...
  win.KhodAiSessionMemory.clear();
  win.KhodAiBusinessOrchestrator.orchestrate("ليه بخسر؟", data);
  const follow = win.KhodAiBusinessOrchestrator.orchestrate("اعمل ايه", data);
...
function assertUnicodeArabicProductResolution(win, failures) {
  ...
  win.KhodAiSessionMemory.clear();
  const uniquePartial = win.KhodAiBusinessOrchestrator.orchestrate("what is cpa for \u0645\u064a\u0627\u0647 \u0644\u0644\u063a\u0633\u064a\u0644 \u0644\u0644\u0633\u064a\u0627\u0631\u0629", data);
...
async function run() {
  ...
  const scenarios = win.KhodAiScenarioDatabase.defaultScenarios;
  const failures = [];
  const counts = {};
  ...
  for (const scenario of mode === "full" ? scenarios : []) {
    ...
    const parsed = win.KhodAiIntentDetector.parse(scenario.userMessage, data, win.KhodAiSessionMemory.get());
    const analytics = win.KhodAiAnalyticsEngine.processIntent(parsed, data);
    const result = win.KhodAiBusinessOrchestrator.orchestrate(scenario.userMessage, data);
