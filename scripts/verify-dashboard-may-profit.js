...
// Deprecated in Phase 3.12: the old body below is retained as historical test
// documentation, but its removed May donor workbook, KHOD-only lifecycle
// buckets, and profit-after-tax expectations are not a valid KHOD WHAAT contract.
if (true) {
  console.log('[DEPRECATED] May-profit dashboard fixture retired; running KHOD WHAAT financial verification.');
  require('./verify-actual-mode-financials');
} else {
...
function verifySection8TopProductsProfitBinding() {
  ...
  assert(
    /earnedProfitAfterTax\s*=\s*productCommissionValue\(p\)/.test(source),
    'Section 8 Top Products binds Earned Profit After Tax to normalized product commission'
  );
  assert(
...
function verifyDeliveredUpdatedDateParsing() {
  const header = [
    'Order Number', 'Customer Name', 'Status', 'Created At', 'Last Updated', 'Phone Number',
    'Address', 'City', 'Cash On Delivery', 'Shipping', 'Notes',
    'Country', 'Tax Profit', 'Order Profit', '', '', 'Products', 'Quantity', 'Prices', '', '', '', 'Order ID on your store'
  ];
  const rows = [
    header,
    [
      'EG-OLD-DELIVERED', 'Customer', 'تم التوصيل', '2025-10-06', '2026-03-01', '0500000000',
      'Address', 'Cairo', '300', '0', '',
      'EG', '0', '250', '', '', 'EG-SKU-1', '1', '300', '', '', '', 'STORE-EG-1'
    ],
  ];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(rows), 'Orders');
  const buffer = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
  const parsed = parseFullMonthSnapshot(buffer, { dateFrom: '2026-03-01', dateTo: '2026-03-31' });
  if (parsed.length !== 1) throw new Error('Delivered updated-date parser test should include one row');
  check('Delivered updated-date parser bucket', parsed[0].orderStatusBucket, 'delivered');
  check('Delivered updated-date parser dashboard date', parsed[0].dashboardDate, '2026-03-01');
  check('Delivered updated-date parser deliveredAt', parsed[0].deliveredAt, '2026-03-01');
  check('Delivered updated-date parser profit after tax', parsed[0].profitAfterTax, 250);
}

(async function main() {
  verifySection8TopProductsProfitBinding();
  verifyArabicDecimalParsing();
  verifyDeliveredUpdatedDateParsing();
  ...
  assert(stageIds.length === EXPECTED_STATUS_ORDER.length, 'Dashboard emits exactly 14 KHOD WHAAT status stages');
  assert(JSON.stringify(stageIds) === JSON.stringify(EXPECTED_STATUS_ORDER), 'Dashboard stages use the approved KHOD WHAAT status order');
